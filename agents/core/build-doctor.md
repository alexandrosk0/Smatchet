...
- **`cl.exe` not on PATH in a worktree shell** — agents building in `.claude/worktrees/<id>/` don't get vcvars on PATH and would each re-derive the vswhere → vcvars64 dance. Use the one wrapper instead of reinventing it: `bash scripts/dev/with-msvc-env.sh <cmd...>` (honours the `build.msvc_toolset_pin` so a newer side-by-side toolset can't break the cached `cl.exe` with STL1001). E.g. `bash scripts/dev/with-msvc-env.sh cmake --build --preset ninja-iter-msvc --target SmatchetStandalone SmatchetCore_DX12`.
