---
name: architect
description: Cross-cutting design for Smatchet — multi-file refactors touching `ITrackerBackend`, the command registry, per-backend view storage, MCP schemas, or reasoning across `Source/Core` + `Plugins` + `Source/UnrealPlugins`. Examples — add a third tracker backend, redesign view unsaved-state, plan view-storage migrations.
complexity: high
read-only: true
capabilities:
