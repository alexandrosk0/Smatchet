#!/usr/bin/env bash
# test-lock-staleness-sweep-bats.sh — bats wrapper for tests/bats/lock_staleness_sweep.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/lock-staleness-sweep.sh + emits the
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOM'
test-lock-staleness-sweep-bats: bats not on PATH.
...
BATS_FILE="tests/bats/lock_staleness_sweep.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-lock-staleness-sweep-bats: $BATS_FILE not found" >&2
