#!/usr/bin/env bash
# test-guard-head-drift-bats.sh — bats wrapper for tests/bats/guard_head_drift.bats.
#
...
# docs/harness/claude-code/hooks/guard-head-drift.sh (the PreToolUse HEAD-drift
# + no-direct-commit guard): worktree-targeted `git -C` commit allowance,
# PowerShell-tool coverage, drift denies, override + no-baseline paths. Emits
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-guard-head-drift-bats: bats not on PATH.
...
BATS_FILE="tests/bats/guard_head_drift.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-guard-head-drift-bats: $BATS_FILE not found" >&2
