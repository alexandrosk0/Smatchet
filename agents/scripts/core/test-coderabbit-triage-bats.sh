#!/usr/bin/env bash
# test-coderabbit-triage-bats.sh — bats wrapper for tests/bats/coderabbit_triage.bats.
#
...
#
# Wraps the bats suite for agents/scripts/core/coderabbit-triage.py `selftest`
# (the md<->py rules-version drift guard) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-coderabbit-triage: bats not on PATH.
...
BATS_FILE="tests/bats/coderabbit_triage.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-coderabbit-triage: $BATS_FILE not found" >&2
