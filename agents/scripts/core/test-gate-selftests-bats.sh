#!/usr/bin/env bash
# test-gate-selftests-bats.sh — bats wrapper for tests/bats/gate_selftests.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/test-gate-selftests.sh
# (the "gate that gates the gates"; Gap C / close-gate-gaps Slice 3) + emits the
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-gate-selftests-bats: bats not on PATH.
...
BATS_FILE="tests/bats/gate_selftests.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-gate-selftests-bats: $BATS_FILE not found" >&2
