#!/usr/bin/env bash
# test-script-freshness-bats.sh — bats wrapper for tests/bats/script_freshness.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/lib/script-freshness.sh
# (the shared gate-logic staleness detector) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-script-freshness: bats not on PATH.
...
BATS_FILE="tests/bats/script_freshness.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-script-freshness: $BATS_FILE not found" >&2
