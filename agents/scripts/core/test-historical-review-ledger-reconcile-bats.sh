#!/usr/bin/env bash
# test-historical-review-ledger-reconcile-bats.sh — bats wrapper for
# tests/bats/historical_review_ledger_reconcile.bats.
#
...
#
# Wraps the bats regression suite for
# agents/scripts/core/historical-review-ledger-reconcile.sh (PR-8: the
# historical-review ledger staleness reconcile pass + SessionStart freshness
# nudge) + emits the canonical `Passed: N  Failed: M` line test-all.sh greps for.
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-historical-review-ledger-reconcile-bats: bats not on PATH.
...
BATS_FILE="tests/bats/historical_review_ledger_reconcile.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-historical-review-ledger-reconcile-bats: $BATS_FILE not found" >&2
