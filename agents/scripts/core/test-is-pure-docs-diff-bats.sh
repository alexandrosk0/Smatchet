#!/usr/bin/env bash
# test-is-pure-docs-diff-bats.sh — bats wrapper for tests/bats/is_pure_docs_diff.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/is-pure-docs-diff.sh + emits the
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOM'
test-is-pure-docs-diff-bats: bats not on PATH.
...
BATS_FILE="tests/bats/is_pure_docs_diff.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-is-pure-docs-diff-bats: $BATS_FILE not found" >&2
