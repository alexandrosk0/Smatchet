#!/usr/bin/env bash
# test-fail-open-authoring-bats.sh — bats wrapper for tests/bats/fail_open_authoring.bats.
#
# Bucket A (CLI) per AGENTS.md § Verification automation. Zero manual steps.
# Auto-enrolled by scripts/dev/test-all.sh via the test-*.sh glob (orphan-bats gate).
# System under test: agents/scripts/core/test-fail-open-authoring.sh (PR-5 fail-open
# authoring-lint). See that bats file's header for detail.
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-fail-open-authoring-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/fail_open_authoring.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-fail-open-authoring-bats: $BATS_FILE not found" >&2
