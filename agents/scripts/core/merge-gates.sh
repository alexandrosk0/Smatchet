...
poll_merge_gates() {
    ...
    for ((p=0; p<MAX_POLLS; p++)); do
        ...
        if [ "$dup_masked_count" -gt 0 ]; then
            echo "WARN: ${dup_masked_count} check name(s) carry a blocking context from an older workflow run that the duplicate collapse discarded: ${dup_masked_names}. The gate follows the newest run, which is what GitHub evaluates, so this does NOT block. If a merge is nonetheless refused with '405 Required status check ... is cancelled', this is the cause: re-run the workflow run that owns the stale check — no push, no force, no PR-body re-pin — and the message moves 'is cancelled' -> 'is expected' -> mergeable. The run id is not in this poll (GraphQL types WorkflowRun.databaseId as a signed 32-bit Int and live ids overflow it); read it from REST, which has no such ceiling: gh api \"repos/${owner}/${repo}/commits/${head_sha}/check-runs?per_page=100\" --jq '.check_runs[] | [.name, (.conclusion // .status), .details_url] | @tsv' — match the name(s) above, take the number after /runs/ in details_url, then gh run rerun <that-id>. See docs/agent-rules/merge-gates.md § Duplicate check-name divergence." >&2
        fi
