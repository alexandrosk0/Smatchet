#!/usr/bin/env bash
# test-unwatched-pr-nudge-bats.sh — bats wrapper for tests/bats/unwatched_pr_nudge.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/unwatched-pr-nudge.sh
# (the unwatched-open-agent-PR SessionStart nudge) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-unwatched-pr-nudge-bats: bats not on PATH.
...
BATS_FILE="tests/bats/unwatched_pr_nudge.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-unwatched-pr-nudge-bats: $BATS_FILE not found" >&2
