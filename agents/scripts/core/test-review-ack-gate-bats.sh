#!/usr/bin/env bash
# test-review-ack-gate-bats.sh — bats wrapper for tests/bats/review_ack_gate.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOM'
test-review-ack-gate-bats: bats not on PATH.
...
BATS_FILE="tests/bats/review_ack_gate.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-review-ack-gate-bats: $BATS_FILE not found" >&2
