#!/usr/bin/env bash
# test-small-helper-audit-bats.sh — bats wrapper for tests/bats/small_helper_audit.bats.
#
...
# test-orphan-bats.sh: every tests/bats/*.bats must be named by a test-*.sh wrapper
# or CI never runs it. See that bats file's header for the system under test
# (agents/scripts/core/small_helper_audit.py, gate-blind-spot-sweep Slice 2).
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-small-helper-audit-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/small_helper_audit.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-small-helper-audit-bats: $BATS_FILE not found" >&2
