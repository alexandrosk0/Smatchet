#!/usr/bin/env bash
# test-pre-push-guard-bats.sh — bats wrapper for tests/bats/pre_push_guard.bats.
#
...
#
# Wraps the bats regression suite for the (A) direct-push-to-protected-branch
# hard-stop in scripts/git-hooks/pre-push: a push whose destination ref is
# develop/main is refused (with a SMATCHET_ALLOW_DEVELOP_PUSH=1 override),
# while feature pushes + branch deletes pass. Emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'MSG'
test-pre-push-guard-bats: bats not on PATH.
...
BATS_FILE="tests/bats/pre_push_guard.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-pre-push-guard-bats: $BATS_FILE not found" >&2
