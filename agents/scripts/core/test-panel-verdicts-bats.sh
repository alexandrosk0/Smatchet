#!/usr/bin/env bash
# test-panel-verdicts-bats.sh — bats wrapper for tests/bats/panel_verdicts.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/panel_verdicts.py
# (the panel-verdict -> verifier-sample adapter, Phase 4 of
# docs/plans/absorb-whip-process.md) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-panel-verdicts-bats: bats not on PATH.
...
BATS_FILE="tests/bats/panel_verdicts.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-panel-verdicts-bats: $BATS_FILE not found" >&2
