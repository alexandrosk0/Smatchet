#!/usr/bin/env bash
# test-safe-admin-merge-bats.sh — bats wrapper for tests/bats/safe_admin_merge.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/safe-admin-merge.sh
# (the admin-merge guard that makes the green assertion an EXIT CODE — the
# preventing gate for the #1180 escape, postmortems.md 2026-06-13) and emits the
# canonical `Passed: N  Failed: M` line that test-all.sh greps for.
#
# selftest: asserts-failure: n/a — pure bats dispatcher (delegates the
#   asserts-failure obligation to the suite, which exercises the guard's REFUSE
#   path; safe-admin-merge.sh itself carries the asserts-failure --selftest).
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-safe-admin-merge-bats: bats not on PATH.
...
BATS_FILE="tests/bats/safe_admin_merge.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-safe-admin-merge-bats: $BATS_FILE not found" >&2
