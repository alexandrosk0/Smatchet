#!/usr/bin/env bash
# test-historical-review-survivors-bats.sh — bats wrapper for tests/bats/historical_review_survivors.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-historical-review-survivors-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/historical_review_survivors.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-historical-review-survivors-bats: $BATS_FILE not found" >&2
