#!/usr/bin/env bash
# test-capture-intent-bats.sh — bats wrapper for tests/bats/capture_intent.bats.
#
...
#
# Wraps the bats regression suite for the prompt-intent capture pipeline
# (agents/scripts/core/redact-intent.py — the security redactor — and
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-capture-intent-bats: bats not on PATH.
...
BATS_FILE="tests/bats/capture_intent.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-capture-intent-bats: $BATS_FILE not found" >&2
