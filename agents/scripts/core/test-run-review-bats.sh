#!/usr/bin/env bash
# test-run-review-bats.sh — bats wrapper for tests/bats/run_review.bats.
#
...
#
# Wraps the regression suite for the review-panel launcher
# (agents/scripts/core/run-review.sh — leg fan-out, command templates, watcher
# landing, write-guard integration; docs/agent-rules/review-panels.md)
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOM'
test-run-review-bats: bats not on PATH.
...
BATS_FILE="tests/bats/run_review.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-run-review-bats: $BATS_FILE not found" >&2
