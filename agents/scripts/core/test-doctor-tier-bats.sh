#!/usr/bin/env bash
# test-doctor-tier-bats.sh — bats wrapper for tests/bats/doctor_tier.bats.
#
...
#
# Covers scripts/dev/doctor.sh --tier (finding C3 / Proposal P5): a required
# toolchain check whose tool is absent AND not in the active tier's `expects`
# list reports [n/a] instead of a RED [FAIL], so a Linux-container / ci-ubuntu
# environment does not flat-RED on the MSVC toolchain that is not its job.
# Emits the canonical `Passed: N  Failed: M` line test-all.sh greps for.
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'MSG'
test-doctor-tier-bats: bats not on PATH.
...
BATS_FILE="tests/bats/doctor_tier.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-doctor-tier-bats: $BATS_FILE not found" >&2
