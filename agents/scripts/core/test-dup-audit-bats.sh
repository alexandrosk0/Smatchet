#!/usr/bin/env bash
# test-dup-audit-bats.sh — bats wrapper for tests/bats/dup_audit.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/dup_audit.py (the DRY-pillar
# duplication delta gate, dry-pillar-dup-gate Slice 1) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-dup-audit-bats: bats not on PATH.
...
BATS_FILE="tests/bats/dup_audit.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-dup-audit-bats: $BATS_FILE not found" >&2
