#!/usr/bin/env bash
# test-pr-burst-guard-bats.sh — bats wrapper for tests/bats/pr_burst_guard.bats.
#
...
#
# Wraps the bats regression suite for scripts/dev/pr-burst-guard.sh (the CodeRabbit-quota PR-burst advisory).
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'MSG'
test-pr-burst-guard-bats: bats not on PATH.
...
BATS_FILE="tests/bats/pr_burst_guard.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-pr-burst-guard-bats: $BATS_FILE not found" >&2
