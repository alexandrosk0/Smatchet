#!/usr/bin/env bash
# test-verifier-review-gate-bats.sh — bats wrapper for tests/bats/verifier_review_gate.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOM'
test-verifier-review-gate-bats: bats not on PATH.
...
BATS_FILE="tests/bats/verifier_review_gate.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-verifier-review-gate-bats: $BATS_FILE not found" >&2
