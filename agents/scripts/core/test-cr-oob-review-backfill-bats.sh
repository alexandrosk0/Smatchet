#!/usr/bin/env bash
# test-cr-oob-review-backfill-bats.sh — bats wrapper for tests/bats/cr_oob_review_backfill.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/cr-oob-review-backfill.sh
# (the cr-out-of-band retroactive review backfill drip) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-cr-oob-review-backfill-bats: bats not on PATH.
...
BATS_FILE="tests/bats/cr_oob_review_backfill.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-cr-oob-review-backfill-bats: $BATS_FILE not found" >&2
