#!/usr/bin/env bash
# test-required-context-parity-bats.sh — bats wrapper for tests/bats/required_context_parity.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-required-context-parity-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/required_context_parity.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-required-context-parity-bats: $BATS_FILE not found" >&2
