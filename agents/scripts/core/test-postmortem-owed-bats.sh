#!/usr/bin/env bash
# test-postmortem-owed-bats.sh — bats wrapper for tests/bats/postmortem_owed.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/postmortem-owed.sh (the
# gate-escape detector) + emits the canonical `Passed: N  Failed: M` line that
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-postmortem-owed: bats not on PATH.
...
BATS_FILE="tests/bats/postmortem_owed.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-postmortem-owed: $BATS_FILE not found" >&2
