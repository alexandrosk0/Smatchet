#!/usr/bin/env bash
# test-resolve-py-bats.sh — bats wrapper for tests/bats/resolve_py.bats.
#
# Bucket A (CLI) per AGENTS.md § Verification automation. Zero manual steps.
# Auto-enrolled by scripts/dev/test-all.sh via the test-*.sh glob. Added with
# the suite itself (the orphan-bats gate requires every bats file to have a
# wrapper so it actually runs in CI). System under test: lib/resolve-py.sh —
# see the bats file's header (py-probe-single-candidate-residual).
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-resolve-py-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/resolve_py.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-resolve-py-bats: $BATS_FILE not found" >&2
