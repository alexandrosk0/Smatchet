#!/usr/bin/env bash
# test-markdown-links-bats.sh — bats wrapper for tests/bats/markdown_links.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-markdown-links-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/markdown_links.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-markdown-links-bats: $BATS_FILE not found" >&2
