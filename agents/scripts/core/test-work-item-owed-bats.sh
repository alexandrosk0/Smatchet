#!/usr/bin/env bash
# test-work-item-owed-bats.sh — bats wrapper for tests/bats/work_item_owed.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/work-item-owed.sh
# (the open-work-item SessionStart nudge) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-work-item-owed-bats: bats not on PATH.
...
BATS_FILE="tests/bats/work_item_owed.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-work-item-owed-bats: $BATS_FILE not found" >&2
