#!/usr/bin/env bash
# test-oob-label-impl-bats.sh — bats wrapper for tests/bats/oob_label_impl.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/test-oob-label-impl.sh (documented-vs-implemented *-out-of-band label parity).
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'MSG'
test-oob-label-impl-bats: bats not on PATH.
...
BATS_FILE="tests/bats/oob_label_impl.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-oob-label-impl-bats: $BATS_FILE not found" >&2
