#!/usr/bin/env bash
# test-pre-push-format-delta-bats.sh — bats wrapper for
# tests/bats/pre_push_format_delta.bats.
#
...
#
# Covers the delta-aware clang-format step of the pre-push local CI gate:
# pre-existing drift in a touched file must not refuse a clean change, while a
# badly formatted new hunk still does. Emits the canonical
# `Passed: N  Failed: M` line test-all.sh greps for.
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'MSG'
test-pre-push-format-delta-bats: bats not on PATH.
...
BATS_FILE="tests/bats/pre_push_format_delta.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-pre-push-format-delta-bats: $BATS_FILE not found" >&2
