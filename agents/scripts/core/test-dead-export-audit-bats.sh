#!/usr/bin/env bash
# test-dead-export-audit-bats.sh — bats wrapper for tests/bats/dead_export_audit.bats.
#
...
# (agents/scripts/core/dead_export_audit.py, gate-blind-spot-sweep Slice 1).
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-dead-export-audit-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/dead_export_audit.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-dead-export-audit-bats: $BATS_FILE not found" >&2
