#!/usr/bin/env bash
# test-fleet-preflight-bats.sh — bats wrapper for tests/bats/fleet_preflight.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/fleet-preflight.sh (the
# Workflow-fleet pre-launch linter) + emits the canonical `Passed: N  Failed: M`
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-fleet-preflight: bats not on PATH.
...
BATS_FILE="tests/bats/fleet_preflight.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-fleet-preflight: $BATS_FILE not found" >&2
