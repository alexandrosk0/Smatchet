#!/usr/bin/env bash
# test-repo-health-facts-nudge-bats.sh — bats wrapper for tests/bats/repo_health_facts_nudge.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/repo-health-facts-nudge.sh
# (the repo-health facts staleness SessionStart nudge) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-repo-health-facts-nudge-bats: bats not on PATH.
...
BATS_FILE="tests/bats/repo_health_facts_nudge.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-repo-health-facts-nudge-bats: $BATS_FILE not found" >&2
