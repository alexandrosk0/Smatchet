#!/usr/bin/env bash
# test-function-size-bats.sh — bats wrapper for tests/bats/function_size.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-function-size-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/function_size.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-function-size-bats: $BATS_FILE not found" >&2
