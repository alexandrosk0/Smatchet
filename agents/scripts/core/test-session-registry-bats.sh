#!/usr/bin/env bash
# test-session-registry-bats.sh — bats wrapper for tests/bats/session_registry.bats.
#
...
#
# Wraps the bats regression suite for the concurrent-session registry liveness
# layer (session-registry-lib.sh + guard-shared-tree.sh + session-tree-banner.sh):
# real-pid liveness, the dead-pid-but-fresh-ts over-block fix, legacy ppid=1 ts
# fallback, the dead+stale prune and >7d sweep, plus the lib --selftest. Emits the
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-session-registry-bats: bats not on PATH.
...
BATS_FILE="tests/bats/session_registry.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-session-registry-bats: $BATS_FILE not found" >&2
