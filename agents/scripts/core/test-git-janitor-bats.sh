#!/usr/bin/env bash
# test-git-janitor-bats.sh — bats wrapper for tests/bats/git_janitor.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/git-janitor.sh + emits the
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOM'
test-git-janitor-bats: bats not on PATH.
...
BATS_FILE="tests/bats/git_janitor.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-git-janitor-bats: $BATS_FILE not found" >&2
