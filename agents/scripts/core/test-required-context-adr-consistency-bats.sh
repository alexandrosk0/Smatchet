#!/usr/bin/env bash
# test-required-context-adr-consistency-bats.sh — bats wrapper for
...
#
# Wraps the fixture suite for test-required-context-adr-consistency.sh (an
# ADR-rejected branch_protection.required_contexts addition must FAIL) + emits
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-required-context-adr-consistency-bats: bats not on PATH.
...
BATS_FILE="tests/bats/required_context_adr_consistency.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-required-context-adr-consistency-bats: $BATS_FILE not found" >&2
