#!/usr/bin/env bash
# test-fuzz-closure-audit-bats.sh — bats wrapper for tests/bats/fuzz_closure_audit.bats.
#
# Bucket A (CLI) per AGENTS.md § Verification automation. Zero manual steps.
# Auto-enrolled by scripts/dev/test-all.sh via the test-*.sh glob, and named here
# so the orphan-bats gate (test-orphan-bats.sh) sees the suite is run. See that
# bats file's header for the system under test (the WARN-first fuzz-closure
# include-resolution gate, PR-6).
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-fuzz-closure-audit-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/fuzz_closure_audit.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-fuzz-closure-audit-bats: $BATS_FILE not found" >&2
