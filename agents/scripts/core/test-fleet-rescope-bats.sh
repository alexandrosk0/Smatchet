#!/usr/bin/env bash
# test-fleet-rescope-bats.sh — bats wrapper for tests/bats/fleet_rescope.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/fleet-rescope.sh (the
# cascade abort + re-scope planner — the launch-protocol other half of the
# read-only workflow-watchdog.sh) + emits the canonical `Passed: N  Failed: M`
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-fleet-rescope: bats not on PATH.
...
BATS_FILE="tests/bats/fleet_rescope.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-fleet-rescope: $BATS_FILE not found" >&2
