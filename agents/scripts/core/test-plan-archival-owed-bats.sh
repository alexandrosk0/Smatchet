#!/usr/bin/env bash
# test-plan-archival-owed-bats.sh — bats wrapper for tests/bats/plan_archival_owed.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/plan-archival-owed.sh
# (the archive-owed SessionStart nudge) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-plan-archival-owed-bats: bats not on PATH.
...
BATS_FILE="tests/bats/plan_archival_owed.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-plan-archival-owed-bats: $BATS_FILE not found" >&2
