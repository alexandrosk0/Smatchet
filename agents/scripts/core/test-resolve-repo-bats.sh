#!/usr/bin/env bash
# test-resolve-repo-bats.sh — bats wrapper for tests/bats/resolve_repo.bats.
#
...
#
# Wraps the bats regression suite for the shared repo-slug resolver
# (agents/scripts/core/lib/resolve-repo.sh) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-resolve-repo-bats: bats not on PATH.
...
BATS_FILE="tests/bats/resolve_repo.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-resolve-repo-bats: $BATS_FILE not found" >&2
