#!/usr/bin/env bash
# test-harness-provisioned-bats.sh — bats wrapper for
# tests/bats/harness_provisioned_doctor.bats.
#
...
#
# Covers the fresh-clone bootstrap hole surfaced by scripts/dev/doctor.sh:
# an unprovisioned tree (no .claude/hooks HEAD-drift guard) must produce a
# `[WARN] harness` line in the doctor preflight, and a wired tree `[PASS]`.
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'MSG'
test-harness-provisioned-bats: bats not on PATH.
...
BATS_FILE="tests/bats/harness_provisioned_doctor.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-harness-provisioned-bats: $BATS_FILE not found" >&2
