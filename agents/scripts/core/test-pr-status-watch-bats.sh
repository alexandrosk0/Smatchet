#!/usr/bin/env bash
# test-pr-status-watch-bats.sh — bats wrapper for tests/bats/pr_status_watch.bats.
#
...
#
# Wraps the bats regression suite for scripts/dev/pr-status-watch.sh (the per-PR
# notify-on-state helper that closes the merge-watcher-is-silent blind spot).
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'MSG'
test-pr-status-watch-bats: bats not on PATH.
...
BATS_FILE="tests/bats/pr_status_watch.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-pr-status-watch-bats: $BATS_FILE not found" >&2
