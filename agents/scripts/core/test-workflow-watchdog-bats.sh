#!/usr/bin/env bash
# test-workflow-watchdog-bats.sh — bats wrapper for tests/bats/workflow_watchdog.bats.
#
...
#
# Wraps the bats regression suite for agents/scripts/core/workflow-watchdog.sh
# (the fleet stall classifier) + emits the canonical `Passed: N  Failed: M` line
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-workflow-watchdog: bats not on PATH.
...
BATS_FILE="tests/bats/workflow_watchdog.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-workflow-watchdog: $BATS_FILE not found" >&2
