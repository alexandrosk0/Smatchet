#!/usr/bin/env bash
# test-setup-branch-protection-bats.sh — bats wrapper for
# tests/bats/setup_branch_protection.bats.
#
...
#
# Wraps the regression suite for the branch-protection PUT body built by
# agents/scripts/core/setup-branch-protection.sh (the endpoint is a full-object
# REPLACE, so a config flag missing from the body is silently switched off on
# the wire) + emits the canonical `Passed: N  Failed: M` line that test-all.sh
# greps for.
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-setup-branch-protection-bats: bats not on PATH.
...
BATS_FILE="tests/bats/setup_branch_protection.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-setup-branch-protection-bats: $BATS_FILE not found" >&2
