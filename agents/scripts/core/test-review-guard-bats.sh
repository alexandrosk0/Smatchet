#!/usr/bin/env bash
# test-review-guard-bats.sh — bats wrapper for tests/bats/review_guard.bats.
#
...
#
# Wraps the regression suite for the review panel's write guard
# (agents/scripts/core/lib/review-guard.sh — signature-keyed dirty state +
# stray-path report; docs/agent-rules/review-panels.md § Write-guard contracts)
# + emits the canonical `Passed: N  Failed: M` line that test-all.sh greps for.
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOM'
test-review-guard-bats: bats not on PATH.
...
BATS_FILE="tests/bats/review_guard.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-review-guard-bats: $BATS_FILE not found" >&2
