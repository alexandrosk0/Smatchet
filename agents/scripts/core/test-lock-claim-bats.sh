#!/usr/bin/env bash
# test-lock-claim-bats.sh — bats wrapper for tests/bats/lock_claim.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-lock-claim-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/lock_claim.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-lock-claim-bats: $BATS_FILE not found" >&2
