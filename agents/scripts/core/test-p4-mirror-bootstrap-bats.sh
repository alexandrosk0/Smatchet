#!/usr/bin/env bash
# test-p4-mirror-bootstrap-bats.sh — bats wrapper for tests/bats/p4_mirror_bootstrap.bats.
#
...
#
# Wraps the bats regression suite for scripts/dev/p4-mirror-bootstrap.sh
# (the GitHub->p4d mirror provisioning script) + emits the canonical
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-p4-mirror-bootstrap-bats: bats not on PATH.
...
BATS_FILE="tests/bats/p4_mirror_bootstrap.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-p4-mirror-bootstrap-bats: $BATS_FILE not found" >&2
