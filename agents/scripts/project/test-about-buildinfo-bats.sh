#!/usr/bin/env bash
# test-about-buildinfo-bats.sh — bats wrapper for tests/bats/about_buildinfo.bats.
#
...
# test-subsystem-docs-bats.sh.
#
# The wrapped suite drives `cmake -P` directly (codegen + dep manifest for the
# Help > About dialog) and self-skips when cmake is absent, so a cmake-less
# runner reports skips rather than failures.
#
...
if ! command -v bats >/dev/null 2>&1; then
    cat >&2 <<'EOF'
test-about-buildinfo-bats: bats not on PATH.
...
BATS_FILE="tests/bats/about_buildinfo.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-about-buildinfo-bats: $BATS_FILE not found" >&2
...
if [ "$PASSED" -eq 0 ] && [ "$FAILED" -eq 0 ]; then
    echo "test-about-buildinfo-bats: no TAP results parsed from $BATS_FILE (expected >= 1)" >&2
    exit 1
fi
