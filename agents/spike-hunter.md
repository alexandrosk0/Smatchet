...
- `MainThreadDispatcher::Drain()` (`Source_Core/include/MainThreadDispatcher.h`) runs at the head of every Draw, before any window code, draining lambdas posted by workers. Drain blocks the frame; oversized lambdas in the queue ARE spikes.
...
- **Always** name the exact exe to run after a rebuild. Multiple build outputs (`build/ninja-iter-msys2/`, `build/ninja-debug-msys2/`, worktree builds) make wrong-exe spike repro common. `ls -la` both candidates, print mtimes side-by-side, tell the user the absolute path before asking them to reproduce.
