...
Before any squash-merge (orchestrator / `git-janitor` / `smatchet-merge-watcher`), the gate-poller (`agents/scripts/core/merge-gates.sh`) checks three conditions via one `gh api graphql` call: **(1) CI** — every required check on the head reaches a passing terminal state (plus non-required checks on the *meant-to-block* allow-list — `Coverage`/`Sanitizer`/`Bucket-`, non-advisory — block too, per the #923 gate-escape fix); **(2) CodeRabbit** — `APPROVED` / `COMMENTED + 0 actionable` passes, findings / changes-requested / review-skipped block (`cr-out-of-band` label downgrades a CR block to WARN); **(3) User comments** — zero unresolved non-bot non-self threads or conversation comments. Plus PR OPEN, `reviewDecision ∈ {APPROVED, null}`, no pagination overflow.
