...
After the loop, emit the **post-ship 4-option `AskUserQuestion`** (Manual verify / Review PR / Register with watcher / Done; skip to option 3 if the user said "merge when green"). **PR batching**: one PR per logical *feature*, not per slice — related slices accumulate on one branch and ship once (respects CodeRabbit's review quota + per-PR file ceiling; split along seams if the diff would exceed the ceiling).
...
**Auto-merge applies only when explicitly authorised** (post-ship "Register with watcher" or in-session "merge when green") — then set `MERGE_GATES_FLIP_READY=true` (flips draft→ready so CR's `auto_review.drafts:false` doesn't bypass review). `SKIP_MERGE_GATES=true` bypasses globally; `tests-out-of-band` / `perf-out-of-band` downgrade named CI checks. REST merge: `gh api -X PUT repos/.../pulls/N/merge -f merge_method=squash`.
