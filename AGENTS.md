...
2. **CodeRabbit** — `APPROVED` or `COMMENTED + Actionable comments posted: 0` passes; `COMMENTED + N > 0`, `CHANGES_REQUESTED`, `DISMISSED`, `STALE_WITH_FINDINGS`, `STALE_UNKNOWN` block. `NONE` falls through after `MERGE_GATES_CR_GRACE_POLLS` (default 10) expires when CR is installed — but the poller now also auto-posts `@coderabbitai review` once per HEAD on the first blocking `NONE` poll (early-nudge, gated by `MERGE_GATES_STALE_REREVIEW_POLLS`; 0 disables), so CR usually resolves before the grace backstop. The `cr-out-of-band` PR label downgrades any CR block (state verdict **and** unresolved CR-authored threads) to WARN — CR gate only; CI + user-comment gates still bind.
