...
Canonical source: `agents/<name>.md` at repo root (per [agents.md spec](https://agents.md/)); shared scripts + skills at `agents/_shared/`. **Agent vs skill** rubric: [`docs/agent-rules/AGENT-VS-SKILL.md`](docs/agent-rules/AGENT-VS-SKILL.md) (skill = bounded-deterministic-inline; agent = exploration / loop / spawn / delegates). Per-harness adapter dirs (`.claude/`, `.codex/`, `.cursor/`) are gitignored — regenerated by `bash agents/scripts/core/setup-harness.sh <name>` (junctions/symlinks, so edits to `agents/*.md` are picked up immediately). Fresh clone: [`docs/harness/SETUP.md`](docs/harness/SETUP.md).
