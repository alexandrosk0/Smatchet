...
- **Work items** — user-gated product features run the staged loop at `docs/work/items/NN-slug/` (spec → design → plan → panel review → implement → review → close; ledgers `docs/work/{IDEAS,BACKLOG,DEFERRED}.md`): [`work-items.md`](docs/agent-rules/work-items.md) + [`review-panels.md`](docs/agent-rules/review-panels.md); loop pauses **only** at the two sign-off gates ([`ship-loops.md`](docs/agent-rules/ship-loops.md) exception 7).
