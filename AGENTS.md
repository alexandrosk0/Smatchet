...
**Draft-vs-ready at `open PR`**: the orchestrator (user's main session) opens `gh pr create` **without** `--draft` when the user has already authorised the merge intent in the same turn (post-ship option 3 / in-session "merge when green") — CodeRabbit's `auto_review.drafts: false` config (`.coderabbit.yaml`) means a draft PR is silently skipped, the gates poller sees `CodeRabbit: NONE`, treats it as pass, and the PR squash-merges before any review ran. Opening ready makes CR actually fire so the gate has a meaningful signal. The orchestrator falls back to `--draft` only when (a) the user has NOT authorised auto-merge for this PR (post-ship options 1 / 2 / 4), OR (b) the change is mid-iteration and the user wants to keep iterating before review. Spawned-child agents (`handoff-implementer`, `pr-iterator`) still open `--draft` unconditionally per § Handoff envelope § Spawned-child PR draft requirement — that contract is unchanged.
