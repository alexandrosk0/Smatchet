...
- **Weekly memory drain** — the harness auto-memory inbox (`~/.claude/projects/<slug>/memory/`) is transient, not durable. Weekly (routine `memory-weekly-drain`) triage every item: **implement** (→ `AGENTS.md` / `docs/agent-rules/*`), **backlog** (→ `docs/self-improvement/categories/*`), or **toss** — verifying each claim against the current tree first, then clearing drained rows. Spec + recipe: [`docs/agent-rules/memory-drain.md`](docs/agent-rules/memory-drain.md).
