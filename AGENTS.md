...
| `bare-json-parse-untrusted` (WARN) | changed untrusted-ingress `**/*.cpp` (curated TU set) | advisory (calibration): a bare `nlohmann::json::parse(` on CLI/MCP/Lua/HTTP/network ingress not routed via `json_safe::ParseBounded` stack-overflows the recursive parser before any try/catch (#1271/#1287); the 3-arg non-throwing form still overflows. WARN-first per the original `duplication` calibration precedent |
