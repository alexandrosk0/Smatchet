...
- **Parent Issue Hierarchy**: Show a view as a tree instead of a flat list. Two per-view switches in the grid's **Sort By ↕** popup — *Story group* nests child tasks/bugs under their parent story (Id cell indented per level, parent rows tinted lavender) and *Hide parent stories* keeps only the leaf rows. Parents that the view's filter did not return are fetched automatically after each sync batch through the backend-agnostic issue reader (Jira falls back to the inward *is part of* issue link when the `parent` field is absent); a failed parent fetch is a sync warning, never a sync error. See [Parent Issue Hierarchy](docs/guides/parent-hierarchy.md).
