...
- **Time Tracking / Logging Hours**: Add either the **Log Work** or the **Time Spent** column to your view (Views editor → Columns), then click that cell on any row to open the time-tracking dialog — enter time spent (`2w 4d 6h 45m` notation), adjust the remaining estimate, pick a start date, and add a description (with reusable templates from **Settings → Preferences → Editing → Work log templates**). Scriptable as `Smatchet.exe cmd ticket.add_worklog --id=PROJ-1 --seconds=5400 --yes` ([CLI Guide](docs/guides/cli.md)). Jira is the only backend implementing work logs today; the others surface the dialog's submit as an "unsupported by this backend" error.
...
- **Parent Issue Hierarchy**: Show a view as a tree instead of a flat list. Two per-view switches in the grid's **Sort By ↕** popup — *Parent group* nests child tasks/bugs under their parent story (Id cell indented per level, parent rows tinted lavender) and *Hide parent stories* keeps only the leaf rows. Parents that the view's filter did not return are fetched automatically after each sync batch through the backend-agnostic issue reader (Jira falls back to the inward *is part of* issue link when the `parent` field is absent); a failed parent fetch is a sync warning, never a sync error, and the fetch can be switched off globally with *Load parent issues* in Preferences → Editing → Grid behaviour. See [Parent Issue Hierarchy](docs/guides/parent-hierarchy.md).
...
- [Parent Issue Hierarchy](docs/guides/parent-hierarchy.md): Story-group tree view, hide-parent-stories toggle, automatic missing-parent fetch, and per-backend parent support.
