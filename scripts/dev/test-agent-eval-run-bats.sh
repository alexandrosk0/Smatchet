#!/usr/bin/env bash
# test-agent-eval-run-bats.sh — bats wrapper for tests/bats/agent_eval_run.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-agent-eval-run-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/agent_eval_run.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-agent-eval-run-bats: $BATS_FILE not found" >&2
