#!/usr/bin/env bash
# coverage-delta-gate.sh — refuse Source_Core/ diffs without matching test deltas.
#
# Per-PR enforcement: if the current branch's diff against `develop` (or the
# configured base) touches any production file under Source_Core/src/ AND
...
#
# Exit codes:
#   0 — gate satisfied (no Source_Core change, or test files also changed)
#   1 — gate failed (Source_Core changed without test delta)
...
for f in "${CHANGED[@]}"; do
    case "$f" in
        # Production surface that the gate cares about. Source_Core/src/*.cpp is
        # the core enforcement target; *.h headers under Source_Core/include/ are
...
if [ "${#PROD_CHANGES[@]}" -eq 0 ]; then
    echo "[coverage-delta-gate] PASS — no production Source_Core/src/*.cpp changes"
    exit 0
fi
...
echo
echo "FAIL: Source_Core/ changes without test deltas."
echo
...
echo
echo "Add tests under tests/Source_Core/ (or tests/Lua/, tests/Plugins/) for the"
