#!/usr/bin/env bash
# test-ui-toolbar-append-cache-cue.sh — bucket-E driver for the Pillar-2
# sync-I/O offload cluster (#1001, site #611).
#
# Verifies the Pillar 2 "UI never freezes without a visible cue" invariant by
# running tests/ui/toolbar_append_cache_cue.test.cpp:
#   - ToolbarAppendCue_VisibleWhileLoading — the in-progress cue is on screen
#     while the toolbar per-tracker append reload runs off-thread, gone after.
#
# Exit codes:
#   0 — every test passed
...
FILTER="${UI_TEST_FILTER:-ToolbarAppendCue}"
