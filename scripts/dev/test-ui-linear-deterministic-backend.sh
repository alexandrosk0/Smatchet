#!/usr/bin/env bash
# test-ui-linear-deterministic-backend.sh — bucket-E driver for the deterministic
# Linear fixture backend UI tests (Slice 4 of linear-tracker-backend.md).
# Sets an isolated SMATCHET_USER_DATA dir, injects the basic-grid fixture via
# SMATCHET_TEST_LINEAR_BACKEND_FIXTURE, invokes `ui_test.run`, and parses the
# JSON envelope.
#
# Exit codes:
#   0 — every test passed
...
PY="${PYTHON:-python}"
TEST_PORT="${SMATCHET_TEST_PORT:-58737}"
# imgui_test_engine filter — matches the LinearDeterministic* test group registered
# in tests/ui/linear_deterministic_backend.test.cpp.
FILTER="${UI_TEST_FILTER:-LinearDeterministic}"
FIXTURE_DIR="${SMATCHET_FIXTURE_DIR:-tests/fixtures/linear_backend}"
FIXTURE="${SMATCHET_TEST_LINEAR_BACKEND_FIXTURE:-$FIXTURE_DIR/basic-grid.json}"
...
echo "[test-ui-linear-deterministic-backend] launching ephemeral Smatchet (port $TEST_PORT)..."
echo "  fixture: $FIXTURE"

RAW_OUTPUT="$(SMATCHET_USER_DATA="$TMPDIR_DATA" \
    SMATCHET_TEST_LINEAR_BACKEND_FIXTURE="$FIXTURE" \
