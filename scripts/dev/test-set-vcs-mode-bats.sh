#!/usr/bin/env bash
# test-set-vcs-mode-bats.sh — bats wrapper for tests/bats/set_vcs_mode.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-set-vcs-mode-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/set_vcs_mode.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-set-vcs-mode-bats: $BATS_FILE not found" >&2
