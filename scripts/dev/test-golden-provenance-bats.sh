#!/usr/bin/env bash
# test-golden-provenance-bats.sh — bats wrapper for tests/bats/golden_provenance.bats.
#
# Bucket A (CLI) per AGENTS.md § Verification automation. Zero manual steps.
# Auto-enrolled by scripts/dev/test-all.sh via the test-*.sh glob, which is also
# what the orphan-bats gate requires — a bats suite with no wrapper never runs in
# CI. See that bats file's header for the system under test (#2099).
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-golden-provenance-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/golden_provenance.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-golden-provenance-bats: $BATS_FILE not found" >&2
