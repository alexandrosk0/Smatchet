...
Phase 3 of `docs/design/smatchet-merge-watcher.md`. Python port of
...
def format_triage_report(findings: list[Finding], pr: int, attempt: int, budget: int) -> str:
    lines = [
        ...
        "",
        f"_Phase 3 classifier per `docs/design/smatchet-merge-watcher.md`. Attempt {attempt}/{budget}._",
        "",
    ]
    if not findings:
        lines += [
