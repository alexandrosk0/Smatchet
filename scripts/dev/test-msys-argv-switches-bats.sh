#!/usr/bin/env bash
# test-msys-argv-switches-bats.sh — bats wrapper for tests/bats/msys_argv_switches.bats.
#
...
# test (native Windows exe /switch invocations vs MSYS argument translation).
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-msys-argv-switches-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/msys_argv_switches.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-msys-argv-switches-bats: $BATS_FILE not found" >&2
