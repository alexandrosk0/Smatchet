#!/usr/bin/env bash
# test-ui-ai-assistant-preferences.sh — bucket-E driver for the 6 AI Assistant
# Preferences flow tests added by slice 9 of autonomous-debugging-no-creds
# (docking / enter-send / validation-banner / save-discard / test-connection /
# verify-on-save). Filters on the "AiPrefsTab" category.

set -euo pipefail

EXE="${SMATCHET_EXE:-build/ninja-ui-test-msvc/Smatchet.exe}"
PY="${PYTHON:-python}"
TEST_PORT="${SMATCHET_TEST_PORT:-58803}"
FILTER="${UI_TEST_FILTER:-AiPrefsTab}"
...
echo "[test-ui-ai-assistant-preferences] launching ephemeral Smatchet (port $TEST_PORT)..."
