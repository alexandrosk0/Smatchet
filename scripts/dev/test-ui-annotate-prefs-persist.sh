#!/usr/bin/env bash
# test-ui-annotate-prefs-persist.sh — bucket-E driver for the Annotate
# Preferences edit → persist → reload round-trip ("item 5" follow-up after
# PR #568 / N14). Invokes `ui_test.run` against an ephemeral spawn, parses the
# JSON envelope, and reports Passed / Failed.
#
# Exit codes:
#   0 — every test passed
...
# NOT a glob. "AnnotatePrefs" matches both registered variants
# (EditPersistsAndReloads / EditClampsThenPersists).
FILTER="${UI_TEST_FILTER:-AnnotatePrefs}"
...
echo "[test-ui-annotate-prefs-persist] launching ephemeral Smatchet (port $TEST_PORT)..."
