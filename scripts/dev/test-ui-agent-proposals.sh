#!/usr/bin/env bash
# test-ui-agent-proposals.sh — bucket-E driver for the T6 Agent proposals
# panel (Empty / ListsPending / Approve / Reject variants). Invokes
...
# Substring match — "AgentProposals" matches all four registered variants.
FILTER="${UI_TEST_FILTER:-AgentProposals}"
...
echo "[test-ui-agent-proposals] launching ephemeral Smatchet (port $TEST_PORT)..."
