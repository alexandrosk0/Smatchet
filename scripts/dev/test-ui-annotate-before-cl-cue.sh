#!/usr/bin/env bash
# test-ui-annotate-before-cl-cue.sh — bucket-E driver for the Pillar-2
# sync-I/O offload cluster (#1001, site #761).
#
# Verifies the Pillar 2 "UI never freezes without a visible cue" invariant by
# running tests/ui/annotate_before_cl_cue.test.cpp:
#   - AnnotateBeforeClCue_VisibleWhileResolving — the in-progress cue is on
#     screen while the day→CL resolve runs off-thread, and gone once it lands.
#
# Exit codes:
#   0 — every test passed
...
FILTER="${UI_TEST_FILTER:-AnnotateBeforeClCue}"
...
echo "[test-ui-annotate-before-cl-cue] launching ephemeral Smatchet (port $TEST_PORT)..."
