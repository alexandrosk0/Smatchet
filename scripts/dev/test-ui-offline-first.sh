#!/usr/bin/env bash
# test-ui-offline-first.sh — bucket-E driver for offline-first UI tests (Slice 3 Quality Pillar 6).
# Sets an isolated SMATCHET_USER_DATA dir, injects the offline-first fixture via
# SMATCHET_TEST_OFFLINE_FIRST_FIXTURE, invokes `ui_test.run`, and parses the
# JSON envelope.
#
# Exit codes:
#   0 — every test passed
...
# imgui_test_engine filter — matches the OfflineFirst* test group registered
# in tests/ui/offline_first.test.cpp.
FILTER="${UI_TEST_FILTER:-OfflineFirst}"
FIXTURE_DIR="${SMATCHET_FIXTURE_DIR:-tests/fixtures/jira_backend}"
FIXTURE="${SMATCHET_TEST_OFFLINE_FIRST_FIXTURE:-$FIXTURE_DIR/offline-first.json}"
...
echo "[test-ui-offline-first] launching ephemeral Smatchet (port $TEST_PORT)..."
