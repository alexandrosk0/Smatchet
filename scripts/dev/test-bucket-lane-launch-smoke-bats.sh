#!/usr/bin/env bash
# test-bucket-lane-launch-smoke-bats.sh — bats wrapper for tests/bats/bucket_lane_launch_smoke.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-bucket-lane-launch-smoke-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/bucket_lane_launch_smoke.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-bucket-lane-launch-smoke-bats: $BATS_FILE not found" >&2
