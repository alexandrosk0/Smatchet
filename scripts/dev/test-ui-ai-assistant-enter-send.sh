#!/usr/bin/env bash
# test-ui-ai-assistant-enter-send.sh — bucket-E driver for the Assistant
# chat input keyboard contract (Enter submits, Ctrl+Enter newline, empty
# submit guarded). Invokes `ui_test.run` against an ephemeral spawn, parses
...
echo "[test-ui-ai-assistant-enter-send] launching ephemeral Smatchet (port $TEST_PORT)..."
