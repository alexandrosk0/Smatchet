#!/usr/bin/env bash
# test-ui-ai-assistant-panel-dock-swap.sh — bucket-E driver for the AI
# Assistant dock-swap toggle (state-replica primary + live-host probe
# informational). Invokes `ui_test.run` against an ephemeral spawn, parses
# the JSON envelope, and reports Passed / Failed.
#
# Exit codes:
#   0 — every test passed
...
PY="${PYTHON:-python}"
TEST_PORT="${SMATCHET_TEST_PORT:-58740}"
# imgui_test_engine filter is substring-match (not glob). "DockSwap" matches
# both _StateReplica_TogglesAllFlags + _LiveHostProbe_PanelMigratesNode.
FILTER="${UI_TEST_FILTER:-DockSwap}"
...
echo "[test-ui-ai-assistant-panel-dock-swap] launching ephemeral Smatchet (port $TEST_PORT)..."
