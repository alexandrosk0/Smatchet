#!/usr/bin/env bash
# test-verifier-calibrate-bats.sh — bats wrapper for tests/bats/verifier_calibrate.bats.
#
...
# verifier advisory-to-blocking calibration report (offline). See that bats file's header for the
# system under test.
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-verifier-calibrate-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/verifier_calibrate.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-verifier-calibrate-bats: $BATS_FILE not found" >&2
