#!/usr/bin/env bash
# test-build-warnings-gate-bats.sh — bats wrapper for tests/bats/build_warnings_gate.bats.
#
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-build-warnings-gate-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/build_warnings_gate.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-build-warnings-gate-bats: $BATS_FILE not found" >&2
