#!/usr/bin/env bash
# test-release-signing-selectors-bats.sh — bats wrapper for
# tests/bats/release_signing_selectors.bats.
#
...
# test (release-github.sh signing selectors, including Azure Trusted Signing).
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-release-signing-selectors-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/release_signing_selectors.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-release-signing-selectors-bats: $BATS_FILE not found" >&2
