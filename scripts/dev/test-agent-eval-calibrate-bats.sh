#!/usr/bin/env bash
# test-agent-eval-calibrate-bats.sh — bats wrapper for tests/bats/agent_eval_calibrate.bats.
#
# Bucket A (CLI) per AGENTS.md § Verification automation. Zero manual steps.
# Auto-enrolled by scripts/dev/test-all.sh via the test-*.sh glob. Pairs the new
# judge-vs-human calibration gate (scripts/dev/agent-eval-calibrate.py, PR-23)
# with a wrapper so the orphan-bats gate is satisfied. See that bats file's
# header for the system under test.
#
# Exit codes:
...
if ! command -v bats >/dev/null 2>&1; then
    echo "test-agent-eval-calibrate-bats: bats not on PATH (npm i -g bats). See BUILD.md § Dev-script CLI tools." >&2
    echo "Passed: 0  Failed: 0  (skipped — bats missing)"
    exit 2
fi

BATS_FILE="tests/bats/agent_eval_calibrate.bats"
if [ ! -f "$BATS_FILE" ]; then
    echo "test-agent-eval-calibrate-bats: $BATS_FILE not found" >&2
