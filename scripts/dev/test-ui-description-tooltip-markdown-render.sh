#!/usr/bin/env bash
# test-ui-description-tooltip-markdown-render.sh — bucket-E driver for the
# description grid-cell tooltip markdown-render coverage added by slice 9 of
# autonomous-debugging-no-creds. Defensive cover for the be2b1d9
# wrapWidth-tooltip-render regression.

set -euo pipefail

EXE="${SMATCHET_EXE:-build/ninja-ui-test-msys2/Smatchet.exe}"
PY="${PYTHON:-python}"
TEST_PORT="${SMATCHET_TEST_PORT:-58804}"
FILTER="${UI_TEST_FILTER:-DescriptionTooltip}"
...
echo "[test-ui-description-tooltip-markdown-render] launching ephemeral Smatchet (port $TEST_PORT)..."
