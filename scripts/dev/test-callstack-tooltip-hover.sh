#!/usr/bin/env bash
# test-callstack-tooltip-hover.sh — bucket-E driver guarding the
# RenderClippedFieldText BeginGroup/EndGroup wrap (PR #147). Mirrors
# test-ui-views-columns-reorder.sh shape: ephemeral spawn → ui_test.run →
# parse JSON envelope → exit 0 / 1 / 2.
#
# Exit codes:
#   0 — every test passed
...
PY="${PYTHON:-python}"
TEST_PORT="${SMATCHET_TEST_PORT:-58734}"
# imgui_test_engine filter is substring + ^/$/, modifiers, NOT a glob.
# "CallstackTooltipHover" matches all three registered variants.
...
echo "[test-callstack-tooltip-hover] launching ephemeral Smatchet (port $TEST_PORT)..."
