...
- Open each feature PR **READY, not draft** (so CodeRabbit reviews immediately — its `auto_review.drafts: false` skips drafts; this also closes the "draft PRs silently bypassed CR" incident at the root). Do **not** present the post-ship menu and do **not** ask the user to mark-ready or authorise the merge.
