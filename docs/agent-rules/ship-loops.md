...
7. **Plan-archive (if this work shipped from a plan-doc)**: the post-ship PR that fills `docs/plans/active/<slug>.md` § Implementation log MUST, in the **same** PR, flip the plan's `> **Status**:` header to `shipped` AND `git mv` it to `docs/plans/shipped/` + sweep refs + `test-plan-index.sh --fix` (`docs/agent-rules/process-rules.md` § Archive on revision; template § Archive checklist). The `git mv` is the historically-dropped step — `plan-archival-owed.sh` nags at SessionStart for any `active/` plan already marked `shipped`. DO NOT defer the move to a later PR.
