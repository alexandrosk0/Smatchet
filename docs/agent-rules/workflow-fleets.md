...
## Verify-stage schema — make downgrade-only fields OPTIONAL
...
**Running `bash agents/scripts/core/fleet-preflight.sh <workflow-script> --strict` and seeing it exit 0 is a MANDATORY pre-launch step — a MUST, not merely "available" — before any `Workflow` fan-out above 2 agents** (the § Concurrency / slot threshold). A hard violation makes `--strict` exit non-zero and **blocks the launch**: at minimum a multi-agent `parallel()`/`pipeline()` fan-out with an unpinned `agent()` (failure 2 — the TPM-starvation death this whole doc was written from). Fix the WARNs (pin every model, scope every prompt) and re-run until `--strict` is clean; do not launch on a non-zero exit. `fleet-preflight.sh --selftest` proves the gate is live (a violating fixture fails `--strict`, a clean one passes).

> **Mechanical enforcement (opt-in):** the MUST above is also enforced by a `PreToolUse` hook on the `Workflow` tool — [`pretool-workflow-fleet-preflight.sh`](../harness/claude-code/hooks/pretool-workflow-fleet-preflight.sh), wired via the `Workflow` matcher in [`settings.json.tmpl`](../harness/claude-code/settings.json.tmpl). It runs `fleet-preflight.sh --strict` on the inline `script` (or `scriptPath`) and **blocks the launch (exit 2)** when a hard violation lands in an above-threshold fan-out (`agent()` count over a configurable minimum, default 2 — so a 1–2 agent helper is never blocked). It is **fail-open** (no jq / no preflight / no inline script → allow) and downgradable to advisory or fully off via the hook's env knobs (see the hook header). It takes effect once `settings.json.tmpl` is applied (re-run the harness setup); until then the `--strict` run above remains the documented MUST enforced by author discipline.
