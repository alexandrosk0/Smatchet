...
> **Deferred follow-up (not implemented):** wiring `fleet-preflight.sh --strict` into a `PreToolUse` hook on the `Workflow` tool would enforce this mechanically rather than by convention. That alters behaviour for every session and needs explicit user opt-in (it edits `settings.json`), so it is left as a noted deferral, tracked in [`tooling.md`](../self-improvement/categories/tooling.md) `fleet-preflight-not-enforced-at-launch`. Until then the `--strict` run above is a documented MUST enforced by author discipline.
