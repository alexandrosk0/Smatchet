...
**Archive on revision**: the same commit that populates § Implementation log + § Deviations + § Verification (actual) MUST also `git mv docs/design/<slug>.md docs/design/archive/<slug>.md` and grep-then-sed every cross-reference to the new path across `docs/`, `agents/`, `scripts/`, `Source_Core/`, `AGENTS.md`. A plan-doc lives in `docs/design/` only while it's still driving in-flight implementation work; once the post-ship sections are populated the doc is reference material and belongs in `archive/`. Two-step check before declaring the archive PR done: (1) `grep -rln "docs/design/<slug>\.md"` returns zero hits to the old path; (2) every former hit has been updated to `docs/design/archive/<slug>.md`. Apply the `tests-out-of-band` PR label if `Source_Core/` comment refs got touched — the test-delta gate counts comment-only changes as production changes; the label downgrade is the gate's own documented dismissal for docs-only diffs. Without this rule, ~62% of post-ship plans drifted into stale-in-place state (empirical sweep 2026-05-28 found 33 of 48 plans in `docs/design/` were eligible for archive but had been left behind).
...
The `agent/<id>` carve-out was deleted post-`ClaudeCodeLocalRunner` (per v1 of `docs/design/archive/github-tracker-backend.md`) and is reinstated here for the p4-task-stream surface: those branches are recovery-style throwaways, created by the script after a successful shelf submit, and they should never carry non-self commits. The `smatchet-merge-watcher` host daemon (per `docs/design/archive/smatchet-merge-watcher.md`) runs in-process, not as a spawned subprocess, so it has no worktree branch that would need this carve-out.
