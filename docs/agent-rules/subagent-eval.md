...
- **What it measures.** For each `(entry, dimension)` it runs the same external judge command the scorer uses (`--judge-cmd` / `$SMATCHET_AGENT_EVAL_JUDGE_CMD`) over the recorded output and computes `|judge − human|`.
