...
Production-resident, header-only helper (shipped by slice 7 of [`docs/design/archive/autonomous-debugging-no-creds.md`](../design/archive/autonomous-debugging-no-creds.md)). Distinct from the **per-investigation template** at `agents/_shared/templates/SmatchetAgentDebug.h.tmpl` (the existing template stays as-is and remains the hypothesis-tracking workflow for `[temp-debug]` markers). Both helpers coexist; no migration.
