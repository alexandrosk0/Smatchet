...
| `offline-sync` | low · read-edit | SQLite cache, offline-queue replay, audit trail AND offline-first reads (Quality Pillar 6) — `LocalCacheManager`, `OfflineQueueService`, `SmatchetOfflineQueueUi`, `TicketSyncService`, `BackendAuditTrail`, `FieldEditAuditSource`, `KeyedLookupCache`, `lookup_cache`, `DataFreshnessCue`. Cache schema additions, queue replay, dead-letter handling, sync diff resolution, audit entries, offline-first reads + freshness cues (v3: Pillar-6 rules+workflow; see agent prompt). |
