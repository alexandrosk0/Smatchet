...
| `offline-sync` | low · read-edit | SQLite cache, offline-queue replay, audit trail AND offline-first reads (Quality Pillar 6) — `LocalCacheManager`, `OfflineQueueService`, `SmatchetOfflineQueueUi`, `TicketSyncService`, `BackendAuditTrail`, `FieldEditAuditSource`, `KeyedLookupCache`, `lookup_cache`, `DataFreshnessCue`. Schema additions, replay, dead-letter, conflict resolution, and any feature that must keep working with the tracker unreachable. |
