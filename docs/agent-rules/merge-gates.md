...
**GitHub merge queue (develop)**: develop is moving behind a GitHub merge queue. The 5 branch-protection required checks (`Test-delta gate`, `Windows + MSVC`, `Windows + MSVC (Smatchet light — AI/Whisper/MCP off)`, `Shell lint (shellcheck)`, `Doc anchors + agent contract`) gate the merge by re-running on the queue's synthetic `merge_group` ref — so all 4 hosting workflows (`build-and-test.yml`, `coverage-gate.yml`, `shell-lint.yml`, `doc-validation.yml`) now trigger on `merge_group` and every required job either runs there or self-gates to a green no-op reporting the same context (a required check that never reports on `merge_group` deadlocks the queue forever — the #880/#881/#882 path-filtered-required-check class). When a queue is active a **direct** REST merge (`PUT pulls/{pr}/merge`) returns 405; the queue-safe path is **enable auto-merge** (`gh pr merge --squash --auto`), which merges immediately if no queue is set and enqueues if one is. `smatchet-merge-watcher`'s PASS handler uses `--auto` accordingly (`squash_merge_pr`), keeping its merge-gates.sh gate-poll as the pre-enqueue check. The custom poller above + the `*-out-of-band` labels remain for the legacy / manual (non-queue) path; **CodeRabbit stays PR-advisory** — it is not a queue-required `merge_group` check.
