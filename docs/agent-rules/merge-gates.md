...
Before the orchestrator, `git-janitor`, OR `smatchet-merge-watcher` (the host daemon per [`docs/design/applied/smatchet-merge-watcher.md`](../design/applied/smatchet-merge-watcher.md)) squash-merges a PR, it polls three conditions via one `gh api graphql` call (`scripts/dev/merge-gates.graphql`):
