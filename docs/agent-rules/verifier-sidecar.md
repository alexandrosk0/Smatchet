...
- `scalar` — the fallback for backends **without** logprobs (e.g. the Anthropic Messages API today): parse the single emitted letter/number into a point score. The letter parse is **anchored** — the answer must BE the letter (bare, or as the first token once punctuation is stripped). A preamble like `Score: B` is an error, not a score: an unanchored scan used to return the `S` out of `Score` (0.053, near-worst) instead of the `B` (0.947), silently mis-ranking candidates with no failure signal. Bare numbers on the **1–20 scale are ranks, like the letters — `1` is BEST, `20` is worst** (a `0`–`1` or `0`–`100` number stays higher-is-better).

Config via `VERIFIER_BASE_URL` / `VERIFIER_MODEL` / `VERIFIER_API_KEY` (or flags). `VERIFIER_API_KEY` rides every call as an `Authorization: Bearer` header, so the producer **refuses a cleartext `http://` base URL** when a key is set and the host is not loopback — use `https://`, a `localhost` backend, or set `VERIFIER_ALLOW_INSECURE_AUTH=1` to accept the exposure with a warning. Two more transport guards apply when a key is set: an authenticated call **never follows HTTP redirects** (urllib would re-send the bearer to whatever URL the server named, cross-authority or downgraded to `http://` — a redirecting endpoint fails loudly instead), and a loopback call **never routes through a configured `http_proxy`** (which would carry the cleartext bearer off-machine, defeating the loopback exemption). Repeated evaluations run the first sample at temperature 0 (the mode) and resamples hotter so `K` draws actually explore. Because calls scale as candidates × criteria × repeats, use the sidecar's pivot-tournament schedule to spend the repeat budget only on the pivots. Pipe the two halves together:
...
**The push gate** (`scripts/dev/pre-ship.sh --ack-review`) is where a verdict is actually *produced*. When `VERIFIER_BASE_URL` + `VERIFIER_MODEL` name an independent backend it runs `verifier-produce.py | verifier-sidecar.py aggregate` over the branch diff, attaches the verdict to the `branch` ack, and records the run to `.verifier-traces/` (gitignored) — those traces are the calibration evidence `verifier-calibrate.py` needs, so producing a verdict without recording one would leave the score permanently un-promotable. Three behaviours are load-bearing: **no backend configured ⇒ no verdict** (never a self-score — the whole point of independence); **backend unreachable or failing ⇒ WARN + presence-only ack**, never a blocked push (a gate that dies when an API dies is worse than no gate); **`hard_veto` ⇒ the push fails** — but note `verifier-produce.py` scores criteria only and **never emits `hard_veto`**, so with the standard producer pipeline a veto is unreachable at push time; it becomes reachable only when a caller attaches an agent-authored verifier object (`review-ack.sh --record --branch --verdict <file>`). Even a worst-case produced score (every criterion pinned to `T` → `overall_score` ≈ 0.01, `recommendation: block`) carries `hard_veto: false` and does NOT block, because gating on an uncalibrated score is exactly what rule 5 forbids. A recorded veto blocks on EVERY subsequent `pre-ship.sh` run, not merely the `--ack-review` invocation that recorded it. Tunables: `VERIFIER_MODE`, `VERIFIER_REPEATS`, `VERIFIER_DIFF_MAX_BYTES` (the diff is truncated with the truncation disclosed in the prompt, so the model never judges a fragment as if it were the whole change).
...
- **`--strict`** guards against a backend that returns *degenerate* logprobs — some DeepSeek versions have been reported to emit placeholder values (e.g. `-9999`) instead of a real distribution. With `--strict` the run fails loudly (`--record` still flushes the offending trace) instead of silently scoring garbage; drop to `--mode scalar` if that happens (the single-letter answer is still clean). Degeneracy is judged against the **raw** `top_logprobs` the backend returned, not the A–T subset left after filtering: a confident backend whose top-20 holds one letter plus whitespace/word-piece variants is healthy, and a peaked-but-real runner-up (say `B` at `-105`) is not a placeholder — both used to abort a good run, and `--strict` is in the recipe above, so that was the default path.
