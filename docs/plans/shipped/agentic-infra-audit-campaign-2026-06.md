...
>
> **Status**: `shipped` (2026-08-16) — the maintainer-gated branch-protection cluster (`merge-pipeline-01`/`-02`/`-06`, the last open item) was greenlit by the maintainer on 2026-08-16 ("full cluster": B1 + B2 + the `enforce_admins` flip) and shipped as Slices B1/B2 (see § Implementation log, final entry). **One pending operator action:** run `bash agents/scripts/core/setup-branch-protection.sh` with a repo-admin `gh` login to apply `enforce_admins: true` live (the config + guard + docs are merged; the flip binds only when the protection object is PUT). Remaining NON-pending: `merge-pipeline-04` (PARTIAL — B6 residual deferred under its own grill) and the by-design set. **Shipping assessment: the tractable slice queue is drained (2026-07-14).** Reconciliation (2026-07-13) found 9 incidental closures + `core-scripts-python-02` fixed + 1 partial; a 2026-07-13 run then shipped #1826/#1828/#1830/#1835, and a **2026-07-14 run** shipped the remainder — D1 #1839, B4 #1844, B3 #1845, B8 #1847, D2+E1 #1848, C5 #1849, bash-06 #1851, F1 #1855, C2 #1857, G1 #1858, H1 #1859, H2 #1860, H3 #1861 (full per-PR finding list in § Implementation log). **Remaining OPEN by design/direction, NOT pending:** `merge-pipeline-01`/`-02`/`-06` (branch-protection / admin-merge — left to a maintainer-gated change, do not auto-apply), `merge-pipeline-04` (PARTIAL — B6 residual deferred), and the by-design set (`core-scripts-python-06`, `project-prompts-skills-07`, `self-improvement-loop-06`).
>
...
- 2026-07-13 · **A1 · python-interpreter probe hardening** · PR #1826 (`63a53bb`) · closes `bats-coverage-01`, `bats-coverage-02`, `core-scripts-bash-08` — `setup-harness.sh` `setup_pi` now resolves via `find_python`, and the `markdown_links` / `agent_eval_run` / `agent_eval_score` bats setups exec-validate `$PY` (a `python3` shim that dies on `-c ''` no longer false-greens). **3 findings closed.**
- 2026-07-13 · **C-fast + HP · prose/drift/link batch** · PR #1828 (`a11ce6b`) · 11 one-liner fixes closing `rule-docs-drift-05`, `rule-docs-drift-07`, `HP-03`, `HP-05`, `HP-09`, `core-scripts-python-07` (`_lock-json.py` docstring — verified accurate against `datetime.now(timezone.utc)`), the Outcome five-state agent-contract drift (`issue-janitor.md`, `test-rig.md`), plus `delegation.md` / `AGENTS.md` `.codex`-mirror / `STRUCTURE.md` / `test-portable-purity.sh` / `project.config.json` supported-harness / `mechanic.md` / `test-authoring` SKILL / `p4-annotate.md` dup-trigger corrections. **≈11 findings closed.**
- 2026-07-13 · **merge-pipeline docstring + enforcement** · PR #1830 (`ec76162`) · closes `merge-pipeline-05` (reword the false "Phase-1 poll+observe only, no auto-merge" `merge-watcher.py` module docstring — Phases 2/3/4a are live) and `merge-pipeline-03` (add an opt-in `--blocking` mode to `postmortem-owed.sh`, exit 1 past `POSTMORTEM_BLOCKING_GRACE`; `--list`/`--nudge` stay advisory; NOT wired into SessionStart; +4 bats cases). **2 findings closed.**
- 2026-07-13 · **A2 · shell strict-mode sweep** · PR #1835 (`fb38a22`) · closes `core-scripts-bash-02` (`smatchet-notify.sh`), `-bash-04` (9 prod scripts `set -uo`→`set -euo pipefail`), `-bash-05` (5 hooks `set -u`→strict, hook bodies `|| true`-guarded), `-bash-09` (`test-shell-lint.sh` shellcheck-install hint), `core-scripts-python-09` (`comment_audit.py` sibling-import marker). `-bash-06` **mostly** closed — `test-setup-harness` + `test-skill-load-log` got full strict mode; `test-lint-hook-split.sh` got `pipefail`-only (errexit deferred: it captures `$?` from the exit-2-by-design drain across ~12 sites, needs an `rc=0; cmd || rc=$?` rewrite). Each `-e` added only after a per-line errexit-safety audit; expected-nonzero commands guarded. Follow-up commit addressed 3 CodeRabbit findings incl. a real `|| echo 0`-doubles-output-under-pipefail bug. **≈6 findings closed (bash-06 partial).**
- 2026-07-14 · **D1 · CI workflow triggers + cache + least-privilege** · PR #1839 (`61d7a6d`) · closes `ci-workflows-03` (`merge_group:` on `perf-pr-fast.yml` so the required context reports on merge-queue refs), `-04` (`sanitizer-nightly.yml` FetchContent cache path + key prefix — no more cold-fetch every nightly), `-05` (`perf-full.yml` workflow-level `contents: read`, elevate only in the baseline-bump job). **3 findings closed.**
- 2026-07-14 · **B4 · CodeRabbit md↔py drift-guard** · PR #1844 (`6ec21d7`) · closes `core-agent-prompts-03` + `core-scripts-python-03` — `coderabbit-triage.py selftest` now parses the `.md` rejection table, asserts a compiled regex per entry, and greps a shared rules-version token across both files; the CI check the `.md` had only *claimed* now exists. **2 findings closed.**
- 2026-07-14 · **B3 · merge-watcher NONE-state** · PR #1845 (`d43ccb1`) · closes `core-scripts-python-01` (seed `MERGE_GATES_PRIOR_NONE_HEAD`/`_NONE_STREAK` into `poll_one`'s subprocess env) + `core-scripts-python-04` (`_parse_gate_carry` now carries `none_head`/`none_streak`, persisted across cycles). The once-per-HEAD CR=NONE grace nudge no longer resets every poll. **2 findings closed.**
- 2026-07-14 · **B8 · CR-thread pagination** · PR #1847 (`d4e5e27`) · closes `core-scripts-python-05` — `_fetch_unresolved_cr_threads` now honours `pageInfo.hasNextPage` and paginates instead of silently truncating at 100 threads. **1 finding closed.**
- 2026-07-14 · **D2 + E1 · setup-harness copy + session-lifecycle hook hardening** · PR #1848 (`cb4156c`) · closes `HP-02` (add the missing `copy_template …/lint-catch-all.py` to `setup_claude_code()` so `lint-cpp-common`'s empty-catch scan can run), `hooks-session-lifecycle-03` (`sync-settings-hooks.sh --check` lists deployed hooks absent from template), `-04` (stderr WARN before the `session-tree-banner.sh` `$PPID` fallback), `-06` (stderr WARN when `clear-session-context.sh` leaves `SESSION_ID` unknown). **4 findings closed.**
- 2026-07-14 · **C5 · repo-derivation hardening** · PR #1849 (`d6946c1`) · closes `core-scripts-bash-07` — four scripts (`plan-archival-owed`, `postmortem-owed`, `setup-branch-protection`, `setup-locks-ruleset`) now derive the target repo via `gh repo view --json nameWithOwner` and fail loudly instead of a hardcoded `alexandrosk0/Smatchet` fallback that would target the wrong repo on a fork/rename. **1 finding closed.**
- 2026-07-14 · **bash-06 completion** · PR #1851 (`90b7e4a`) · closes the deferred `core-scripts-bash-06` residual — `test-lint-hook-split.sh` rewritten to `rc=0; cmd || rc=$?` across the exit-2-by-design drain sites so full `set -euo pipefail` is safe. **`core-scripts-bash-06` fully closed.**
- 2026-07-14 · **F1 · new bats suites for uncovered gate scripts** · PR #1855 (`d0075c0`) · closes `bats-coverage-05` (`lock-staleness-sweep.sh`), `-06` (`git-janitor.sh`), `-07` (`is-pure-docs-diff`/`verify-cr-reply`/`merge-gates-prompt`) — one fixture-driven suite + wrapper each. **3 findings closed.**
- 2026-07-14 · **C2 (partial) · backlog-hygiene path drift** · PR #1857 (`a33a1dc`) · closes `self-improvement-loop-05` (flat plan paths → `active/`/`shipped/` tiers) + `-09` (`sort-applied-md.sh` header comment paths). C2's remaining halves are **not** open defects: `-07` migration already ran (reconciliation FIXED — ~310 in `applied.md`); `-04` was overstated (≈1 residual ref, swept here); `-06` (keep `applied.md` excluded from anchor validation) is **by-design**. **2 findings closed.**
- 2026-07-14 · **G1 · agent-size stale-baseline prune** · PR #1858 (`b164e11`) · closes `core-scripts-python-08` — `agent_size_audit.py --prune-stale-baseline [--fix]` cross-references `git ls-files` and the `--selftest` warns on stale entries (+4 bats). G1's siblings need no work: `-07` shipped in #1828; `-06` (md_lint single MD028 rule) is **by-design** (documented intentional starting point). **1 finding closed.**
- 2026-07-14 · **H1 · prose/string debt batch** · PR #1859 (`8863e53`) · closes `project-prompts-skills-04` (drop the agent-only `## Self-improvement` line from `perf-gatekeeper/SKILL.md`), `bats-coverage-09` (`check-required-tools.sh` bats scope → "all agentic suites"), `HP-04` (regenerate the `setup.md` "What the script generates" table from the real copy/link inventory). **3 findings closed.**
- 2026-07-14 · **H2 · harness setup residue** · PR #1860 (`5321042`) · closes `HP-07` (delete the orphan `lint-cpp-pillar2.sh` template + fix its dangling refs) + `HP-06` (wire `pretool-edit-p4-lock-check.sh` into `setup-harness.sh`'s p4 path + `settings.json.tmpl` PreToolUse, self-gated on `SMATCHET_AGENT_VCS=p4`). `HP-09` shipped in #1828. **2 findings closed.**
- 2026-07-14 · **H3 · merge-pipeline + python debt** · PR #1861 (`032cd1b`) · closes `core-scripts-python-10` (extract the registry primitives to an import-legal `merge_watcher_registry.py`; drop the fragile `importlib.util` dynamic-import + all `_CLI.*` indirection) + `bats-coverage-08` (`issue-sweep.sh --apply` gh-stubbed coverage: bot-stray relabel + merged-PR OOB-label strip, never a human Issue). `merge-pipeline-09` (GraphQL pagination cap) was already FIXED incidentally (reconciliation). **2 findings closed.**

- 2026-07-14 · **CAMPAIGN CLOSE-OUT.** The tractable slice queue is **drained** — every P0→P3 slice has shipped, been FIXED incidentally, or is dispositioned below. Cumulative: reconciliation FIXED 9 + campaign PRs #1826/#1828/#1830/#1835/#1839/#1844/#1845/#1847/#1848/#1849/#1851/#1855/#1857/#1858/#1859/#1860/#1861 (plus the in-doc python-02 fix). Explicitly **NOT** closed:
  - **`merge-pipeline-01` (P0), `-02` (P0), `-06` (P2)** — the override-label race, the admin-merge ledger gap, and `enforce_admins: true`. Left OPEN by maintainer direction at close-out; **RESOLVED 2026-08-16** after the maintainer greenlit the full cluster (Slices B1/B2 — see the final Implementation-log entry). `safe-admin-merge.sh` remains the sanctioned admin path (now also a ledger writer).
  - **`merge-pipeline-04` (P1)** — **PARTIAL** (unchanged since reconciliation). `cr-out-of-band` now needs a paired `cr-disposition` attestation, but a CR-never-ran bypass survives *with* an attestation. Slice B6 residual **deferred** (needs a "≥1 CR StatusContext SUCCESS / explicit Review-skipped" precondition; touches production gate logic — author under its own grill).
  - **By-design (no defect):** `core-scripts-python-06` (`md_lint.py` documents its single MD028 rule as an intentional starting point), `project-prompts-skills-07` (the `drain-memory` `ls` glob is required as written), `self-improvement-loop-06` (excluding `applied.md` from anchor validation is intentional).

- 2026-08-16 · **B1 + B2 · the maintainer-gated branch-protection cluster** · PR #2033 · closes `merge-pipeline-01` (P0), `-02` (P0), `-06` (P2) after the maintainer greenlit the full cluster on 2026-08-16.
  - **B1 (`merge-pipeline-01`, stale-override guard):** `merge-gates.graphql` now fetches `completedAt` + `LabeledEvent` times (`timelineItems(last:100, LABELED_EVENT)`); the `10-gate-filter.sh` downgrade arms for the LABEL-REACTIVE overrides (`tests-`/`perf-out-of-band` — their workflows read labels live and re-run on `labeled`) gained a freshness conjunct: a failing run that COMPLETED before the label landed is NOT downgraded, so the gate defers until the labeled-triggered re-run reports. `intent-`/`plan-lock-out-of-band` are documented-exempt (label-blind workflows, no re-run to wait for — a freshness demand would wedge). Projection grew 33→35 fields (staleOverrideNames/Count) with a poll-loop WARN naming the refused downgrade; count guard + canary bumped. 2 new `merge_gates.bats` race-window cases; full suite 208/208.
  - **B2 (`merge-pipeline-02`, ledger writers):** `safe-admin-merge.sh` is now the ADR-0017 "remaining writer" — post-merge it appends the snapshot itself (actor `safe-admin-merge`; `redChecks` from the new shared `downgraded_red_checks` projection, plus the literal `CodeRabbit` when `cr-out-of-band` waived a real block, detected by re-evaluating the CR gate sans-label; the terminal `exec` became a checked call). `git-janitor.sh --post-merge` gained Step 5.5: backfill a missing row for merges NO actor recorded (human/UI merges) with verdict `BACKFILLED` + actor `git-janitor`, age-capped (default 6 h, undatable fails closed — ADR-0017's stale-line-worse-than-hole rule kept intact). +5 `safe_admin_merge.bats` and +4 `git_janitor.bats` cases; ADR-0017 + `ship-loops.md` updated.
  - **B2 (`merge-pipeline-06`, `enforce_admins`):** `project.config.json` `branch_protection.enforce_admins` flipped `false`→`true`; `safe-admin-merge.sh` header documents the post-flip semantics (the `--admin` bypass goes inert server-side; the guard stays the sanctioned admin path because its gate set is stricter than the required set, and label-dismissed required gates self-report green so the override escape hatch survives). The stale `enforce_admins=false` in the Phase-7d merge-queue recipe (`git-ref-plan-locks.md`) was corrected so running it cannot silently revert the flip. **Operator action: run `setup-branch-protection.sh` (repo-admin `gh`) to apply live.**
  - Also swept: the 6 flat `docs/plans/<name>.md` 404 links in `AGENTIC_INFRA_AUDIT.md` repointed to tiered paths (closes the tracked #1654 historical-review finding), with the campaign links aimed at this file's post-archive `shipped/` home. **3 findings closed → the campaign's open count is 0; archived to `docs/plans/shipped/` in the same PR.**
  - **Pre-push adversarial review hardening (same PR):** a 9-finder review pass surfaced and fixed: `evaluate_rollup` still honouring tests-/perf-out-of-band with no timestamps (the admin path retained the exact merge-pipeline-01 race → the guard no longer honours the label-reactive pair; `test-oob-label-impl.sh --parity` re-specified to the sanctioned asymmetry); freshness conjunct gained a `startedAt` fallback + a manual-re-run escape hatch in the WARN (GITHUB_TOKEN-applied labels never trigger workflows); ledger writers now SKIP the append on projection failure (an empty-`redChecks` row would masquerade as clean and suppress the live fallback); the janitor's sourcing of `safe-admin-merge.sh` moved into a subshell (a sourced fail-closed `exit 2` would have killed the cleanup mid-run); post-merge `gh pr view` no longer folds stderr into the JSON; empty `mergedAt` fails closed before GNU `date -d ""` fails open; BSD date fallback; ledger dedup re-anchored to the script's repo root (worktree split) with a pre-network fast path; `cr_waived_red` keyed on label presence, not verdict prose; the config-fallback label union completed (`coverage-out-of-band` was missing) and `plan-lock-out-of-band` added to `merge_gates.override_labels` (source of truth — the watcher now records it too); stale `enforce_admins: false` claims amended in ADR-0013 + `solo-merge-review-gate.md`; `pr-status-watch.sh` divergence documented; `$staleOverride` derived by set subtraction.
  - **Deferred to the memory-drain:** `self-improvement-loop-08` (split the `tooling.md:310` masked env-var sub-item) rides the next `/drain-memory` pass, not a standalone PR.
