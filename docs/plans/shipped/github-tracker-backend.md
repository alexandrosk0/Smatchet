...
- 2026-07-14 · PR10 + PR11 — the last two § Remaining items, shipped together on one branch (single-branch session constraint; PR4/PR5-bundle precedent). PR10: `tests/ui/preferences_tracker_switch.test.cpp` bucket-E round-trip under the `JiraDeterministic` group (registered in `ui_tests_registry.cpp` + `tests/ui/CMakeLists.txt`; no new driver script — the Jira-fixture lane's filter picks the group up). PR11: `403`+`Retry-After` → `RateLimited` in `ClassifyTrackerResponse`, capped Retry-After honoring in `TrackerHttpRequestWithRetry` (`kTrackerHttpMaxRetryAfterHonorMs` = 30 s), pure `ParseRetryAfterSeconds` / `ComputeTrackerRetryDelayMs` helpers in `TrackerHttpPure`, doctests in `TrackerHttpRetry.test.cpp`.
...
7. ✅ **PR10 — bucket-E coverage for Preferences tracker switch** — shipped 2026-07-14 (`tests/ui/preferences_tracker_switch.test.cpp`). One round-trip scenario under the `JiraDeterministic` group (so the existing `test-ui-jira-deterministic-backend.sh` driver + the CI Jira-fixture bucket-E lane run it with the fixture boot they already provide): Jira fixture rows → switch to GitHub → asserts the live backend is a NEW instance of the requested kind and no stale row lingers → switch back → fixture rows return. App-state-coupled per the sibling deterministic-backend convention — it drives `SyncWithBackend` with a `TrackerType`-changed cfg override (the exact `onPreferencesSaveAndSync` call) rather than the combo/footer-button ImGui items (the icon-leading Save & Sync id is a documented non-stable probe).
8. ✅ **PR11 — rate-limit + retry policy** — shipped 2026-07-14. The bulk was already covered by the shared `TrackerHttpRequestWithRetry` (Transport / 429 / 5xx with exponential backoff); the remaining `Retry-After` delta is now closed: `ClassifyTrackerResponse` classifies a `403` carrying a `Retry-After` header as `RateLimited` (retryable + offline-queueable) instead of auth-shaped — GitHub's secondary-rate-limit / abuse-detection shape — and the retry wrapper raises its backoff to the server-requested delay, capped at `kTrackerHttpMaxRetryAfterHonorMs` (30 s) so a hostile value can't pin a worker (the backoff sleep polls the cancel token throughout). Pure helpers `ParseRetryAfterSeconds` / `ComputeTrackerRetryDelayMs` in `TrackerHttpPure`; delta-seconds form only (HTTP-date → plain backoff).

All eight items are now shipped. The two items previously deferred here (`ListProjects` returning empty since PR2; Projects v2 custom fields per § Out of scope) shipped via docs/plans/shipped/github-projects-v2-fields.md.
