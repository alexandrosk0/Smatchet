...
- **Batch 4 — `IAppAutomation` + `IAppTicketData` + `Ai` include-drop** (PR [#1670](https://github.com/alexandrosk0/Smatchet/pull/1670), squash `9d8557c0`). Two new rank-0 leaves plus one facet-less include removal, migrating **three** command TUs off `AppController.h` — a clean **−3**.
  - `IAppAutomation` (`RunAutoScript`, `RunFlatScriptAsync`, `RunLuaSetupScript`, `GetLuaGlobalActionNames`) ← `BuiltinCommands_Automation.cpp`. All std-typed, no poster, no free-function coupling. **Default-arg-must-STAY finding (opposite of batch 2/3):** `RunAutoScript`'s `processAll = false` default is mirrored on **both** the facet base **and** the `AppController` override, because `LuaConsolePlugin.cpp` calls it through a **concrete** `AppController&` with the 2-arg form — static binding resolves the default from `AppController`'s declaration, so it can't be dropped there; `automation.*` calls through the facet, which needs it too. (In batch 2/3 every concrete caller passed explicit args, so the concrete default *could* be dropped; here one relies on it. Rule: drop the concrete default only when no concrete-typed caller relies on it.)
  - `IAppTicketData` (`GetActiveTicketsSnapshot`) ← `BuiltinCommands_Tickets.cpp`. Read-only active-ticket data; the element `CachedTicket` is rank-0 (`CachedTicketTypes.h`, included directly — no fwd-decl needed). **Pillar-1 note:** `GetActiveTicketsSnapshot` is called once per frame from the grid, but through a concrete `AppController&` and is an out-of-line accessor (lock + `shared_ptr` copy, never inlined) → ≤1 vtable lookup/frame, not the inlined-hot-getter case the exclusion protects. The adjacent inline `GetActiveTicketsRevision` (a Pillar-1 getter) stays off any facet.
