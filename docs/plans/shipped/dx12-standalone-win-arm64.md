...
- **Perf baselines bootstrapped in CI but the seed files are NOT committed this session** — #1424 runs `perf-baseline-bootstrap.py --host ci-windows-arm64` and uploads the six `docs/perf/baselines/*.ci-windows-arm64.json` files as a run artifact, but committing them from here is blocked: the sandbox egress allowlist rejects the signed artifact download host (`*.blob.core.windows.net`, HTTP 403 "Host not in allowlist"). The baselines exist and are correct (the job is green); they just need to be fetched from the artifact and committed in a follow-up once the host is allowlisted or the user downloads the artifact. Tracked in `docs/self-improvement/categories/tooling.md`. This is a tooling/egress limitation, not a code gap — the ARM64 perf path itself is proven by the green bootstrap run.
...
| ARM64 perf-baseline bootstrap | `perf-baseline-bootstrap.py --host ci-windows-arm64`, uploaded as `perf-arm64-baselines-<run_id>` artifact (#1424) | **PASS (run)** — six baselines seeded green in CI; *commit of the seed files deferred*, see Deviations (egress block on the artifact host). |
...
**NOT-RUN / deferred (one residual — tooling, not code):**
- **Commit of the bootstrapped `ci-windows-arm64` perf baselines** — the six `docs/perf/baselines/*.ci-windows-arm64.json` files exist as the green #1424 run artifact but could not be fetched + committed from this sandbox: egress allowlist rejects `*.blob.core.windows.net` (HTTP 403). Follow-up: allowlist the artifact host or download the `perf-arm64-baselines-<run_id>` artifact and commit the six files. The ARM64 perf path itself is proven by the green bootstrap run; only the seed-file commit remains. Tracked in `docs/self-improvement/categories/tooling.md`.
