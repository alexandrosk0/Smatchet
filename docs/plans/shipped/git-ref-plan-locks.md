...
| 4 | **Enable GitHub merge queue on develop** (Phase 7d). | **CLOSED (unavailable) 2026-07-14** — the merge queue is an org-only feature (Team/Enterprise), absent from both classic branch protection and rulesets on this user-owned repo; the underlying goal was met another way (`strict=false` on required checks — PRs merge on their own green head, post-merge CI backstop). See `docs/plans/active/build-quality-velocity-hardening.md` item #14 + its § Deviations. Re-open only if the repo moves under an org. | — | § Phase 7d |
