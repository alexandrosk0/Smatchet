...
The drift is **structural, not incidental**: [`project.config.schema.json:89`](../../../project.config.schema.json#L89) declares `branch_protection` with `additionalProperties: false` and only five properties, so the config *cannot express* the key — every future run of the script re-drops it. Live evidence that this script's body has never been applied: the live `required_contexts` array is in strict chronological-append order, not the config's order. That is an ordering argument and proves only that no PUT carrying *this* config's context order has landed — it is not an audit trail, so it does not exclude some other full-object PUT (a manual one, or an earlier config order) having run at some point.
...
- `33c92f8b` · `fix(agentic): make branch_protection config a complete PUT-body statement` — config + schema + body builder + bats suite (**10 cases at this point**; the post-review round below grows it to 12) + wrapper, and the plan revision folded into the same commit.
- `18a6f9ae` · `docs(plan): archive branch-protection-config-completeness as shipped` — status flip + `active/` → `shipped/` move.
- `77ff55b8` · `docs(plan): record the repo-wide shell-lint sweep as passed` — § Verification, after the backgrounded sweep returned exit 0.
- `3836b65a` · `fix(agentic): correct app_id semantics + require every branch_protection key` — the CodeRabbit round: schema `anyOf`, subscript reads + `exit 2` on a missing key, **bats 10 → 12 cases**.
- `7c15e43a` · `fix(docs): anchor the app_id correction claim in the plan`, `3c9d68f4` · `docs(agentic): state that the app_id config key is never absent` — doc follow-ups pinning the corrected three-value semantics.
