...
- **Merge-snapshot ledger (C2-part-1) remains deferred** to its own plan `docs/plans/active/merge-snapshot-ledger.md` (authored + grilled in #870, **unstarted**); Slice 3 shipped only the mechanical mergedAt-ordering fix, with the live `statusCheckRollup` query as the documented interim fallback.
