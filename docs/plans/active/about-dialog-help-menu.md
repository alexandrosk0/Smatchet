...
- **Slice 4 — fixed first-use window size instead of `ImGuiWindowFlags_AlwaysAutoResize`.** The plan called for auto-resize, but the modal renders filesystem paths and a 40-char SHA: `TextWrapped` inside an auto-resizing window is a width feedback loop, and an unwrapped long Windows user-data path stretches the modal off-screen. Uses `SetNextWindowSize(560×520, ImGuiCond_FirstUseEver)` + `ImGuiWindowFlags_NoSavedSettings`, with wrapping only on the path/SHA rows.
