...
- **RISK — fake-vs-real semantic drift (central):** a hand-written `FakeSyncCache` could diverge from `LocalCacheManager`'s real semantics, so the "pure" tests would validate the fake, not production. **Mitigation:** the shared contract suite (#3) runs identical assertions against both impls; a service test only trusts the fake for behavior the contract suite pins.
