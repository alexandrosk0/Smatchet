...
- **Sprint-2 status (as of 2026-06-06):**
  ...
  - **Remaining 🟢:** none — #12 batch-2, #20/#28, and **#10 TSan (MVP shipped — see below)** all landed by 2026-06-08. The escalated 🔴 tier is now **#19 in progress (19a + 19b shipped; 19c assessed + deferred)** + #21 (`Optional<T>`).
  - **Escalated 🔴 (await user):** #19 (pImpl `AppController`) — **19a + 19b SHIPPED**, 19c assessed + deferred (see #19 entry below); #21 (`Optional<T>`/`Result`).
- **#19 — pImpl `AppController` (sol2 header lift): step 19a SHIPPED (2026-06-08, PR #1016, branch `feat/appcontroller-pimpl`).** 19a is the pImpl scaffold only — forward-declared nested `struct Impl`, `std::unique_ptr<Impl> impl_` declared LAST, empty `Impl{}` + out-of-line ctor/dtor in `AppController.cpp` (incomplete-type discipline compile-verified). **Zero behaviour change.** Verified: dual-target build (`SmatchetStandalone` + `SmatchetCore_DX12`, `ninja-iter-msvc`) green; `SmatchetTests` 1460/14678 + `SmatchetLuaTests` 30/167 green; `test-lint-rules.sh --diff origin/develop` PASS; clang-format clean. Perf-neutral by construction (`impl_` touched only in ctor/dtor; no hot accessor de-inlined).
  ...
  - **19c ASSESSED + DEFERRED — completion plan + blast-radius evidence.** 19c is the actual sol2 lift: extract `ILuaBindingHost` onto an Impl-owned binding-host object (Impl gains an `AppController& app_` back-ref + implements the 10 interface methods), **relocate the ~18 sol-typed methods off the public class onto `Impl`** (`InitLuaCore`/`InitLuaUi`/`PrepareFreshLuaState`/`ReplayActiveSetupScripts`, the `Lua*Bind` overrides + the UI-side `LuaRegister*`/`LuaUiRegisterWindowBind`/`LuaUiInvalidateFieldCacheBind`/`ParseMcpToolDef`/`ResolveLuaFieldProvider`, `RunAutomation{Job,AutoScript,FlatScript,ActionCall}`) + the nested `McpToolDefinition` struct (holds `sol::protected_function`), rewire the sol registration in `InitLuaUi` to bind the Impl object's methods, re-point `state["__smatchet_app"]`/`__smatchet_app_ui` from `AppController*` to the binding host, then drop `<sol/sol.hpp>` + `ILuaBindingHost.h` from `AppController.h` and sweep the ~109 includers for transitive-sol2 reliance. **Blast-radius evidence (measured 2026-06-08):** the ONLY external consumer of the relocated surface is `Source/Plugins/Mcp/McpPlugin.cpp` (`GetLuaMcpTools()` → `McpToolDefinition`); the other ~18 methods are internal to the `AppController_Lua*.cpp` TUs. **Why deferred:** the core is a per-body **receiver rewrite** of those ~18 method bodies (hundreds of lines across 3 TUs) where every identifier must be disambiguated as an `app_.`-member vs an Impl-member — not mechanizable by sed, and its correctness lives in Lua-runtime dispatch + shutdown destruction-ordering that only the 30-case `SmatchetLuaTests` suite canaries. Per the escalate-when-unvalidatable invariant + "never ship a broken build / never skip Lua validation," 19c is split out as a focused follow-up slice rather than bolted onto the end of the (already large) 19b pass. 19b de-risks it materially: the members already live in `Impl` with `impl_->` access, so the 19c relocation reduces to moving method bodies + flipping `impl_->X`→`X` / app-member→`app_.X`.
