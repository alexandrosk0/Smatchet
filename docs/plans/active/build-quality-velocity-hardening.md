...
- **Sprint-2 status (as of 2026-06-06):**
  ...
  - **Remaining 🟢:** none — #12 batch-2, #20/#28, and **#10 TSan (MVP shipped — see below)** all landed by 2026-06-08. The escalated 🔴 tier is now **#19 in progress (19a shipped, 19b/19c deferred)** + #21 (`Optional<T>`).
  - **Escalated 🔴 (await user):** #19 (pImpl `AppController`) — **19a SHIPPED**, 19b/19c deferred (see #19 entry below); #21 (`Optional<T>`/`Result`).
- **#19 — pImpl `AppController` (sol2 header lift): step 19a SHIPPED (2026-06-08, PR #1016, branch `feat/appcontroller-pimpl`).** 19a is the pImpl scaffold only — forward-declared nested `struct Impl`, `std::unique_ptr<Impl> impl_` declared LAST, empty `Impl{}` + out-of-line ctor/dtor in `AppController.cpp` (incomplete-type discipline compile-verified). **Zero behaviour change.** Verified: dual-target build (`SmatchetStandalone` + `SmatchetCore_DX12`, `ninja-iter-msvc`) green; `SmatchetTests` 1460/14678 + `SmatchetLuaTests` 30/167 green; `test-lint-rules.sh --diff origin/develop` PASS; clang-format clean. Perf-neutral by construction (`impl_` touched only in ctor/dtor; no hot accessor de-inlined).
  ...
  - **19b/19c DEFERRED — completion plan.** **19b** (migrate the cold member block into `Impl`; rewrite ~250 member-access sites across the 9 `AppController*.cpp` TUs to `impl_->…`). Destruction-order mitigation: end `~AppController` body with an explicit `impl_.reset();` (after `JoinBackgroundTasks()`) so the moved members tear down at the same logical point as today, while every kept member is still alive — and preserve `sol::state lua` declared before the `protected_function` maps inside `Impl`. **19c** (extract `ILuaBindingHost` into an Impl-owned binding host implementing the 10 interface methods; move the ~20 sol-typed methods + the `McpToolDefinition` struct + the `RecordLuaWindow`/`ResolveLuaFieldProvider`/automation-job/`ApplyOrQueueLuaWindowOp` surface out of the public header into `Impl`; re-point `state["__smatchet_app_ui"]` from `AppController*` to the binding host; drop `<sol/sol.hpp>` + `ILuaBindingHost.h` + `AppController_LuaTypes.h` from `AppController.h`; add a direct `#include <sol/sol.hpp>` to `AppController_LuaBindings_detail.h`; then sweep the ~109 includers for any symbol they relied on transitively through sol2). **Why deferred:** 19b+19c together are a 1500–2500-line, high-blast-radius change whose correctness depends on Lua-runtime dispatch + shutdown destruction-ordering paths only partially covered by the automated suite — split out rather than big-banged in one autonomous pass, per the escalate-when-unvalidatable invariant + "do not ship a broken build."
