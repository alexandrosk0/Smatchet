...
S1c-3 (the membership/disappearance reconcile half of item 10 — `FetchIssueKeysForView` → `RemovedKeys` → `ProbeIssueExists` → 404 cache DELETE / 200 LeftView removal) follows, then the deferred concrete Jira `expand=changelog` HTTP override.
