...
>
> **Status**: `active` — kept under `active/` because the maintainer-gated branch-protection cluster is still open (below); flip to `shipped` + `git mv` to `docs/plans/shipped/` only once the maintainer decides it. **Shipping assessment: the tractable slice queue is drained (2026-07-14).** Reconciliation (2026-07-13) found 9 incidental closures + `core-scripts-python-02` fixed + 1 partial; a 2026-07-13 run then shipped #1826/#1828/#1830/#1835, and a **2026-07-14 run** shipped the remainder — D1 #1839, B4 #1844, B3 #1845, B8 #1847, D2+E1 #1848, C5 #1849, bash-06 #1851, F1 #1855, C2 #1857, G1 #1858, H1 #1859, H2 #1860, H3 #1861 (full per-PR finding list in § Implementation log). **Remaining OPEN by design/direction, NOT pending:** `merge-pipeline-01`/`-02`/`-06` (branch-protection / admin-merge — left to a maintainer-gated change, do not auto-apply), `merge-pipeline-04` (PARTIAL — B6 residual deferred), and the by-design set (`core-scripts-python-06`, `project-prompts-skills-07`, `self-improvement-loop-06`).
>
...
- 2026-07-14 · **CAMPAIGN CLOSE-OUT.** The tractable slice queue is **drained** — every P0→P3 slice has shipped, been FIXED incidentally, or is dispositioned below. Cumulative: reconciliation FIXED 9 + campaign PRs #1826/#1828/#1830/#1835/#1839/#1844/#1845/#1847/#1848/#1849/#1851/#1855/#1857/#1858/#1859/#1860/#1861 (plus the in-doc python-02 fix). Explicitly **NOT** closed:
  - **`merge-pipeline-01` (P0), `-02` (P0), `-06` (P2)** — the override-label race, the admin-merge ledger gap, and `enforce_admins: true`. **Left OPEN by maintainer direction** — these mutate branch-protection / admin-merge policy and must not be auto-applied. Carried as a live backlog item for a human-gated change (Slice B1/B2, `safe-admin-merge.sh` remains the sanctioned bypass).
