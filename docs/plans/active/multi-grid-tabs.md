...
9. [`Source/Core/include/AppController.h`](../../Source/Core/include/AppController.h:989) — own `map<paneId, GridLiveContext>` (start size 1, behaviour-identical); retire-to-graveyard ([`:960`](../../Source/Core/include/AppController.h:960)) on context teardown. Sync completion tagged with `paneId`, routed via `MainThreadDispatcher` to the right context.
...
17. `AppController` + `GridLiveContext` — visibility-driven lifecycle: visible → ensure context + start sync; hidden → pause sync, retain snapshot, retire after grace; per-context cancel/`Superseded`; N concurrent sync workers, all off-UI-thread.
...
2. *`git mv docs/plans/active/multi-grid-tabs.md docs/plans/shipped/multi-grid-tabs.md`,*
