...
- **Slice 4 — Slices 4+5 remain; plan stays `active`.** Slice 4 (item 20) is shipped bar the deferred catalog VALUE-read above; Slice 5 is partially landed — the two perf scenarios shipped in Slice 5b (`feat/mgt-s5-perf-scenarios`); durable cross-restart pane-snapshot cache + hidden-pane LRU cap + the mandatory ThreadSanitizer pass remain — the plan does NOT archive until those land.
