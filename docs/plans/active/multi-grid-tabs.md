...
- **Slice 3 — `side-by-side-2-grid` + `concurrent-sync` perf IScenarios NOT authored in this slice** (named deferral, time-boxed implementation window): the dispatcher-budget seam is covered by the shared-deadline + rotation logic in `TickAllContexts` and the swap/lifecycle bucket-A net; the two scenarios + `perf-pr-fast-set.json` registration move to the Slice 5 perf pass (where the plan already gates them). Until then the existing `priority-grid-scroll` PR-fast scenario covers per-pane render.
...
- **Slice 4 — Slices 4+5 remain; plan stays `active`.** Slice 4 (item 20) is shipped bar the deferred catalog VALUE-read above; Slice 5 (durable cross-restart pane-snapshot cache + hidden-pane LRU cap + the two perf scenarios + mandatory ThreadSanitizer pass) is not started — the plan does NOT archive until Slice 5 lands.
