...
- **Slice 2b — `IAiAssistantUiState` seam (DONE — thin field-accessors).** The 8 `g_ui` chat fields are now behind `IAiAssistantUiState` (`AssistantStreamBuf()`/`AssistantHistory()`/… return the same mutable refs, so the controller's cap/flush/trim logic is byte-for-byte preserved — pure indirection, no behaviour change). Production binds the **process-lifetime** `GetGlobalAiAssistantUiState()` adapter (`AiAssistantUiStateAdapter.cpp`, AI-gated) — critical for the post-destruction safety of drained tasks. The controller's `.cpp` no longer references `g_ui` (only the adapter does). **Remaining for 2c:** the controller `.cpp` still `#include`s `AppController.h` (transitively ImGui), so it is logically decoupled but not yet link-free — 2c removes that include + resolves the AiClientFactory/cpr link closure, then adds the TSan test.
...
- *(slice 2b)* `IAiAssistantUiState` thin-accessor seam — new `IAiAssistantUiState.h` (8 fields, ref-returning) + `AiAssistantUiStateAdapter.{h,cpp}` (process-lifetime `GetGlobalAiAssistantUiState()` forwarding to `g_ui`, AI-gated). The controller takes `(MainThreadDispatcher&, IAiAssistantUiState&)`; the 3 posted-lambda blocks capture `ui` and call `ui->Assistant*()` instead of `g_ui.assistant*` (logic preserved verbatim). Controller `.cpp` is now `g_ui`-free. Interface header syntax-checked (C++14, `-Wall -Wextra`); full build is CI's job (heavy includes + no local FetchContent).
