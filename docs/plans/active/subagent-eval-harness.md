...
**MVP = Phase 1 only** (3 highest-leverage diagnostic agents, offline, manual / PR-scoped trigger). Phases 2-3 are deferred — see § Out of scope.
...
11. `docs/self-improvement/AGENT_SELF_IMPROVEMENT.md` — wire the eval gate into the loop as the "optimize against evals" (EDD Phase 2) step.
...
- **Phase 2 — trace flywheel**: auto-harvest real session archives / transcripts into new golden cases (the video's "production traces become eval datasets"). The trace source already exists (`.session-context.archive/`, transcripts) — follow-up plan once Phase 1 is calibrated.
