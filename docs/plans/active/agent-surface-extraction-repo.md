...
- *(this PR)* · **Publication audit** (the Publication gate in § Context point 1; seed phase 4b's precondition) — 70 of 75 manifest rows `CLEAR`, 5 `PENDING` on one owner decision (**D1**, below). Nothing in Phase B has touched the layer repo; rows 9 and 11–16 still untouched.
...
| 21 | Per-path publication audit | `seed-audit-sweep.py` (full history) + gitleaks 8.30.1 + provenance grep + 8-word overlap scan, hand-triaged | **PARTIAL** — 70 of 75 rows `CLEAR`; 5 `PENDING`, all on owner decision **D1** (Whip-Process licensing). Phase 4b still refuses the push, correctly |
