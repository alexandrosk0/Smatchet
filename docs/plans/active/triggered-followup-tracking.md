...
Non-obvious decisions (recorded for the grill): the `fired=` write-back is **orchestrator-stamped, not script-stamped** (keeps the script read-only); `pr-count` uses **`gh pr list --base develop --state merged --search 'merged:>=<date>' --jq length` as the authoritative primary, with an offline `git log origin/<base> --since=<date> --format='%s' | grep -cE '\(#[0-9]+\)'` fallback** (counts squash-landed PR subjects). Verified against the tree: `origin/develop` **does** carry merge commits (local develop-syncs + older PR merges), so the fallback must **not** use `git log --merges`; the `(#N)`-paren grep naturally excludes them (a `Merge pull request #N` subject lacks the `(#N)` parens) and ignores direct-pushed non-PR commits. gh-first because SessionStart network is not guaranteed; ambiguous/offline → SKIP-and-retry, never a false fire.
