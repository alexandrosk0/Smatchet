...
- `AGENTS.md` **257 → 218 L**. § Project rules (61→ a tight card): kept the **every-edit invariants** (C++14 ban, never-`printf`, RAII, the Don'ts) + an **enforcement contract-card** (the gated rule-ids + zone globs + caps as a table) + an **on-demand rule-doc router table**. Debug-techniques + Harness-adapter sections collapsed to stubs + pointers.
- **Approach: Option B (contract-card), forced by a discovered selftest coupling** — see § Deviations #11.

**Slice 5 status: landed at 218, NOT ~120 — the always-set is inherently larger (§ Deviations #12).** The on-demand *detail* is fully routed out (the real context-tax win); reaching ~120 needs trimming the always-*summaries* (Delegation/ship-loop/Merge-gates/Pillars) into tighter router stubs — a judgment call deferred to the human pilot-review gate (their detail already lives in linked docs, but inline gist has every-session value).
...
12. **AGENTS.md landed at 218, not ~120** (same optimistic-target pattern as Slice 2's ~300). The plan's ~120 assumed the always-set was ~120, but the faithfully-kept always-sections (Operating principles, Quality Pillars, ship-loop, Merge-gate summary, the Project-rules card, Process rules, Delegation index, Semantic-search, Agent-locations, Self-improvement, Dual-VCS, caveman, vexp) sum to ~200 on their own. Slice 5 routed out **all the on-demand detail** (build / cpp-mechanics / debug / harness-table → 4 trigger-docs) — the real every-session context-tax win — landing 218. Reaching ~120 requires trimming the always-*summaries* (which are already summaries-with-pointers) into terser router stubs; their detail already lives in linked docs, but doing so trades inline every-session gist for line-count. **Deferred to the human pilot-review gate as an explicit decision** (do a second always-summary-trim pass, or accept on-demand-routed-out as the win). Anchor-safety held: 97 `AGENTS.md §` refs all resolve (one shipped-plan ref `Tiered enforcement zones` needed `cpp-rules.md`'s heading to be exactly `## Tiered enforcement` so the substring-resolver matches). The 4 new docs are portable dirs → portable-purity baseline refreshed (git-add-before-refresh, per #9).
...
- **Line count: `AGENTS.md` 257 → 218** (−15%). 4 new trigger-docs: `build.md`, `cpp-rules.md`, `debug-techniques.md` (docs/agent-rules), `capability-adapter.md` (docs/harness).
