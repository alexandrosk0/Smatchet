...
- `<pending-merge-sha>` · feat(memory): S1–S5 in one PR — 6 detach→pool migrations + shared-backend ownership (ADR 0012); `no-detach` absolute lint + bats; `perf.memory` pull snapshot (`MemoryTelemetry` + dispatcher `QueueLen` + icon-cache gauges); bounded attachment header read; off-thread thumbnail decode + dispatcher upload + cue + `pendingThumbnailUploads` gauge.
...
- **Site #2 consumer fix — `RefreshFieldCatalog` latches a local `shared_ptr` (self-review catch).** Migrating site #2 to the pool did NOT fix its live-swap UAF: the worker calls `app.RefreshFieldCatalog`, which deref'd the `Backend` member across a blocking `FetchFieldCatalog` HTTP call with no local strong handle — a tracker swap mid-fetch frees the backend → UAF. Fixed by latching `std::shared_ptr<ITrackerBackend> backend = Backend;` at the top (ADR 0012 § Scope note). The sibling `Backend->`-across-blocking-call sites (`FetchFieldCatalog` overloads, `FetchIssueEditMeta`, `Mutations()` payload) share the pattern from other async entry points — tracked follow-up, not this PR's migration set.
