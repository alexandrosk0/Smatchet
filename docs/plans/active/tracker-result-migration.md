...
**Slices 8–9 (FOLLOW-ON phase, beyond the virtual interface — flag if user wants it).** Flip the `AppController` public wrappers from `bool + outError` to `Result`/`TrackerError`, rippling to UI/Command callers (`AnnotateAnalysisUi_Modals.cpp`, `AnnotateAnalysisUi_Window.cpp`, `BuiltinCommands_Users.cpp`, `BuiltinCommands_TicketMutations.cpp`, `TrackerGridFieldDisplay.cpp`, `SmatchetAutocompleteUi.cpp`, `SmatchetGridUiSupport.cpp`, `SmatchetUI.cpp:243`); plus non-virtual helpers `IssueDraftHelpers::FromJson`, `IssueCreatePipeline::Run` / step fns, `FieldCatalogCache` free fns, `TrackerFieldPayload.cpp` facade. These are the "Plus non-virtual" items; each is its own small PR. **Defer unless user confirms — not required for the FULL *virtual-interface* migration (Slices 1–7).**
