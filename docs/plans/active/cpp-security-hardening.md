...
1. **Unbounded JSON ingest (25 findings)** — bare `nlohmann::json::parse(...)` on network / IPC / local-file input that bypasses the project's own `smatchet::json_safe::ParseBounded`. A depth-bomb payload crashes the process via the recursive `~json` DOM destructor (an uncatchable stack overflow — *not* a `try/catch`-able exception) or exhausts the heap. Plus a few **hand-rolled recursive walkers** over server-supplied JSON that have the same teardown-recursion problem but are not `parse` sites.
