...
- **Stream B — global omnibox (larger, multi-subsystem).** Three slices: **(2a)** refactor the existing embedded JQL editor so its buffer + autocomplete state are a reusable `JqlEditorState` instance (the omnibox needs its *own* instance to avoid sharing request-ids with the dashboard editor — recon flagged a real in-flight-future collision); **(2b)** reserve a top toolbar region in the ImGui host (`BeginViewportSideBar(ImGuiDir_Up)` **before** `DockSpaceOverViewport`) and fill it with the JQL editor wired to the focused pane via the existing `SyncWithCurrentView` path; **(2c)** add ticket-key/title detection that, on Enter, routes to `ITrackerIssueReader::FetchIssuesForKeys` (async, worker-thread) → select-in-grid or open-browse.
...
*Slice 2b — host bar region + omnibox content*
12. `Source/Core/src/Ui/SmatchetImGuiHost.cpp:753` — **before** `DockSpaceOverViewport`, call `ImGui::BeginViewportSideBar("##SmatchetOmnibar", viewport, ImGuiDir_Up, barHeight, flags)` and invoke a UI-supplied content callback (or set the reserved region for SmatchetUI to fill). This is the architectural seam — space reservation must precede the dockspace or the bar overlaps panes. Route to `ui-host` agent.
...
- **Host-layer seam (2b) is the highest-risk change** — reserving the bar region must happen **before** `DockSpaceOverViewport` (`SmatchetImGuiHost.cpp:753`) or the bar overlaps the panes. *Mitigation*: route to `ui-host`; the feature genuinely spans host + UI + tracker + grid (3+ subsystems) → consider an `architect` framing pass before 2b. Fallback if `BeginViewportSideBar` ordering proves fragile: render the bar as the first child inside the dockspace passthrough host window (documented as the lower-fidelity alternative).
