...
Reuse the contention signal that already exists rather than inventing one. The concurrent-session guard `docs/harness/claude-code/hooks/guard-shared-tree.sh` already computes live-sibling count via `sr_count_live_siblings` (`agents/scripts/core/session-registry-lib.sh`). Layer A's new hook gates on the **same** signal: when `sr_count_live_siblings == 0` it exits 0 (allow — no tax on solo work, identical bias-to-allow to the #1388/#1379 false-positive hardening); when `> 0` it requires that the edit's target path be covered by a `refs/locks/*` claim whose `owner` matches `AGENT_ID`, else it emits a `permissionDecision: "deny"` with a copy-paste `lock-claim.sh` line. The lock-table read is `git ls-remote`-backed and cached in a temp file with a ~30 s TTL so the per-edit network cost is amortised; any network/parse failure **fails open** (advisory, never wedge a session).

The three layers are deliberately asymmetric in their fail-mode. A and B are **advisory / fail-open** local hooks (catch the mistake early, cheaply, but never hard-wedge a flaky-network session) — A at edit time inside Claude Code, B at `git push` time for any harness (the catch-all for non-Claude-Code agents, wired via `core.hooksPath`). C is the **fail-closed hard net**: a required GitHub Actions check that recomputes contention server-side (overlap between this PR's changed files and any other open-PR / live-lock write set owned by a different branch) and blocks merge — unbypassable except the `plan-lock-out-of-band` label that downgrades block→WARN, symmetric with the existing `cr-out-of-band` / `tests-out-of-band` / `perf-out-of-band` overrides in `merge-gates.sh`. This is the same advisory-pair-plus-hard-net shape the tree-guards already use (`guard-shared-tree.sh` advisory + `guard-head-drift.sh` hard net).
...
1. `docs/harness/claude-code/hooks/guard-plan-lock.sh` (new) — PreToolUse(Edit|Write|MultiEdit|NotebookEdit) hook. Parses `.tool_input.file_path` from stdin (mirror the `json_field` helper in `docs/harness/claude-code/hooks/guard-shared-tree.sh:77-86`); short-circuits `exit 0` when `sr_count_live_siblings == 0` (force-on-contention) or `SMATCHET_REQUIRE_PLAN_LOCK` bypass exported pre-launch; else checks the target path against the AGENT_ID-owned lock table and emits the deny JSON (mirror `:149-152`). Fails open on missing lib / network / parse error.
...
3. `agents/scripts/core/lock-table-cache.sh` (new) — `git ls-remote refs/locks/*`-backed lock table with ~30 s TTL temp-file cache + `AGENT_ID`/path-coverage query helper, shared by Layers A and B. Reuses `_lock-json.py` to parse `owner` / `branch` / write-set out of each claim. Fail-open contract documented at top.
...
5. `.github/workflows/plan-lock-gate.yml` (new) — required check modeled on `.github/workflows/cr-finding-gate.yml`. Computes this PR's changed files; enumerates `refs/locks/*` claims + other OPEN-PR changed-file sets; blocks (non-zero) when an overlap is owned by a different `branch`. Honours stale locks as non-blocking (older than `lock-staleness.yml`'s `THRESHOLD_DAYS=14`). Override: `plan-lock-out-of-band` label → block downgraded to WARN.
...
- `sr_count_live_siblings` — `agents/scripts/core/session-registry-lib.sh` — the force-on-contention signal; reused verbatim so Layer A's allow/deny boundary is identical to the proven `guard-shared-tree.sh` boundary (no new false-positive surface).
...
- **False-block on solo work.** Mitigation: force-on-contention — `sr_count_live_siblings == 0 → exit 0`. Zero tax on the common case; reuses the proven #1388/#1379-hardened signal, so no new false-positive surface.
