...
**Identity-derivation asymmetry (deliberate — do not "unify").** Layer A keys on **`owner`** (`claim.json.owner == AGENT_ID` — "do *I* hold a lock covering this path?") because it runs locally with `AGENT_ID` in scope. Layers B and C key on **`branch`** (`claim.json.branch` ≠ this push's / PR's branch — "is this another branch's lock?") because B's pre-push context and C's Actions context have no reliable `AGENT_ID`. The two formulations are equivalent in the normal one-session-one-branch case; the difference is purely which identity field is derivable where. An implementer who "simplifies" Layer C to match on `owner` breaks it (no `AGENT_ID` server-side → every lock reads as un-owned → either all-block or all-pass).
