...
**Deferred (not yet shipped):** 1b-ii locks cluster (`lock-*`/`locks-*`/`setup-locks-ruleset`/`test-lock-primitives*` + the locks-render GH action) and comment/lint-rules cluster (`comment_*.py`/`assert-code-unchanged.sh`/`test-comment-tooling.sh`→core, `test-lint-rules*`→project, build-job gate at `build-and-test.yml:173`); **Phase 2** daemon/hook clusters (`merge-*`, `merge-watcher*`, `agent-progress`, `setup-harness`+`.ps1`, `clear-session-context`, `vexp-strip`, `memory-drain-nudge`, `check-main-repo-clean`, `coderabbit-triage`, `git-janitor`+`is-pure-docs-diff`, `tail-agent`, `p4-task-stream*`, `smatchet-notify*`, `agent-tokens-report.py`) + live `.claude/settings.json` edit + daemon restart — **needs user authz** (self-mod guard); `is-pure-docs-diff.sh` §6 regex (moves with its Phase-2 cluster).
...
## Verification (actual)
- **Test-count invariant (silent-skip guard):** baseline `find scripts/dev -name 'test-*.sh'` = **57**; after every move multi-root discovery = **57** — held across 1a / 1b-i / cluster A.
...
- **Not yet verified (needs build/live env):** probe-PR CI behaviour (agentic-only → `code=false`/skipped; build script → run); daemon smoke (Phase 2).
