...
**Cache backend (locked):** GHA-native — `mozilla-actions/sccache-action` + `SCCACHE_GHA_ENABLED=true` (the action sets `ACTIONS_CACHE_URL`/`TOKEN`; sccache talks to the GH cache API directly, handles sharding + LRU eviction, no manual key). The existing `CMakeLists.txt:55-60` launcher already invokes `sccache <compiler> <args>` as a wrapper — sccache ignores the inert `CCACHE_*` env and reads `SCCACHE_GHA_*` from the action — so no CMake change beyond the Q1 `/Z7` line.
