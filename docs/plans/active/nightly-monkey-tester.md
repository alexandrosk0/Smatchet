...
The non-obvious trade-off (why *guaranteed-reject* rather than "run handlers"): running arbitrary handlers headless would deref subsystems that `AppController::Initialize` never wires (`WireCoreServices` is a separate method) and assume mid-GUI-frame state, producing false-positive crashes — the exact reason `BuiltinCommandsDispatch.test.cpp` stays pre-handler. Handler-body execution is therefore the allow-list-gated **1b** follow-up, not this slice.
...
- **Layer 1b — handler-body execution**: boot with `SetBackendFactory` + `Initialize`, run real handlers for an **allow-list** of headless-safe commands (starts empty). Follow-up plan; the driver already reserves `--mode=handlers`.
