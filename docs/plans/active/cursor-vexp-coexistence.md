...
4. [docs/harness/cursor/rules/agents.mdc](docs/harness/cursor/rules/agents.mdc) — Slice 3: add 2–3-line semantic-search-exceptions block citing root `AGENTS.md`.
5. [docs/harness/cursor/setup.md](docs/harness/cursor/setup.md) — Slice 4: document auto-migration + generated shims + refresh-after-pull + re-install-heals.
6. [docs/harness/SETUP.md](docs/harness/SETUP.md) — Slice 4: Cursor row in per-subsystem table.
...
- **Non-goal**: widening `vexp-strip-agents-md.sh` to portable `agents/core/*.md` (separate P2 [tooling backlog](docs/self-improvement/categories/tooling.md) — `vexp-strip-hook-misses-agent-core-md`).
- **Non-goal**: Cursor hook parity for lint drains / HEAD-drift guard (Claude-only; documented in [docs/harness/cursor/hooks-equivalent.md](docs/harness/cursor/hooks-equivalent.md)).
...
- **Manual residue**: two smoke checks remain manual (acceptable, named here) — (a) new Cursor Agent chat cites `AGENTS.md` delegation table when asked; (b) opening a file under `Source/Core/src/Tracker/` surfaces Tracker invariants without an explicit `@AGENTS.md` read. Cursor has no headless agent-chat driver; deferred-automation action plan → [tooling backlog](docs/self-improvement/categories/tooling.md) entry `cursor-agent-chat-headless-smoke`.
