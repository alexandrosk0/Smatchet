...
**Sequencing dependency — lands after [`multi-grid-tabs.md`](multi-grid-tabs.md).** Multi-grid de-singletons the app: `AppController::Backend` (one `atomic_load`ed `ITrackerBackend`) becomes `map<paneId, GridLiveContext>` (each pane its own `unique_ptr<ITrackerBackend>`), and the single active query (`cfg.JqlQuery`) becomes per-pane `viewId` with global actions targeting `focusedPaneId`. This window's two cross-cutting touch-points — **which backend answers user/activity/group lookups** and **where add-to-query writes** — both assumed those singletons. Post-multi-grid they resolve through a **pane**: the window captures the **source pane** (the pane the user was selected in) at `Open()` and uses *that* pane's `GridLiveContext` backend for all tracker lookups and *that* pane's view for add-to-query. This plan therefore depends on multi-grid **S1** (de-singleton + per-pane backend + focused-pane model) and **S3/S4** (focus model + global-action routing); its multi-grid-independent parts (P4 query, shared-helper extraction, config keys) carry no such dependency and could land earlier. Exact accessor names (`AppController::BackendForPane(paneId)` / `GridLiveContext::Backend()` / `UiDrawSession::focusedPaneId`) are the *assumed* multi-grid post-landing shape — confirm against the merged API before Slice 2/3.

Scope decisions (confirmed with user 2026-06-06, refined in the `grill-with-docs` pass):
- **Tracker support = Jira full + Plane/GitHub graceful-degrade, resolved per source-pane backend.** `PlaneClient::Collaboration()` returns `nullptr` ([PlaneClient.cpp:149](../../../Source/Core/src/Tracker/PlaneClient.cpp:149)) — Plane exposes no collaboration role. The window **null-checks `Collaboration()` on the source pane's backend** (post-multi-grid there is no single active backend) and renders empty tracker sections when it's null (identity + P4 still work). So whether a given open degrades is decided by the backend of the pane the user was selected in — a Plane pane → empty tracker sections, a Jira pane → full. **Zero new Plane/GitHub backend code** (enabling Plane collaboration is a separate future plan).
...
- **`onAddToQuery` = replace the source pane's view JQL with `key = "{key}"`.** Post-multi-grid there is no single active query — add-to-query targets the **source pane's** view (the grid the user was opened from) through multi-grid's focused/per-pane view-mutation path, not the retired global `cfg.JqlQuery`. If the source pane has closed while the window is open, fall back to `focusedPaneId` (or no-op + status line if none). **Deliberate deviation from spec 7.4**: the activity Issue cell passes the parsed **key** (not the issue URL) to add-to-query, so the query actually works; the URL is kept only for the context-menu "Open in Browser". (Spec flags the URL behavior as a bug to fix-deliberately-not-by-accident — we fix it.)
...
Extend `ITrackerCollaboration` (backend-agnostic — no `Jira*` leak) with: `FetchUserActivity(accountId, dayFrom, dayTo, progress&, out&, err&)` returning the new in-memory `TrackerActivityEntry` (timestamp, issueKey, issueUrl, summary, actionLabel, details); `FetchGroupMembers(groupName, out&, err&)`; `ClearUserActivity()`; plus a `TrackerActivityProgress` (current/total) for the loading bar. **Jira implements** (one changelog-item-per-row, author-filtered, JQL `assignee WAS / reporter WAS` discovery — the Jira-shaped JQL stays behind `JiraClient`, e.g. a `JiraActivityFeed.{h,cpp}`); **Plane/GitHub keep returning `nullptr` Collaboration** → callers degrade empty. The in-memory **group roster** (group → members, `GroupMemberCache`) is held **per `GridLiveContext`** (each pane's backend has its own roster — post-multi-grid there is no single backend to hang it off), loaded lazily. The AppController delegators (`FetchUserActivity` / `FetchGroupMembers` + the existing `FetchUserGroupNames`) take a **pane/backend selector** (`paneId` or `backendKey`) instead of latching the retired single `Backend`, resolving the source pane's `GridLiveContext.Backend()` and null-checking its `Collaboration()`. All HTTP routes through `TrackerHttpClient` (subsystem invariant). Delegate to `tracker-backend` (+ `architect` for the per-pane delegator reshape, which couples to multi-grid's de-singletoned `AppController`).

**Slice 3 — the UI window.**
New `SmatchetUserInfoUi` (header + P4 section + activity section + groups section + shared issue-key context-menu popup), following the `AnnotateAnalysisUi` / `SmatchetPerfUi` full-window pattern: `DrawWindow(AppController&, UiDrawSession&, bool* pOpen)` + `Open(sourcePaneId, displayName, email, accountId)` — the **source pane id** is captured so all tracker lookups + add-to-query target that pane's backend/view. Registered in `SmatchetUI::drawSecondaryWindows()`. Reuse the **extracted** `P4ClPreview` tooltip + `P4vLaunch`. `onStatus` / `onAddToQuery` thread through `UiDrawSession`; `onAddToQuery` replaces the **source pane's** view JQL with `key = "{key}"` via multi-grid's per-pane view-mutation path (fall back to `focusedPaneId` if the source pane closed). Issue/details/description key rendering uses `FindFirstIssueKeyInText`. All backend/P4 fetches run on a **worker thread**, posting back via `MainThreadDispatcher` (the `SmatchetAuditUi` pattern) — never sync on the ImGui thread (Pillar 2). Window resolves + null-checks the source pane's `Collaboration()` → empty tracker sections on Plane/GitHub / closed pane. One-shot group-fetch flag; Escape/Close → `ClearUserActivity` on that backend; per-render group resolution. Delegate to `grid-engine` / general UI.
...
12. [AppController.h](../../../Source/Core/include/AppController.h:948) + [AppController_CatalogAndFieldEdit.cpp:1609](../../../Source/Core/src/AppController_CatalogAndFieldEdit.cpp:1609) — delegator methods taking a **pane/backend selector** (`paneId`/`backendKey`); resolve the source pane's `GridLiveContext.Backend()` (multi-grid) and null-check its `Collaboration()` (the `FetchUserGroupNames` guard pattern, now per-pane). Lazy in-memory **group roster** (`GroupMemberCache`) held **per `GridLiveContext`**. **No PlaneClient/GitHubClient edits** — they keep returning `nullptr` Collaboration → degrade empty. (Couples to multi-grid's de-singletoned `AppController`; co-design with `architect`.)
...
**Slice 3 — UI**
14. New `Source/Core/include/Ui/SmatchetUserInfoUi.h` + `Source/Core/src/Ui/SmatchetUserInfoUi.cpp` — the window + `Open(sourcePaneId, displayName, email, accountId)` (light zone; ImGui-draw 200-line cap; split into `_Sections.cpp` if it grows). Worker-thread fetch + `MainThreadDispatcher` post-back; resolves the source pane's backend for all tracker lookups.
...
17. [SmatchetUiSession.h](../../../Source/Core/include/Ui/SmatchetUiSession.h:139) — thread `onAddToQuery` (replace the **source pane's** view JQL with `key = "{key}"` via multi-grid's per-pane view-mutation path; `focusedPaneId` fallback) / `onStatus`. Reads `focusedPaneId` + the pane vector that multi-grid adds here.
...
- **Hard dependency on `multi-grid-tabs` landing first.** Slices 2–3 build on multi-grid's de-singletoned `AppController` (`map<paneId, GridLiveContext>`, per-pane backend) + focused/source-pane model — APIs that don't exist until multi-grid S1+S3/S4 merge. Risk: building against an assumed-but-unmerged shape. Mitigation: the assumed accessor names are flagged inline (Context, Existing-utilities) as "confirm against merged API"; **Slice 1 (P4 query + shared-helper extraction + config keys) carries no multi-grid dependency** and can land independently/earlier. If multi-grid slips, Slices 2–3 wait; Slice 1 still ships.
...
- **Plane/GitHub degradation** — both return `nullptr` Collaboration, so the tracker activity + groups sections are empty when the **source pane's** backend is Plane/GitHub. Mitigation: window null-checks the source pane's `Collaboration()`; identity + P4 sections still work. This is the chosen meaning of "Jira + Plane support" (grill Q1) — **no Plane/GitHub backend code in this plan**. (Multi-grid makes mixed Plane+Jira panes concurrent, so the degrade decision is genuinely per-open.)
...
- **Build gate**: `cmake --build --preset ninja-iter-msvc --target SmatchetStandalone SmatchetCore_DX12` (dual-target) — per slice, anchored to the new TUs added to both source lists. **Slices 2–3 gate on `multi-grid-tabs` being merged into `develop` first** (they reference `GridLiveContext` / per-pane backend / `focusedPaneId`); rebase this branch on the merged multi-grid before building them. Slice 1 builds independently.
