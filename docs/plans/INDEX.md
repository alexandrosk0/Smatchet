...
The table below is **auto-generated** by `scripts/dev/test-plan-index.sh` from the files in `docs/plans/shipped/` — do not hand-edit between the markers. To change a plan's one-line summary, add an `<!-- index-summary: ... -->` comment in that plan file. CI fails on drift (`doc-validation.yml`); regenerate with `bash scripts/dev/test-plan-index.sh --fix`.
