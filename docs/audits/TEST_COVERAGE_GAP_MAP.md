...
| Metric | Value (2026-07-14) | (was 2026-07-05) |
|---|---|---|
| Core TUs compiled into ≥1 test target | **190 / 340 (56%)** | 144 / 304 (47%) |
| Core src LOC compiled into ≥1 test target | **~51.7K / 103.7K (50%)** | ~42.7K / 100.8K (42%) |
| Untested TUs | **150** | 160 |
| Stale CMake test references (phantom TUs) | 0 | 0 |

> The untested-TU backlog stays below 160. Between 2026-07-05 and 2026-07-13 the Tier-2 backend HTTP shells joined the rig (loopback-fixture suites, `coverage-gap-tier2-backend-shell-fixtures.md`), and the 2026-07-13 pass added `Vcs/GitHubCommits.cpp` (the last Tier-2 shell), 9 facet-based `Commands/Builtin` handler TUs + `CommandRegistry.cpp`/`FuzzyMatch.cpp` into `SmatchetTests`, and two new tested UI `_detail` extractions (`SmatchetFieldIconRender_detail`, `AnnotateAnalysisUi_Modals_detail`). The 2026-07-14 pass migrated `Commands/Builtin/BuiltinCommands_Fields.cpp` onto a new narrow `IAppFields` facet (moving the `fields.*` handler bodies into the direct denominator), lifted the autocomplete `\x7F` caret-sentinel into the tested `SmatchetAutocompleteUi_detail` (routing its user-token quoting to the shared `tracker_query_suggest` helper), and split the pure grid-column sort comparator + date/attachment field classifiers out of `TicketGridModel.cpp` into a new tested `TicketGridSortPure` (the model's render-plan/column-builder code keeps it tied to the ImGui grid-draw headers, so the comparators rode along uncovered until the split). (The denominator rose past the 2026-07-05 snapshot as feature work landed new TUs on `develop`; the recount is live as of 2026-07-14.) The Tier-1 parent shells (`TrackerGridFieldDisplay.cpp`, `TrackerDateTimeFieldEditor.cpp`, `JqlSuggestEngine.cpp`, `SmatchetMergeWatchNotifyServer.cpp`, `AiPrefsTestConnection.cpp`, …) remain untested by design — the pure logic was extracted out of them.
...
| Command registration + handlers (`Commands/`, strict zone) | 24 | 4.6K | Consolidated to 19 handler TUs; **12 now compiled into `SmatchetTests`** (2026-07-14) + registry Dispatch — see Tier 3 for the 7-TU residue |
...
|---|---|---|---|
| 1 | `TicketGridModel.cpp` | 495 | ~~2 findings in `CPP_CODE_AUDIT.md`, incl. **#3 Medium: infinite loop in `ParseDurationToSecondsForSort`**~~ — the duration parser was extracted to `TicketGridDurationSortPure` (2026-07-05, fix + fuzz). **2026-07-14:** the remaining column-sort comparator (`CompareFieldValuesForSort`) + date/attachment classifiers (`IsTrackerDateOrDateTimeField` / `IsAttachmentFieldId`) split into tested `TicketGridSortPure`. Residue in this TU is the render-plan/column-builder code, which stays tied to the ImGui grid-draw headers (`TrackerGridFieldDisplay`/`TrackerDateTimeFieldEditor` predicates). |
...
3. *2026-07-13 pass:* the 9 facet-based handler TUs (`_Helpers`, `_App`, `_Tickets`, `_TicketMutations`, `_Sync`, `_Users`, `_Offline`, `_Attach`, `_Automation`) plus `CommandRegistry.cpp`/`FuzzyMatch.cpp` are now compiled **directly into `SmatchetTests`** with fake `IApp*` facets (`tests/Commands/BuiltinFacetCommands.test.cpp`), driving the handler BODIES through real Dispatch — confirm gate, validation, aliases — and putting them in the OpenCppCoverage denominator. *Follow-up 2026-07-13:* `_Meta` (`commands.*`) and `_Config` (`config.*` + `tickets.monitor`) joined too — both had an **unused** `AppController&` param, dropped in the same change (registry-introspection + `ConfigManager` singleton respectively), so they now take no app object and are both harness-tested and off the AppController fan-in. **11 handler TUs** now in the direct denominator. *Follow-up 2026-07-14:* `_Fields` (`fields.list_available`/`get`/`refresh_catalog`/`icon_for`) migrated off `AppController&` onto a new narrow `IAppFields` facet (`GetAvailableFields`/`FindFieldById`/`RefreshFieldCatalog`/`GetFieldCatalogError`/`TryGetFieldIconMapTarget`; the middle two mirror signatures already on `IAppTicketMutations`/`IAppSync`, so one AppController override serves every base), driven by a `FakeFields` fake in `BuiltinFacetCommands.test.cpp`. **12 handler TUs** now in the direct denominator.

Residue: **7** handler TUs remain out — the `AppController&`-coupled `_Perf`/`_Ai` and the UI-session `_Debug`/`_Ui`/`_BugReport`/`_Scenario`/`_UiTest`, plus the `BuiltinCommands.cpp` dispatcher — covered by layers 1–2 only. Migrating them follows the fan-in Phase-5 facet track: each TU that moves off `AppController&` onto a narrow facet (or, like Meta/Config, sheds an unused one) becomes harness-eligible. `_Perf` and the UI-session TUs pull `imgui`/`SmatchetUiSession`, so they stay the `SmatchetCommandsTests` lane's job; `_Ai` is AI-gated and carries an unused `AppController&` param (a Meta/Config-style drain once the AI build variant is wired into this rig).
...
- The bucket-C/E lane is bootable and gating merges again, re-arming the scenario TUs + 5.3K LOC of existing harness and closing the Pillar-4 visual-validation gap that previously fell to humans. (Historically this section's highest-leverage action was *restoring* that lane — now done.) The residual UI-coverage work is continued `_detail` extraction to pull draw-layer logic into the ctest-instrumented denominator. *2026-07-14:* the `\x7F` caret-anchor `StripCaretAnchorSentinel` was lifted out of the `SmatchetAutocompleteUi.cpp` anon namespace into the already-tested `SmatchetAutocompleteUi_detail` sibling (bucket-A tested); the co-located user-token quoting was a copy of the shared `tracker_query_suggest::InsertForValueToken` (itself already tested), so the shell now calls that directly instead of carrying its own untested copy — one fewer DRY clone, no new duplicate.
...
4. ~~**One command-invocation harness PR** (Tier 3).~~ **Done** in three layers (scenario sweep #1618; Linux `SmatchetCommandsTests` dispatch harness; 2026-07-13 facet-handler suite in `SmatchetTests`, now 12 handler TUs incl. Meta/Config/Fields). Residual: the 7 remaining AppController/UI-session-coupled handler TUs (see Tier 3); `_Ai` is the next facet drain once the AI build variant joins this rig.
