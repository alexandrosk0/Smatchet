...
1. **Live credentials** — every tracker backend (Jira / GitHub / Plane), every AI client (OpenAI / Anthropic / Ollama), and the `P4Annotate` C++ feature shell out to real services. Reproducing a backend-touching bug needs real keys.
...
Each backend that today requires credentials gets a different fake shape, chosen by the backend's real-world seam rather than unified onto a single mechanism. Documented at `docs/plans/shipped/autonomous-debugging-no-creds.md` § Approach point 1.
...
- **`P4Annotate` C++ feature (annotate + describe-cache)** injects a **runner-seam fake** at the single `P4Annotate.cpp:P4RunCommand` spawn-site via a new `AnnotateAnalysisConfig::P4RunOverride` function-pointer field. `tests/support/FakeP4Runner.h` installs a lambda that returns canned `(exit, stdout, stderr)` triples keyed on argv-prefix. **Explicitly rejected**: a stub `p4` binary on `PATH`. The dev-environment `scripts/dev/p4-*.sh` shell layer (30+ subcommand shapes, opt-in `SMATCHET_AGENT_VCS=p4`, exercised manually) is out of scope; the C++ annotate feature has exactly two subcommands and one spawn-site, so the runner-seam approach is dramatically smaller.
...
A new header-only macro `SMATCHET_AGENT_DEBUG_LOG(category, json_obj)` appends one structured NDJSON line per call to `<userData>/agent-debug/<session-id>.ndjson` (production) or `tests/_debug/scratch/<test-name>.ndjson` (doctest / bats). Closed-set category enum (`backend-call`, `ui-event`, `worker-handoff`, `lock-claim`, `scenario-phase`, `cli-command`, `temp-debug`). 5-field schema (`ts`, `category`, `pid`, `tid`, `payload`). Mutex-guarded ofstream; one file per session; `git-janitor` end-of-session prunes files older than 30 days. Full operational contract at `docs/plans/shipped/autonomous-debugging-no-creds.md` § Slice 7.
