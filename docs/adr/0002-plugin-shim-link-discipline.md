...
Every Smatchet plugin static library that includes `Source_Core/include/` headers must link the same `*Shim` INTERFACE libraries that the Source_Core impl target it talks to was built against. Mismatched defines silently shift struct layout and cause UB at runtime.

## Background

`Source_Core/include/SmatchetUiSession.h` carries three macro-gated member blocks inside `UiDrawSession`:
