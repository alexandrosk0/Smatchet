...
The full agent surface moves to its own **public** repository, **`the-unwilling-agentic-bunch`** ("The Unwilling Agentic Bunch"), with history preserved via `git filter-repo`. Smatchet consumes it as a **git submodule mounted at `agent-layer/`**, referenced by an **absolute, tokenless public HTTPS `.gitmodules` URL** (`https://github.com/alexandrosk0/the-unwilling-agentic-bunch.git` — absolute rather than relative, so a fork of Smatchet does not resolve the submodule to a non-existent `<fork-owner>/the-unwilling-agentic-bunch`). Layer changes reach Smatchet only through **pointer-bump PRs**, opened automatically per layer merge (`auto-bump.yml`), gated by a host-side integration lane (`agent-layer-integration.yml`) so a bump can never land unexercised. **Public** is a gated outcome, not an assumption: the seed is an explicit path allowlist and every path on it clears a portability **and history** audit before the first push (*Publication is allowlisted, not subtractive*, below) — `agents/project/**` and `agents/scripts/core/historical-review-sweep.js` are project-scoped and are excluded or scrubbed rather than published as-is.

Four boundary calls shaped the split:
...
- **Publication is allowlisted, not subtractive**: the seed manifest is an explicit path allowlist, and every path on it must clear a portability **and history** audit (working-tree content plus `git log -p --follow`, for internal identifiers, hostnames, ticket URLs, customer data and credentials) plus a history-wide secret scan of the *rewritten* history, before the first public `git push`. `agents/project/**` and `agents/scripts/core/historical-review-sweep.js` are known project-specific and are excluded or scrubbed; a path that cannot clear the audit stays host-side. The gate runs before the push because publishing is one-way — a public history cannot be un-published, only revoked and rotated after the fact.
