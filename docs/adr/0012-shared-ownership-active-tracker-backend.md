...
- **Scope note:** a worker that dereferences `app.Backend` across a blocking call must latch a **local `shared_ptr` copy** for the call's duration — reading the member fresh is not enough, because the swap can free the object mid-call. `AppController::RefreshFieldCatalog` (reached from site #2's worker) is fixed this way in S1. The remaining `Backend->…`-across-blocking-call sites in `AppController_CatalogAndFieldEdit.cpp` (the `FetchFieldCatalog` overloads, `FetchIssueEditMeta`, the `Mutations()` payload path) follow the same pattern and are reachable from other async entry points; latching them all is a tracked follow-up (see the memory-plan § Deviations + `tooling.md`), not part of this PR's migration set.
