...
- **Conflict context** gains a top-level `kind` discriminator (`text` | `scalar` | `unverified`); rows written before this change carry no `kind` and default to `text` so the existing rich-text modal path stays byte-identical. This `kind` is **orthogonal to** the existing per-field `richKind` key (`adf`|`html`, written by `OfflineQueueService`'s field-edit enqueue path): `kind` selects the conflict *category*, while `richKind` is **unchanged** and keeps driving the rich-branch markdown→ADF/HTML reconversion at resolve. New rich conflicts write `kind:"text"` alongside the existing `richKind`.
...
- Full design + verification buckets: `docs/plans/offline-conflict-ask-all-fields.md`.
