...
Fields in that order (`; ` between them). Four `when=` kinds (`;`-delimited `key=val` inside the spec — **no space** after the `;`, which is how the parser tells the within-spec `;` from the between-field `; `):
