...
### What escaped
`docs/plans/shipped/msvc-build-onboarding-hardening.md:85` (§ Deviations from plan) states: "`build_standalone.ps1` (plan file 1) already had the MSVC bootstrap from slice 1." That bootstrap **does not exist and never did** — `git log -S 'vcvars' -- scripts/dev/local/build_standalone.ps1 scripts/dev/build_standalone.ps1` is **empty across all history**. The file's only `vswhere` use is `Get-VsWherePath` (:71-95), which locates `MSBuild.exe`; there is no `vcvars64.bat` import anywhere in it. PR #493 (`a9058b96`) planned the capability, PR #495 (`da36b45f`, 2026-05-28) closed the row as already-done, and the plan was archived `shipped` on 2026-06-06. **No gate reads a plan's post-ship prose against the tree**, so a false record of delivered capability sat load-bearing for ~2 months and seeded a false § Context premise into a downstream plan (`dev-onboarding-first-run-quickstart`).
