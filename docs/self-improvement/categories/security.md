...
- 2026-05-30 · log-a-bug-github · [security] · P1 — bug-reporter must NOT ship a GitHub token if Smatchet is distributed to external/untrusted users
  ...
  Status: resolved-in-code (relay shipped) — **deploy required before external release**. Relay built at `tools/bug-report-relay/` (Cloudflare Worker; holds the token server-side, creates the issue + uploads screenshots, optional `x-relay-key`, 256 KB cap). App relay-mode wired: config keys `bugreport_relay_url` / `bugreport_relay_key`; when the URL is set, `ResolveBugReportTarget` returns `UseRelay` and `SubmitViaRelay` POSTs the report — no GitHub token on the client. Remaining: the user must `wrangler deploy` the Worker + set the `GITHUB_TOKEN` secret, then add a CF rate-limit rule. Keep open until deployed for an external build.
