...
- 2026-05-30 · log-a-bug-github · [security] · P1 — bug-reporter must NOT ship a GitHub token if Smatchet is distributed to external/untrusted users
  ...
  Concrete next action: before any external/public release, build a minimal relay endpoint (accepts redacted report JSON + optional screenshot, re-files via a server-held PAT, rate-limited + abuse-guarded) and point `BugReportGitHubBaseUrl` at it; until then keep the feature internal-only and never bundle a PAT. ~1–2 d.
  Status: open
