...
- 2026-05-31 · orchestrator · [tooling] · P1 — Delta gates (`function_size_audit.py --diff`, `comment_audit.py --diff`) false-flag a PR branched before a decomposition landed, because CI's shallow clone defeats `git merge-base`
  Details: During the decompose-top-20-monoliths parallel rollout, PR #633 (a Tracker-only change) failed the high-integrity gate inside `test-all.sh` with `function-too-long`/`comment-commented-out-code` findings on `SmatchetActiveProjectGridUi.cpp` — a file it never touched. Root cause: #633 branched from develop *before* the `drawActiveProjectWindow` canary (#632) merged. `function_size_audit._merge_base_or_ref` runs `git merge-base origin/develop HEAD`; under the CI checkout's shallow history that merge-base commit isn't present, so it returns empty and **falls back to tip-of-develop**. The gate then diffs the PR's stale tree (992-line monolith still present) against develop's decomposed 140-line version → the monolith looks "new". Any stacked/parallel PR cut before a sibling decomposition lands hits this. Workaround used this session: `git merge origin/develop` into the branch before merging (verified: clears the false-positive). `comment_audit.py` shares the identical `_merge_base_or_ref` fallback, so the comment gate has the same failure mode.
  Concrete next action: make the CI checkout fetch enough history for merge-base to resolve — `actions/checkout` with `fetch-depth: 0` (or a targeted `git fetch --deepen` / fetch of the PR base before the gate step) in the workflow(s) that run `test-all.sh` / the delta gates. Alternatively, have `_merge_base_or_ref` fail **closed** (error, not silent tip-fallback) when merge-base can't resolve, so the misconfiguration is loud rather than a confusing false-fail. ~30-45 min (workflow edit + one stacked-PR validation). Until fixed, the standing workaround is "merge develop into the branch before merging a decomposition PR."
  Status: open
