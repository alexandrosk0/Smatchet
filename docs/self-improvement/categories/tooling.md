...
- 2026-05-30 · orchestrator · [tooling] · P3 — `test-portable-purity` runs only in CI, not in `test-all.sh` or a lint hook → NEW-literal leaks caught post-push
  Details: `scripts/dev/test-portable-purity.sh` (blocks NEW project literals in portable dirs `agents/core`, `agents/_shared`, `docs/agent-rules`, `docs/harness`) is wired into the `Doc anchors + agent contract` CI job but is **absent** from `scripts/dev/test-all.sh` and from any PostToolUse lint hook. So an agent editing a portable doc (e.g. `docs/agent-rules/memory-drain.md`) only learns of a leaked literal after push → CI fail → fix → re-push. Cost a full CI round + merge-gate delay on PR #557 (`SMATCHET_*` env names + `Source/Core` example leaked; would have been a 5-sec local catch). Distinct from the existing infra entry about *shrinking* the baseline — this is about *catching* NEW leaks at edit/pre-push time.
  Concrete next action: add `test-portable-purity.sh` to `test-all.sh` (pre-push gate), and/or a PostToolUse hook that runs it when an edited path is under a portable dir (`agents/core/`, `agents/_shared/`, `docs/agent-rules/`, `docs/harness/`). ~20 min. Note: it also covers a self-mod-guard edge — the orchestrator can't always run it inline, so the pre-push gate is the reliable catch.
  Status: open
