...
- 2026-06-01 · orchestrator · [process] · P2 — Decomposition PRs must NOT edit `docs/high-integrity/function-size-baseline.md` — it's an informational snapshot, not the gate input
  Details: The function-size gate is the live `function_size_audit.py --diff origin/develop` merge-base delta; the checked-in `function-size-baseline.md` is informational only (the script's own `--baseline-md` header states this). A decomposed function simply leaves the over-cap set, so the delta gate passes with **no** baseline edit. The prior decompose batch (#665–671) nonetheless edited baseline.md in every PR to drop its entry — unnecessary, and it created a cross-PR **merge cascade**: each merge re-conflicted every sibling PR on baseline.md, forcing repeated regen+resolve (this taxed ~5 PRs). Verified the fix this session: the flagship batch (#676–680) touched only each PR's own source file (+ tests), zero baseline edits → zero cross-PR conflicts; one end-of-campaign regen (#682) refreshed the snapshot. (Compounded with, but distinct from, the shallow-clone `merge-base` false-flag in tooling.md P1 — that's CI history; this is self-inflicted baseline churn.)
  Concrete next action: add a one-liner to `docs/guides/imgui-draw-pattern.md` + the decompose plan § Approach — "decomposition PRs leave `function-size-baseline.md` alone; regenerate once per campaign." Consider retiring the checked-in baseline entirely since the gate is git-derived. ~15 min.
  Status: open
  Last-reviewed: 2026-06-01

- 2026-05-30 · orchestrator · [process] · P2 — Git loss/residue audits must diff vs `origin/<base>` (post-fetch); `ahead`-count + tip-diff are NOT loss signals
  Details: A "what's lost in git?" sweep this session produced two consecutive wrong "🔴 lossable — push it" verdicts, both retracted. Root cause: branches were diffed against the **local** `develop` ref, which was 12 commits behind `origin/develop` (a sibling agent/daemon had advanced origin and squash-merged the exact work via PR #576). Against the stale local ref the merge-watcher branch looked "ahead with unique code"; against `origin/develop` its code was byte-identical — already shipped. Compounding it: squash-merge leaves a merged branch's `ahead`-count > 0 and its tip-diff non-empty permanently (new commit hash + develop moving on), so both were misread as loss signals. The only merge-truth is `gh pr view <head-PR> --json state == MERGED` (or byte-identical content vs `origin/<base>`). Distinct from the 2026-05-30 "verify file-behavior claims" entry below (that = file *contents*; this = branch *merge-state* methodology) and the daemon-thrash infra entry (that = HEAD swapping mid-edit; this = which base ref to diff against).
  Concrete next action: add a § callout to AGENTS.md § Debug techniques (or a `docs/agent-rules/git-audit.md` stub): "Loss/residue audits — `git fetch` first, diff every branch vs `origin/<base>`, never a local tracking ref (siblings/daemon advance origin → local goes stale). Squash inflates `ahead`-count + tip-diff permanently → never loss signals; `gh pr state == MERGED` is the only merge-truth." Enforced concretely by the paired `git-leftover-audit.sh` tooling entry. ~10 min doc edit.
  Status: open
  Last-reviewed: 2026-05-30

- 2026-05-30 · orchestrator · [process] · P3 — Backlog/self-improvement/plan entries assert file-behavior claims without verifying the cited file → caught downstream at a CI/CR round
  Details: twice in one session (2026-05-30) the orchestrator filed a docs entry whose factual premise was wrong because it was written from assumption, not from the cited file: (1) an infra entry claiming the MSVC toolset pin had "regressed out of `with-msvc-env.sh`" and "no `project.config.json` exists" — both false on develop (the read had landed on a watcher-swapped sibling branch; retracted after `git show origin/develop:<file>`); (2) a tooling entry claiming `test-portable-purity.sh` was "absent from `test-all.sh`" — false: `test-all.sh:46` globs every `test-*.sh`, so it is discovered (CodeRabbit flagged it on PR #562, costing a fix+re-push round). Both would have been a one-line `grep`/`git show` catch at authoring time. The existing memory-drain "verify-first" rule covers *memory* items; this is the same discipline for *backlog/SI/plan-doc* claims about a script or file's behavior.
  Concrete next action: before asserting in any backlog/self-improvement/plan entry that a specific file does/doesn't do X, verify it — `git show origin/develop:<file>` (never the mid-session working tree, per the daemon-thrash infra entry) or a targeted `grep`. Add a one-line forcing rule to `docs/self-improvement/AGENT_SELF_IMPROVEMENT.md` § workflow ("claims about file behavior must cite a verified line"). ~10 min doc edit. Related: the memory-drain.md § Verify-before-trusting rule + the daemon-clone-isolation infra entry.
  Status: open
  Last-reviewed: 2026-05-30
...
- 2026-05-26 · orchestrator · [process] · P3 — Source/Core source files should be kept under 67 KB; files exceeding this limit must be split and the split technique encoded in project rules
...
- 2026-05-24 · orchestrator · [process] · P3 — slice→agent routing should reject test-rig for production-header slices
  Details: Wave-A slice 7 of `autonomous-debugging-no-creds.md` was dispatched to `test-rig`. The agent correctly declined: scope was ~80% production code (new `tests/_debug/SmatchetAgentDebug.h` production-resident header, `Source/Core/include/Logger.h` macro edit, CMake option wiring, `docs/agent-rules/delegation.md` docs edit, branch/commit/push/PR ship authority) — all outside test-rig's "doctest files under `tests/Core/`" charter. Re-dispatched to `general-purpose` and shipped (PR #445). Cost a clean abort + restart cycle; clean self-flag from test-rig (no half-shipped state).
  Concrete next action: extend the orchestrator's slice→agent routing heuristic in `docs/agent-rules/delegation.md` § Subsystem specialists with: "if slice creates new production headers or edits `Source/Core/include/*.h`, do NOT route to test-rig — route to the closest subsystem specialist or `general-purpose`. test-rig stays scoped to tests/Core/ doctest expansion against an already-shipped helper." 5-minute edit; surfaces only on the next slice that pre-mixes production + test surfaces.
  Status: open
  Last-reviewed: 2026-05-24

- 2026-05-24 · orchestrator · [process] · P3 — plan-doc drafts should grep-verify pre-named TUs don't already exist
  Details: Wave-A slice 1 of `autonomous-debugging-no-creds.md` directed the agent to create `Source/Core/{include,src}/GitHubIssueMappingPure.{h,cpp}`. Those names didn't exist, but the equivalent pure helpers had already shipped under different names from PR12 (`GitHubIssueSearchMapping.{h,cpp}` + `GitHubClientHelpers.{h,cpp}` + `GitHubQueryFromJql.{h,cpp}`). Agent caught the duplication via a 30-second sanity grep and reused the existing TUs — correct outcome, but the plan author shouldn't have to depend on per-agent rigour. Low friction this round; high cost the day an agent doesn't catch it and lands a parallel duplicate.
  Concrete next action: add a one-liner to `docs/plans/active/_plan-template.md` § Files to modify: "Before listing a new `<Foo>.{h,cpp}` here, run `rg -l '<Foo>' Source/Core/` to confirm it doesn't already exist under that or a synonym name." 2-minute edit. Surfaces every time a plan author lists fresh TUs without first grepping the codebase.
  Status: open
  Last-reviewed: 2026-05-24
