...
- 2026-06-02 · test-author · [test] · P1 — deterministic Jira fixture backend (#663) is broken on develop: sync completes but tickets never load
  Details: `tests/ui/jira_deterministic_backend.test.cpp` fails `!tickets.empty()` after a completed `SyncWithBackend()` in the spawn harness — `IsStreamingSyncActive()` goes false (sync "completes") but `GetActiveTickets()` stays empty, so the fixture's SMAT-1/SMAT-2 never reach the active-tickets model. Found while building the Phase-0 yellow pilot (`feat/funcsize-phase0-yellow-pilot`, committed RED): both the new grid test AND the canonical #663 control fail identically on a fresh develop build. Ruled out: env propagation (CreateProcessA inherits parent env), tracker-type mismatch (default IS "Jira", factory returns the scripted client). Remaining suspect: the scripted-client → `TicketSyncService` streaming-result sink (scripted client may not feed the result sink like the real Jira client, or sync early-exits as a no-op). **Blocks the ENTIRE Phase-0 yellow batch** (grid / ViewsDashboard / NewIssueDraft / GridHeader / OfflineQueue / cell editors) — no fixture-coupled bucket-E coverage can go green until fixed → the data-dependent Phase-5 decompositions stay gated.
  Concrete next action: dispatch `debug-detective` to repair the scripted-client sync→active-tickets propagation. Once tickets load, the yellow pilot + its copy-skeleton go green with zero test-side changes. The pilot test + fixture-aware driver (`scripts/dev/test-ui-funcsize-grid-render.sh`) are ready on `feat/funcsize-phase0-yellow-pilot`.
