...
- 2026-05-31 · general-agent · [test] · P2 — `SmatchetTests` link hard-errors locally on `tests/Core/OfflineQueueServiceRuntime.test.cpp` (`CHECK_THROWS` under exceptions-disabled MSVC) — blocks local ctest, forces a standalone-harness workaround
  Details: Two monoliths slices this session (HtmlToMarkdown #639, plus a near-miss on FetchFieldCatalog #643) could not run the real `SmatchetTests` doctest rig locally because `OfflineQueueServiceRuntime.test.cpp` uses doctest's throwing assertions (`CHECK_THROWS*`) while the `SmatchetTests` target builds with no `/EHsc`, so doctest emits a hard `#error "Exceptions are disabled"` and the whole test binary fails to compile/link. Agents then reconstruct a standalone doctest harness from the same sources to verify their new tests (48/48, 26/176 etc.) — wasted round-trips, and it means the *real* rig is unrunnable locally for anyone whose toolchain hits it. CI passes because its test build configures the correct exception flags; this is a local-build-config gap. (Note: one toolchain in the same session did NOT hit it — so it's toolchain/preset-dependent, which makes it a confusing intermittent blocker.)
  Concrete next action: either (a) add `/EHsc` to the `SmatchetTests` target's compile options in `tests/CMakeLists.txt` so throwing assertions compile everywhere the test rig builds, or (b) guard the `CHECK_THROWS*` assertions in `OfflineQueueServiceRuntime.test.cpp` behind `#if !defined(DOCTEST_CONFIG_NO_EXCEPTIONS)` (or convert them to non-throwing checks) so the rig links without exceptions. Prefer (a) — the suite already exercises throwing paths, so exceptions-on is the honest config. ~30 min + a clean `ninja-test-msvc` ctest run to confirm. Lets agents run the real `ctest -R <unit>` locally instead of rebuilding a harness per slice.
  Status: open
...
- 2026-05-19 · coderabbit-triage · [test] · P2 — `AgentProposalStore.test.cpp` SQLite tests live in pure-logic rig; no bucket-E SQLite lane exists yet
  Details: CodeRabbit findings #15 + #16 on PR #264 (handoff-half re-review) flagged `tests/Core/AgentProposalStore.test.cpp` as violating the "tests/Core/** must be pure-logic doctest only" rule — `test-rig` agent contract refuses SQLite surfaces. The tests were knowingly authored under the H10 plan as integration-coverage in the doctest rig because no bucket-E SQLite harness exists today. Moving them now has no destination lane. Triage marked DEFERRED rather than rejected because the suggestion's intent (proper lane separation) is correct; the blocker is missing infrastructure.
