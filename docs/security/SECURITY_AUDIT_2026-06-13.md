...
| H2 | **Secrets stored plaintext at rest on every non-Windows target (POSIX + Android)** | `ConfigManager_PathUtils.cpp:388-424` (non-Windows `ProtectSecretForConfig`/`UnprotectSecretFromConfig` seam — the `#else` that historically did `{ return plainText; }`, now the Android Keystore / POSIX-perms path); sink writes `token`/`plane_api_key`/`github_pat`/`mcp_auth_token`/`ai_*_api_key`/`whisper_api_key` raw at `ConfigManager.cpp:473-496` | `ProtectSecretForConfig` is DPAPI only under `_WIN32`; the `#else` branch is a no-op, so all API tokens land as cleartext JSON with default umask, no `chmod 0600`, no Android Keystore. Confirmed **HIGH** by the re-run. Windows (the shipping target) is unaffected. | Per-platform: `chmod 0600` + verify-mode-on-read on POSIX; back Android secrets with the Keystore (or mark the platform unsupported for secret storage). Document the crypto gap. | **FIXED** (POSIX perms PR `fix/posix-secret-at-rest-perms-h2`; Android Keystore PR `feat/android-keystore-secret`). **POSIX (Linux/macOS) done**: `AtomicWriteTextFile` chmods the temp file to `0600` *before* the atomic rename (no 0644 window); `LoadJsonFile` LOG_WARNs on a group/world-readable user config; decision logic extracted to the pure `IsLooseConfigFileMode` (unit-tested). Crypto gap documented at `ProtectSecretForConfig`. **Android Keystore DONE** — config secrets are now sealed at rest via an AndroidKeyStore AES-256-GCM key (alias `SmatchetConfigSecret`, StrongBox-preferred with TEE fallback, provider-generated 12-byte IV prepended, base64) installed as the process-wide ConfigManager secret provider at boot *before* the first `Load` (Core seam `ApplyConfigSecretProvider` + `ConfigManager::SetAndroidSecretProvider`; JNI `SmatchetAndroidSecretBridge` ↔ Java `SmatchetSecretStore` via `SmatchetActivity.protectSecret`/`unprotectSecret`). Fail-safe: every provider arm returns EMPTY on any Keystore/JNI failure, so a secret is never persisted or surfaced in cleartext; and Core **fails closed** when no provider is wired (pre-Keystore build / host-init failure) — an unsealed Android secret is dropped (never written cleartext) and reads return empty, with a loud WARN flagging the dropped secret. `_WIN32` DPAPI path UNTOUCHED. Closed — POSIX `0600` + Android Keystore both shipped |
...
| **Command registry runs destructive commands with no `ctx.Source` authz** | `CommandRegistry.cpp:264,298` — gates only on `Destructive && !ConfirmedDestructive` | MCP/Lua-sourced commands have the same reach as UI-sourced; no per-source trust gate. **RESOLVED 2026-06-14** — `Dispatch` now routes the destructive gate through the pure source-aware predicate `RequiresExplicitConfirm(source, destructive, confirmed, dryRun)` ("require-confirm destructive on non-UI sources" posture) and emits a `LOG_WARN` audit line for every destructive dispatch from an automation source (CLI/MCP/Lua). | synthesis #2 |
| **MCP registry dispatch un-gated after Authorize** | `McpPlugin.cpp:426-445,625-642` | After loopback+token Authorize, dispatch reaches the full command registry — token possession == full reach. **RESOLVED 2026-06-14** — token possession no longer implies destructive authority: MCP/Lua `ConfirmedDestructive` is set only from an explicit per-call `__confirm` arg (the Lua binding previously never read confirm at all — now fixed), so a destructive registry command from MCP/Lua without explicit confirm returns `confirm-required`. | synthesis #3 |
...
| **OfflineQueue serialized `draft` string bypasses audit redaction** | `OfflineQueueService.cpp:356,362` serialize to a JSON string before `BackendAuditTrail.cpp:124-148` redaction runs | `RedactJson`/`LooksSensitiveKey` never see the nested keys. Redact the draft structurally pre-serialization. **RESOLVED 2026-06-14** — `MakeAuditDraft()` parses the payload back to a structured object + `RedactJson` before it reaches the trail; enqueued/replayed payload unchanged. | synthesis #10 |
