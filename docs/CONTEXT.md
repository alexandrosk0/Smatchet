...
- **Source tracker** — where the issue being triaged lives. Persisted as `AgentProposal.sourceTracker` ∈ {`github`, `jira`, `plane`}; composite-PK on `agent_poll_cursor(source_tracker, repo_key)`. Independent from where the resulting PR lives (see Code host). Triage half is source-tracker-polymorphic; ImplementIssue proposals from any source tracker can produce a PR. **The source tracker equals the active grid tracker** — there is no separate "poll source" config axis. The scheduled-poll worker reads `TrackerType` to pick the backend, then dispatches `ITrackerClient::ListIssueKeysUpdatedSince(query, sinceUnixSec)` against it. The legacy `AgenticPollSource` config field is retired; old configs are silently ignored on Load.
- **Code host** — where a PR + check-runs + CodeRabbit review live. GitHub today, only. Independent from the source tracker of the originating issue: a Jira-source `ImplementIssue` proposal still produces a GitHub PR. The Handoff half (H0–H10) is code-host-coupled by construction — `PrCommentWatcher`, `PrCheckRunWatcher`, CodeRabbit GraphQL surface are GitHub-typed. Future Bitbucket / GitLab code-host integrations would extend the code-host axis without perturbing the source-tracker axis.
- **HandoffClarificationPostToSourceTracker** — config flag (renamed from `HandoffClarificationPostToGithub`) controlling whether clarification mirrors land on the source tracker's issue. Source tracker, not code host — the issue is where the human watches for context; the PR is for code review. When the source tracker is GitHub the destination is the same issue that fed the proposal; when Jira / Plane it is the corresponding ticket via `AddIssueCommentPlain`.
- **`UpdateField` semantics — set-replace** — the `ITrackerClient::UpdateField(issueId, field, values, outError)` contract is **set-replace** for multi-value fields (labels, assignees, components): `values` is the **intended full set** after the edit, not a delta. Jira / Plane native APIs are set-replace; the virtual reflects them. GitHub's native API is additive (per-name `LabelAdd`/`LabelRemove`); `GitHubClient::UpdateField` reconciles internally — it pre-fetches the current set, computes a diff against `values`, then issues `LabelAdd` / `LabelRemove` calls per element. The pre-fetch HTTP cost lives inside GitHub's impl; callers see one virtual call.
- **TrackerIssueKey** — canonical issue identifier carried across `ITrackerClient` virtuals. Per-backend shape: GitHub `owner/repo#N` (parsed via `GitHubClientHelpers::ParseGitHubIssueKey`); Jira `PROJ-N` (uppercase project + numeric); Plane `<issue-uuid>` (workspace + project + UUID resolved by the client). The triage controller treats the string as opaque between fetch + write; only the backend impl parses it. Multi-tracker triage relies on the per-backend `GetTrackerType()` to pick the right parser before any URL composition.
- **Audit-trail actor** — JSONL field on every `smatchet_backend_audit.jsonl` row ∈ {`user`, `triage`, `ci_react`, `coderabbit_react`, `lua`, `mcp`}. Discriminates **what triggered** a backend write — distinct from `source` (which backend client) and `action` (which verb). Pre-refactor, triage and user grid edits used different call paths, so the trigger was inferrable from `source`+`action`+timestamp; post triage-tracker-agnostic refactor, both route through the same `ITrackerClient` virtuals and `actor` is the only discriminator. The audit substrate is append-only JSONL, NOT a SQLite table — no migration runs; legacy lines without the key default-read as `"user"`. Required to keep "show me everything triage did to this issue" queries working.
