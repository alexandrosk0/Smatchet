...
- **Source tracker** — where the issue being triaged lives. Persisted as `AgentProposal.sourceTracker` ∈ {`github`, `jira`, `plane`}; composite-PK on `agent_poll_cursor(source_tracker, repo_key)`. Independent from where the resulting PR lives (see Code host). Today the agentic-triage scheduled-poll worker is GitHub-only; making it polymorphic across `sourceTracker` is a deferred refactor (see `docs/design/github-tracker-backend.md` § Out of scope § Agentic triage tracker-agnostic refactor). The `AgenticPollSource` config field remains in tree until that refactor lands.
