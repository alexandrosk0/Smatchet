...
- **Audit-trail actor** — `agent_audit_trail.actor` column ∈ {`user`, `triage`, `ci_react`, `coderabbit_react`, `lua`, `mcp`}. Discriminates **what triggered** a backend write — distinct from `source` (which backend) and `action` (which verb). Pre-refactor, triage and user grid edits used different call paths, so the trigger was inferrable from `source`+`action`+timestamp; post triage-tracker-agnostic refactor, both route through the same `ITrackerClient` virtuals and the `actor` column is the only discriminator. Required to keep "show me everything triage did to this issue" queries working.
