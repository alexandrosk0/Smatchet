...
## Status: Shipped end-to-end (H10 — 2026-05-18)

All 21 phases shipped (T0 → T9 triage half + H0 → H10 handoff half + audit fix-ups A/B/C).
...
| T9 | #234 | `b4a05fd` | schema-version bump (v1) + triage-half plan revision |

### Handoff half

| Phase | PR | sha | Summary |
|---|---|---|---|
| H0 | #240 | `f38c0bf` | plan-lock + ADRs 0005/0006 + glossary update |
| H1 | #244 | `1b9a575` | `SubprocessCapture` lift from P4Blame + helper exes |
| H2 | #248 | `17cc5c9` | `CodingHarnessTypes` + `ICodingHarnessRunner` + SeedBuilder |
| H3 | #251 | `d1bc456` | `ClaudeCodeLocalRunner` + stub-claude exe + env allow-list |
| H4 | #252 | `fcaa533` | `HarnessRunState` FSM + `AgenticHandoffController` + handoff.* CLI |
| H5 | #253 | `33e0b3a` | clarification dual-channel + GitHub-comment poll |
| H6 | #254 | `a5a21cf` | PR-open path + `gh pr create` fallback |
...
| Bundle | PR | sha | Scope |
|---|---|---|---|
| A (Pillar 2) | #235 | `f16f603` | UI-thread non-blocking sweeps |
| B (security + correctness) | #237 | `c7f1fd0` | Atomic per-issue insert, schema-drift fail-closed, transactional Approve/Reject |
| C (quality + minors) | #239 | `51edc58` | Filter cap, log warmth, minor copy/style |

Lock release: PR body carries `lock-slug: agentic-flow` so `.github/workflows/lock-cleanup.yml` auto-releases the ref on merge. Manual fallback for the orchestrator: `AGENT_ID=orchestrator bash scripts/dev/lock-release.sh agentic-flow`.

End-to-end probe (real GitHub PAT + real `claude` install): see § Verification — H10 § End-to-end probe recipe. Runs once post-merge; not a CI gate.
...
- **H9** (this PR) · `feat(agentic): H9 cross-flow wiring + Start-handoff button (ImplementIssue filter)` — `AgentProposalStore` gains an injectable `OnApprovedFn` callback (header `Source_Core/include/AgentProposalStore.h` + impl `Source_Core/src/AgentProposalStore.cpp`). `Transition()` captures the row's action under the same SELECT that reads its state, and fires the callback AFTER `SQLite::Transaction::commit()` succeeds — so receivers see the durable post-commit row. Pending→Approved is the only edge that triggers; other edges (Pending→Rejected, Approved→Applied, etc) silently no-op. Receiver-side exceptions are swallowed with `LOG_WARN` so a misbehaved callback cannot cause `Transition` to report a false-negative to the caller. `AppController::GetAgenticHandoffController` wires the production callback inside the same `std::call_once` block that constructs the controller — the callback filters to `ProposedAction::ImplementIssue`, re-reads `ConfigManager::Load().HandoffAutoStartOnApprove` on every fire (no captured cfg so a Preferences flip takes effect on the next approval), and on auto-start dispatches `Start()` via `LaunchBackgroundTask`. A `HandoffAutoStarted` audit row with `trigger=approval-callback` precedes the dispatch so the audit trail can distinguish auto vs manual paths. The manual path lives in `SmatchetAgentProposalsUi::ApproveAndStartHandoff` (called from the new `[Start handoff]` button), which emits a `HandoffStarted` audit row with `trigger=user-button` and races cleanly with auto-start (the controller's "already in flight" branch surfaces as a softer toast). New cfg field `TrackerConfig::HandoffAutoStartOnApprove` defaults to FALSE per plan-locked decision #5 — the shipped trust baseline is manual handoff start; auto-start is opt-in via the new `Auto-start handoff when an ImplementIssue proposal is approved` checkbox on the Agentic Preferences tab. JSON round-trip via `j.value()` default — additive, no schema bump. The `[Start handoff]` button is gated on `row.action == ImplementIssue && app.GetAgenticHandoffController() != nullptr` (defense in depth — the callback applies the same filter on its side, so a future regression on either guard cannot silently kick off a handoff for a non-handoff proposal). Coverage: 4 new doctest cases in `AgenticHandoffController.test.cpp` (non-ImplementIssue approval no-ops; ImplementIssue + auto_start=false no-ops; ImplementIssue + auto_start=true dispatches; manual button path appears in SnapshotActive). Bucket-E coverage: `tests/ui/agent_proposals_handoff_button.test.cpp` with 4 variants — `CommentAdd_NoHandoffButton`, `LabelAdd_NoHandoffButton`, `ImplementIssue_ButtonVisible`, `ImplementIssue_ClickStartsHandoff` — driver `scripts/dev/test-ui-agent-proposals-handoff-button.sh` auto-enrolled by `test-all.sh`. Five new locale keys under `agent.proposals.*` (`startHandoff`, `toastHandoffControllerUnavailable`, `toastHandoffAlreadyInFlight`, `toastHandoffStartFailed`, `toastHandoffStarted`) — fallback strings inline at the call sites so missing-locale degrade gracefully.
...
- **H10** (this PR) · `feat(agentic): H10 scenario + SQLite schema bump + plan revision (FINAL)` — closeout phase of the 21-phase plan. (1) New `Source_Core/src/Commands/Scenarios/AgentHandoffScenarioStep.cpp` drives the full Pending→Spawning→Running→PrOpen→Complete FSM against a scripted `ScenarioFakeRunner` (no real subprocess, no GitHub PAT, no HTTP). Registered via `ScenarioRunner::RegisterFactory("agent-handoff-roundtrip", ...)` in `AppController::Initialize` behind `#if defined(SMATCHET_WITH_AGENTIC)`; source-list-conditional in `CMakeLists.txt`. Asserts SnapshotAllForTests carries `state==Complete` + non-empty prUrl + exactly 4 HandoffStateTransition audit rows in the expected order. (2) SQLite schema `agent_proposals.schema_version` bumped v1 → v2 (`kCurrentSchemaVersion = 2`). New `agent_pr_watch` table persists `PrCommentWatcher` cursor + iteration count across restarts (was in-memory only in H7); PK is `proposal_id`. New `agent_proposals.handoff_status` column mirrors `CodingHarness::RunState` as a string for UI surfacing without joins. Migration body in `EnsureSchemaVersion` is idempotent — re-opening a v2 db is a no-op; an existing v1 db migrates in place via `ALTER TABLE ADD COLUMN` guarded by the `SqliteTableHasColumn` helper (mirrors LocalCacheManager pattern). (3) `PrCommentWatcher` gains `SetProposalStore` + `LoadCursorsFromStore` for cursor durability; `Tick()` persists `agent_pr_watch` after each successful dispatch and deletes the row on budget-exhausted transition (terminal cleanup). New public APIs on AgentProposalStore: `SetPrWatch` / `GetPrWatch` / `DeletePrWatch` for the watch row, `SetHandoffStatus` / `GetHandoffStatus` for the new column. (4) New CLI smoke `scripts/dev/test-agentic-handoff-scenario.sh` runs `scenario.run --name=agent-handoff-roundtrip --spawn` + asserts `passed=true` + the FSM transition trail. 6 new doctest cases extend `AgentProposalStore.test.cpp` (v1→v2 migration round-trip; agent_pr_watch Set/Get/Delete round-trip; agent_pr_watch persistence across re-open; handoff_status Set/Get round-trip; bumped kCurrentSchemaVersion==2 invariant guard; new cross-restart durability test). 3 new doctest cases in `PrCommentWatcher.test.cpp` cover the `SetProposalStore` seam + `LoadCursorsFromStore` no-op-on-no-handoffs + persistence round-trip across watcher teardown. End-to-end probe documented (morning manual residue per orchestrator stub-only directive). Lock-slug `agentic-flow` released by `.github/workflows/lock-cleanup.yml` on merge.
...
- **H9 — callback wired inside `GetAgenticHandoffController` (not in `InitAgentProposalStoreOnWorker`).** The proposal store is constructed on a worker thread before the handoff controller exists. Wiring the callback at store-init time would force the store to forward-declare the controller (or use a `std::function` global). Wiring it inside the controller's `call_once` block keeps the store free of handoff concerns AND guarantees both objects are alive before the first Pending→Approved edge can fire. The cost: callbacks registered before the first `GetAgenticHandoffController()` call never fire — but the only consumer is the handoff controller itself, so the precondition holds by construction.
- **H9 — single-subscriber callback (not a multi-listener vector).** YAGNI applies — the only consumer for the foreseeable future is the cross-flow handoff dispatcher. A vector + per-listener exception handling adds API surface for zero current benefit. The `OnApprovedFn` setter doc-comments the contract ("last setter wins"); upgrade to multi-listener when a second consumer materialises.
- **H9 — callback re-reads cfg on every fire (not at register time).** Capturing `cfg.HandoffAutoStartOnApprove` inside the registered lambda would freeze the auto-start gate at controller-construction time, making the Preferences checkbox a no-op until the next restart. `ConfigManager::Load()` returns a cheap value copy of the cached config so per-fire re-reads are sub-microsecond.
- **H9 — `[Start handoff]` button gating uses `app.GetAgenticHandoffController() != nullptr` (not a separate cfg check).** The handoff controller's nullptr path already encodes the precondition cascade (proposal store alive + PAT configured); duplicating that logic on the panel side would drift. The single guard subsumes "AGENTIC enabled" + "store ready" + "controller constructed" in one consistent check.
- **H9 — manual-button path emits its own audit row (`HandoffStarted` with `trigger=user-button`) BEFORE the worker dispatch.** The auto-start path mirrors the same pattern with `HandoffAutoStarted` + `trigger=approval-callback`. Both fire before `Start()` runs so the audit trail captures the user's intent even if `Start()` fails synchronously (precondition error, controller race). The two trigger literals are intentionally distinct so future audit queries can distinguish operator-clicked starts from cfg-driven auto-starts without parsing call-site context.
- **H9 — `[Start handoff]` race with auto-start surfaces as Info toast, not Error.** When auto-start is on AND the user clicks the button anyway (rare but legal), `Start()` returns `false` with `"already in flight"`. We surface that as a softer Info-level toast ("Handoff already in flight for this proposal") because the user's intent — implement this proposal — is being satisfied. An Error toast would mis-signal a failure where the system actually did the right thing.
- **H10 — `agent_pr_watch.last_seen_comment_id_str` carries unix-seconds, not GitHub's `id` integer.** The H7 `PrCommentWatcher` tracks comments by `createdAtSec`, not by GitHub's comment `id` (the watcher de-dupes via a monotonic cursor against the in-memory `prCommentCursorSec`). Persisting the wall-clock cursor as the `id_str` column keeps the schema generic for a future variant that switches to id-based dedup; the column name stays `last_seen_comment_id_str` so the migration body never has to rename the column.
- **H10 — `LoadCursorsFromStore` replays via `MarkHandoffIteration` rather than mutating internal state directly.** A direct mutation helper would bypass the FSM contract; reusing the existing public helper preserves the audit-trail symmetry (a hydrated handoff fires N audit rows on startup matching N persisted iterations). Trade-off: N audit rows on every restart per in-flight handoff. For the typical iteration budget (≤ 10) this is acceptable; if a future variant accumulates many iterations, a dedicated "hydrate-without-audit" helper can replace this loop. Documented inline.
- **H10 — `agent_pr_watch` row deleted on budget-exhausted transition only, not on Complete/Cancelled.** Complete/Cancelled paths route through different code (controller's `ControllerTransition` → terminal state); the watcher itself only sees PrOpen → budget-trip, so the cleanup hooks there. A future polish slice can add a controller-side `DeletePrWatch` call when the handoff terminates outside the watcher's tick — for H10 the orphan row is harmless (the next time the proposal id is reused the INSERT OR REPLACE overwrites; if the proposal id is never reused the row sits at zero cost).
- **H10 — handoff_status column writes are NOT wired in this PR.** The schema column exists; the `SetHandoffStatus` / `GetHandoffStatus` API is exposed. The AgenticHandoffController side that would call `SetHandoffStatus` after each FSM transition is deferred — H10 ships the schema + storage seam, and the UI panel that surfaces it can land as a separate UI-only polish slice. The column defaults to `''` (empty string) so any reader sees the documented "no handoff started" sentinel correctly.
- **H10 — sanitizer build is documented residue when the preset is missing.** Per plan decision #15 (stop conditions), `ninja-debug-msys2-asan` is the mandatory gate. If the preset is not configured on the agent's worktree the result is recorded as residue (mirroring T9's path); CI runs the preset on a real toolchain.
- **H10 — end-to-end probe is morning manual residue.** Per the orchestrator stub-only directive, the full end-to-end probe (real GitHub PAT + real `claude` install + real PR thread) does NOT run in this PR. Recipe documented in Verification § End-to-end probe so the next post-merge session can execute it once.
...
- **H9** (this PR):
  - `ninja-iter-msys2` dual-target build (`SmatchetStandalone` + `SmatchetCore_DX12`) green — both targets compile the extended `AgentProposalStore.cpp` + `AppController.cpp` + `SmatchetAgentProposalsUi.cpp` + `SmatchetPreferencesUi.cpp` cleanly.
  - `SMATCHET_WITH_AGENTIC=OFF` build of `SmatchetStandalone` green — the new H9 code paths are all inside `#if defined(SMATCHET_WITH_AGENTIC)` blocks (callback wiring in AppController, button in SmatchetAgentProposalsUi, checkbox in SmatchetPreferencesUi, cfg field in ConfigManager). The new `AgentProposalStore::SetOnProposalApproved` setter lives in the AGENTIC=OFF source-list-excluded TU and is not visible to OFF callers.
  - `ninja-test-msys2` ctest green including the 4 new `AgenticHandoffController.test.cpp` cases (non-ImplementIssue approval no-op; ImplementIssue + auto_start=false no-op; ImplementIssue + auto_start=true dispatches; manual button path appears in SnapshotActive).
  - `ninja-ui-test-msys2` bucket-E build green — registers 4 new tests under suite `AgentProposalsHandoffButton`. Standalone `bash scripts/dev/test-ui-agent-proposals-handoff-button.sh` invocation result documented in PR body (subject to the known `infra.md` P2 ephemeral-spawn timeout flake; CI retry-once policy mitigates).
  - `bash scripts/dev/test-ui-agent-proposals.sh` standalone PASS (4/4) — confirms the H9 button addition did not regress the existing T6 Approve / Reject paths.
  ...
  - `bash scripts/dev/test-agentic-approve-reject.sh` regression check PASS (T8 scenario intact — Pending→Approved transitions still work without firing handoffs in the no-callback default test path).
  - `bash scripts/dev/test-grid-edit-perf-postfix.sh` perf gate green.
  - `bash scripts/dev/test-all.sh` full sweep — non-bucket-E green; bucket-E failures (where present) are the known `infra.md` P2 environmental flake, not regressions introduced by this PR.
...
- **H10** (this PR — FINAL):
  - `ninja-test-msys2` ctest green including the new schema-migration + agent_pr_watch + handoff_status doctest cases in `AgentProposalStore.test.cpp` (v1→v2 migrate in place; agent_pr_watch round-trip; agent_pr_watch persistence across re-open; handoff_status round-trip; bumped `kCurrentSchemaVersion == 2` invariant guard) + the 3 new `PrCommentWatcher.test.cpp` cases (`SetProposalStore` is optional; `LoadCursorsFromStore` no-op-on-no-PrOpen-handoffs; persistence round-trip across watcher teardown).
  - `ninja-debug-msys2-asan` sanitizer build — gate result attached in PR body (preset-missing case documented as residue per plan decision #15; CI runs the preset on a real toolchain).
  - `ninja-ui-test-msys2` bucket-E rebuild — existing H8 + H9 panels continue to compile + drive cleanly (no new bucket-E variants this PR; the scenario is the canonical end-to-end gate).
  - `ninja-iter-msys2 --target SmatchetStandalone SmatchetCore_DX12` dual-target green.
  - `cmake -DSMATCHET_WITH_AGENTIC=OFF --build --preset ninja-iter-msys2 --target SmatchetStandalone` drift sentinel green — `AgentHandoffScenarioStep.cpp` excluded by `list(REMOVE_ITEM ...)`, factory call in `AppController` ifdef-wrapped, new `agent_pr_watch` table + `SetPrWatch` / `GetPrWatch` / `DeletePrWatch` / `SetHandoffStatus` / `GetHandoffStatus` API live in `AgentProposalStore.cpp` which is already AGENTIC-gated at the source list.
  - `Smatchet.exe cmd scenario.list` includes `agent-handoff-roundtrip`.
  - `Smatchet.exe cmd scenario.run --name=agent-handoff-roundtrip --spawn` → `passed=true`.
  - `bash scripts/dev/test-agentic-handoff-scenario.sh` PASS (new auto-enrolled smoke).
  - `bash scripts/dev/test-agentic-handoff-cli.sh` regression check PASS (H3/H4 surface intact).
  - `bash scripts/dev/test-agentic-handoff-clarification.sh` regression check PASS (H5 surface intact).
  - `bash scripts/dev/test-agentic-handoff-iterate.sh` regression check PASS (H7 surface intact).
  - `bash scripts/dev/test-agentic-approve-reject.sh` regression check PASS (T8 triage scenario intact).
  - `bash scripts/dev/test-agentic-triage-cli.sh` regression check PASS (T5 CLI surface intact).
  - `bash scripts/dev/test-grid-edit-perf-postfix.sh` perf gate green.
  - `bash scripts/dev/test-all.sh` full sweep — non-bucket-E green; bucket-E failures (where present) are the known `infra.md` P2 environmental flake, not regressions introduced by this PR.

### End-to-end probe (H10 — manual residue)

The full end-to-end probe runs once post-merge against a real `claude` install + real GitHub repo. NOT a CI gate; documented for the next post-merge session.

```text
1. Set cfg.GitHubPat in smatchet_config.json (real PAT with repo scope)
2. Set cfg.AgenticPollQuery = "alexandrosk0/test-repo" (real test repo)
3. Toggle Preferences > Agentic > Enable scheduled triage
4. Wait for poll → AgentProposalsUi shows real proposals
5. Approve one (ImplementIssue) → click [Start handoff]
6. Watch AgentHandoffsUi for Spawning → Running → PrOpen transition
7. Open the PR in browser, verify draft + branch name + body
8. Leave a comment → PrCommentWatcher picks it up + harness iterates
9. Mark PR ready-for-review, merge → handoff transitions to Complete
```

Pass criteria: every numbered step proceeds without operator intervention beyond what the step describes.
