...
- **Slice 3 override label is `cr-out-of-band` (new), not the plan's literal `tests-out-of-band`/`perf-out-of-band`.** Rationale: those two labels downgrade specific CI checks and are meaningless for the CR gate; the plan's intent ("honour the override-label *pattern* for consistency") is satisfied by a dedicated `cr-out-of-band` label using the same mechanism.
- **Slice 3 uses `jq` directly** (runner-side) rather than merge-gates.sh's jq-free "Option B" — the jq-free constraint exists only for the local Windows dev env; ubuntu-latest runners always ship jq.
- **Branch-protection required-check promotions (slices 1 & 3 final step) DEFERRED to post-merge.** A context can only be added to `required_status_checks` after its workflow is on `develop`. Sequenced: add `Pillar 2 scanner` after #518 merges; add `CR findings (0 actionable)` after #519 merges + its success-flip is verified on a live CR review. Safe because `enforce_admins` stays `false` (admin-reversible) throughout.
...
- **Slice 3 — PASS (live, pending-path).** `cr-finding-gate` jq program validated against sampled CR responses for the 0-actionable / N>0 / head-mismatch cases; on PR #519 the `CR findings (0 actionable)` StatusContext correctly posted `pending — awaiting CodeRabbit review on current head` on the initial run (no false-pass). Success-flip on CR's review-submitted event to be confirmed once CR completes its #519 review.
...
- **Slices 2 & 5 — DEFERRED (see Deviations).**
