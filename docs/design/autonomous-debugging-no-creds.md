...
Per user decision: reproducer-first contract (no fallback to "user repro steps" inside the debug loop). Aligned with the § Context "concrete enough description to reproduce" threshold.
...
- Update `agents/debug-detective.md` § Process step 2 (Reproduce) — replace the "user repro steps fallback" with a hard refusal: if no deterministic reproducer (CLI command / `scenario.run --name=<x>` / Lua snippet / failing-doctest name / registered bucket-E action) is supplied or discoverable, the agent's first action is to **add a scenario** that reproduces the bug (slice 5 pattern + new `Source_Core/src/Commands/Scenarios/<NewBugRepro>Scenario.cpp`). Scenario-add happens on the same branch as the fix; it's not a separate PR.
- Update `agents/debug-detective.md` § Self-improvement template to add a new optional category: "missing-scenario" — when the agent had to add a scenario before debugging, it records the scenario name + the bug category so the orchestrator's pattern-mining loop learns which surfaces lack coverage.
- Update `docs/agent-rules/delegation.md` § Debug-mode pause-loop — name the reproducer-first contract; add a new phase 0 (concreteness check) before the existing phase 1 (Clarify); document that phase 0's `AskUserQuestion` is the *only* user-input point in the loop.
