...
| 5 | 2026-05-12 · tracker-backend · tooling / new-agent — no test rig in the repo | medium effort | **Promote** — migrate existing plan from `~/.claude/plans/test-rig-agent-shy-margulis.md` to `docs/design/test-rig-agent.md` per AGENTS.md § Plan location; then execute per its 5-commit migration order. |
...
| 9 | 2026-05-13 · test-author · new-agent / tooling — bucket E (ImGui Test Engine) not wired | large effort | **Plan only** — write a dedicated `docs/design/imgui-test-engine-bucket-e.md` plan scoping FetchContent + `SmatchetUiTest` target + `tests/ui/` + `ui_test.run` CLI. Do not execute until a real bucket-E item arrives (per the backlog defer rule). |
...
```bash
git mv ~/.claude/plans/test-rig-agent-shy-margulis.md docs/design/test-rig-agent.md
```
...
Write a **new** `docs/design/imgui-test-engine-bucket-e.md` covering:
...
Per the migration order in `docs/design/test-rig-agent.md` — five sub-commits inside one session, finished with the lint-hook filter extension. **Defer to a dedicated session**; this umbrella plan only promotes the spec to canonical location.
...
2. **Commit B** (this plan executes immediately): copy `~/.claude/plans/test-rig-agent-shy-margulis.md` → `docs/design/test-rig-agent.md`; update backlog entry 5 path.
3. **Commit C** (this plan executes immediately): write `docs/design/imgui-test-engine-bucket-e.md` (scope-only); update backlog entry 9 path.
4. **Commit D** (separate session, follow `docs/design/test-rig-agent.md`'s own 5-commit migration order): execute test-rig.
