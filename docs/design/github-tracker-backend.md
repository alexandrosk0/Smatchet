...
- **Doc / AGENTS.md / agent-md / script / workflow / ADR cleanup of stale references** — bit-rot accepted per decision 1. **v2 follow-up plan**: [`agentic-ripout-doc-cleanup-v2.md`](agentic-ripout-doc-cleanup-v2.md) (stub; lands after v1 PR1 + PR2 merge). Covers: strip AGENTS.md § Handoff envelope (added by PR#248/#299/#300); delete `agents/handoff-implementer.md` + `pr-iterator.md` + `coderabbit-triage.md`; delete `docs/agentic/` + `docs/design/agentic-*.md` + `coderabbit-react-loop.md`; withdraw ADRs 0004 + 0005; delete `scripts/dev/test-agentic-*.sh` + `test-coderabbit-react.sh` + `test-ui-agent-*.sh`; audit `.github/workflows/`; trim AGENTS.md § Merge gates / § Autonomous ship-loop / § Post-ship § option 3 / § Force-push carve-out (grill-with-docs at v2 time decides aggressive vs surgical).
