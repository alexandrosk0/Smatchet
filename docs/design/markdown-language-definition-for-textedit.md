...
- Bucket D (sanitizer) — not run in this session; existing ASan/UBSan presets remain wired through `ninja-debug-msys2-asan`.
- Bucket E (ImGui Test Engine) — rig is wired (`docs/design/applied/imgui-test-engine-bucket-e-execution.md`); drag-select replay + Ctrl+C clipboard assertion remain the canonical bucket-E targets for Slice 4 — no test landed in this slice.
