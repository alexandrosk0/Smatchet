...
- **CR rate-limiting** — if multiple PRs go ready simultaneously (per AGENTS.md § Post-ship § option 3 ready-before-poll), CR may rate-limit + skip some. Already observed this session (#350 + #357). Mitigation: watcher staggers `gh pr ready` calls by 30s when batch-registering.
