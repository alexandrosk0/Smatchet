...
**Current state**: `agents/git-janitor.md` § FF-clean docs-batch exception precondition 4 requires `bash scripts/dev/test-all.sh` exit 0 before the FF push. For pure-docs diffs (single file under `docs/design/**`, no executable code) the gate adds ~3 min wall-clock with nothing to validate.
