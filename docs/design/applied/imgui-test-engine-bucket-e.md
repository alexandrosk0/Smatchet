...
- **Test-rig plan structure** (`docs/design/applied/test-rig-agent.md`) — this plan mirrors its CMake gating + sibling-directory layout + `test-*.sh` auto-enrolment.
...
- Every bucket-E item ships with: (a) its `tests/ui/<feature>.test.cpp`, (b) its `scripts/dev/test-ui-<feature>.sh` wrapper, (c) a plan-revision entry in the originating `docs/design/<slug>.md`.
