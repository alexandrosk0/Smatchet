...
foreach(_src IN LISTS _lua_sources)
    ...
    if(NOT _lang STREQUAL "CXX")
        message(FATAL_ERROR
            ...
            "(see docs/design/lua-recorded-cmd-list.md § Lua build mode).")
    endif()
endforeach()
```
...
- `perf.snapshot` measurement: **NOT RUN**. The harness for `scenario.run lua-cell-stress` was not invoked in this round — the build / link verification consumed the budget. Plan §Verification expects `RenderFieldCell lastTotalMs` to drop ~7 ms → <1 ms with 18 cached Lua cells; `LuaDrawList::Replay` < 5 µs / cell; `LuaDrawList::Record.calls` = 0 between events. Recommend a follow-up perf-measure agent invocation to fill in measured numbers per the post-impl revision banner at the top of this doc.
- **Fuzz scenario** (`scenarios/lua_recorder_fuzz.lua`): **NOT IMPLEMENTED** in this round. The recorder methods enforce the bounds documented in §Crash-safety hardening / Recorder-method input validation; a dedicated fuzz scenario remains a follow-up.
