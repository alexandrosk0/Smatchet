...
| Permission mode | **Full-auto, sandboxed by env + path discipline (NOT by an OS sandbox).** Harness spawned with `--permission-mode bypassPermissions`. Edits / Writes / Bash run without per-call user prompts. **Important — `bypassPermissions` is not an OS-level sandbox.** The child process can still read/write outside the worktree, make arbitrary network calls, and read inherited env. Blast-radius mitigations (all enforced **before phase 3**, not deferred — see also Q7 lifted here): (1) **env allow-list** — `ClaudeCodeLocalRunner` constructs a fresh `STARTUPINFO`/`environ` from a fixed allow-list `{PATH, HOME, USER, USERPROFILE, TEMP, TMP, SYSTEMROOT, GH_TOKEN, GITHUB_TOKEN, ANTHROPIC_API_KEY}`; **no other env vars inherit**, especially no `SMATCHET_*` vars that may carry secrets / PATs; (2) **branch is never `develop` / `main`** — runner asserts before `git worktree add`; (3) **PR is always `--draft`** and requires human merge; (4) **every tool-call audited** via stream-json → `BackendAuditTrail`; (5) **`--add-dir` not passed** — child's tool-access defaults to its cwd (the worktree); (6) **filesystem containment is by cwd + env, not by OS** — documented explicitly so future readers do not over-trust the boundary. Hardening upgrades (Windows Job Object, POSIX `prlimit` / namespaces, Bubblewrap) deferred but tracked as a follow-up. |
...
- Spawns: `claude --print --output-format stream-json --verbose --permission-mode bypassPermissions --append-system-prompt-file SEED.md "<positional user prompt: implement SEED.json>"`. (Verified against `claude --help` on the local install: `--print`, `--output-format stream-json`, `--permission-mode bypassPermissions`, `--append-system-prompt-file`, and the **positional `prompt` argument** are all in the current CLI surface; there is no `--prompt-file` flag. `--verbose` is required with `--print` + `stream-json` for the per-tool-call event stream to land in stdout. The SDK is under active development — phase-3 re-verifies against `claude --help` of the day and fails the runner with a clear error if any of these flags have been renamed.)
...
- Subscribes to `AgentProposalStore::OnProposalApproved` (new callback signal on the store, added in this plan). **Handoff-eligibility filter:** the controller spawns a harness **only** when `proposal.proposedAction == ImplementIssue` (the companion plan's new enum value). Approving any other action — `CommentAdd`, `LabelAdd`, `LabelRemove`, `AssigneeSet`, `StateTransition`, `DerivedTicketCreate` — fires the callback but the handoff controller no-ops on it (those are pure triage writes owned by `AgenticTriageController`). When `proposedAction == ImplementIssue` and `agentic.handoff.auto_start_on_approve` is true, the handoff auto-starts; when false, the user clicks `Start handoff` in the proposals panel. The same filter applies to the `[Start handoff]` button — it is rendered only on rows whose action is `ImplementIssue`.
...
```
The SQLite-side schema-version bump (for the new `agent_pr_watch` table + `agent_proposals.handoff_status` column) lands in **`AgentProposalStore.cpp`** (the SQLite owner), not in `ConfigManager.h`. The `cache_meta` `schema_version` row gets bumped + the new tables / columns added via the same `CREATE TABLE IF NOT EXISTS` + `SqliteTableHasColumn`-guarded `ALTER TABLE ADD COLUMN` idiom used by `LocalCacheManager.cpp:60-115`. Bump **held until phase 10** per AGENTS.md § "Schema-version bumps". The JSON config block above is additive and ships per-phase with no version gate (config fields default to the listed values when absent).
...
| 1 | `SubprocessCapture` lift from `P4Blame`. Pure helpers (argv quoting / env build). Doctest with **tiny checked-in C++ helper exes** (see Verification § "Subprocess doctest"), not POSIX shell builtins. Repoint `P4Blame.cpp` to use the new helper. Both targets compile. | `Source_Core/{include,src}/SubprocessCapture.{h,cpp}`, `Source_Core/{include,src}/SubprocessCapturePure.{h,cpp}`, `Source_Core/src/P4Blame.cpp` (rewired), `tests/_helpers/subproc_helper_exit.cpp` + `tests/_helpers/subproc_helper_sleep.cpp` + `tests/_helpers/subproc_helper_flood.cpp` (tiny test-only exes), `tests/CMakeLists.txt` (add the helper exe targets, depend on them from the test rig), `tests/Source_Core/SubprocessCapturePure.test.cpp`, `tests/Source_Core/SubprocessCapture.test.cpp` (auto-locates the helper exes via a configure-time `add_compile_definitions(SMATCHET_TEST_HELPER_DIR=...)`) | dual-target build + ctest + bucket-D screenshot diff of P4 blame view (replaces the manual P4 smoke) |
| 2 | `CodingHarnessTypes` + `ICodingHarnessRunner` + `CodingHarnessSeedBuilder` (pure: assemble SEED.md + SEED.json from `AgentProposal`). Doctest seed builder. **Plus** the new `agents/handoff-implementer.md` agent file + the new `AGENTS.md § Handoff envelope` section (matched to the SEED.json shape this phase defines). | `Source_Core/include/CodingHarnessTypes.h`, `Source_Core/include/ICodingHarnessRunner.h`, `Source_Core/{include,src}/CodingHarnessSeedBuilder.{h,cpp}`, `tests/Source_Core/CodingHarnessSeedBuilder.test.cpp`, `tests/Source_Core/HandoffAgentFrontmatter.test.cpp` (asserts the new agent file parses + `version: 1` + class = Implementer via the `AgentsMdLoader` parser), `agents/handoff-implementer.md`, `AGENTS.md` | build + ctest (frontmatter assertion in doctest, no manual review) |
| 3 | `ClaudeCodeLocalRunner` — spawn via `SubprocessCapture`, write SEED files, parse stream-json stdout, poll sentinel files, surface events via callbacks. Worktree creation via `git worktree add` (use `SubprocessCapture`). Stub `claude` is a tiny C++ exe (`tests/fixtures/stub-claude/stub_claude.cpp`) that emits canned stream-json + writes sentinel files (Bash stub rejected for cross-platform brittleness — see Verification). Env construction enforces the allow-list from § Decisions locked. | `Source_Core/{include,src}/ClaudeCodeLocalRunner.{h,cpp}`, `tests/fixtures/stub-claude/stub_claude.cpp`, `tests/fixtures/stub-claude/CMakeLists.txt`, `tests/Source_Core/ClaudeCodeLocalRunner.test.cpp` (also asserts the env allow-list: spawns the stub with a poisoned env containing `SMATCHET_SECRET=leak`, asserts the stub's recorded env file shows the secret is **absent**) | build + ctest + CLI smoke (`Smatchet.exe cmd handoff.dry-run …`) |
...
| 7 | `PrCommentWatcher` — poll-loop thread, dedupe via SQLite cursor, dispatch `runner.Resume(...)`. Budget enforcement (`pr_iteration_budget` — abort after N iterations). **Plus** the new `agents/pr-iterator.md` agent file (lands alongside the watcher it pairs with). **Plus** the full `GitHubClient` PR-surface extension — `FetchPrComments(prNumber)`, `CreatePullRequest(...)`, **and the four CI-react methods folded in from `docs/design/coderabbit-react-loop.md` so the `GitHubClient` HTTP surface arrives in one PR**: `FetchCheckRuns(headSha)` → `GET /repos/{o}/{r}/commits/{sha}/check-runs`, `FetchCheckRunAnnotations(checkRunId)` → `GET /repos/{o}/{r}/check-runs/{id}/annotations`, `FetchActionsJobLogs(jobId, tailLines)` → `GET /repos/{o}/{r}/actions/jobs/{id}/logs` (302 → plain text, tail the response after redirect), `RerunWorkflowRun(runId)` → `POST /repos/{o}/{r}/actions/runs/{id}/rerun`. All six follow the existing `TrackerHttpRequestWithRetry` + `BackendAuditTrail` pattern that `GitHubClient`'s existing methods use; no new HTTP machinery, just new endpoints. `PrCommentWatcher` itself only consumes `FetchPrComments`; the four check-run methods land here for `GitHubClient` cohesion and are consumed downstream by `coderabbit-react-loop`'s `PrCheckRunWatcher`. | `Source_Core/{include,src}/PrCommentWatcher.{h,cpp}`, `Source_Core/src/AgenticHandoffController.cpp` (own the watcher), `Source_Core/src/GitHubClient.cpp` + `Source_Core/include/GitHubClient.h` (six new methods), `tests/Source_Core/PrCommentWatcher.test.cpp` (with recorded comment fixtures), `tests/Source_Core/GitHubClient_PrSurface.test.cpp` (new — covers all six new methods against recorded HTTP fixtures), `tests/Source_Core/PrIteratorAgentFrontmatter.test.cpp` (asserts the new agent file parses + `version: 1` + class = Maintenance), `tests/fixtures/github_check_runs_sample.json`, `tests/fixtures/github_check_run_annotations_sample.json`, `tests/fixtures/github_actions_job_logs_sample.txt`, `agents/pr-iterator.md`, CLI smoke `test-agentic-handoff-iterate.sh` | build + ctest + CLI smoke (frontmatter assertion in doctest, no manual review) |
...
| 9 | Cross-flow wiring: when companion plan's `SmatchetAgentProposalsUi` calls `agent.proposal.approve <id>`, the proposal store's `OnProposalApproved` callback (added here) fires and (`proposedAction == ImplementIssue` + `auto_start_on_approve`-gated) auto-starts a handoff. Adds a `[Start handoff]` button on the proposals row for the manual path — rendered **only** on rows whose action is `ImplementIssue`. | `Source_Core/src/AgentProposalStore.cpp` (callback signal + `ImplementIssue`-filter assertion in the test fixture), `Source_Core/src/SmatchetAgentProposalsUi.cpp` (conditional button), `Source_Core/src/AgenticHandoffController.cpp` (subscribe + filter), `tests/Source_Core/AgenticHandoffController.test.cpp` (asserts non-`ImplementIssue` approvals do not start a handoff), `tests/ui/agent_proposals_handoff_button.test.cpp` (bucket-E: button visible only on `ImplementIssue` rows) | build + ctest + bucket-E |
| 10 | Scenario step + recorded fixtures for replay regression. Schema-version bump (SQLite: new `agent_pr_watch` table + `agent_proposals.handoff_status` column — migration in the existing store via `LocalCacheManager.cpp:60-115`-style `CREATE TABLE IF NOT EXISTS` + `SqliteTableHasColumn` guarded `ALTER TABLE ADD COLUMN`, with a `cache_meta` `schema_version` row bump). Plan-revision sections appended (`## Implementation log`, `## Deviations from plan`, `## Verification`). | `Source_Core/src/Commands/Scenarios/AgentHandoffScenarioStep.cpp`, `tests/fixtures/handoff_seed_sample.json`, `tests/fixtures/pr_comments_sample.json`, `Source_Core/src/AgentProposalStore.cpp` (new table + ADD COLUMN + cache_meta bump — owned by the **store**, not ConfigManager; the `handoff.*` JSON block in `ConfigManager.h` already shipped per phase 6 as additive fields, no version gate), `docs/design/agentic-coding-handoff.md` (revisions) | full regression: `scripts/dev/test-all.sh` + dual-target build + screenshot diff + sanitizer build |
...
- **Subprocess doctest** (real `SubprocessCapture` against tiny **checked-in C++ helper exes** — chosen over POSIX shell builtins for cross-platform determinism under MSYS2 UCRT64 + future MSVC):
  - `tests/_helpers/subproc_helper_exit.cpp` — `int main(int argc, char**) { return argc >= 2 ? std::atoi(argv[1]) : 0; }`. Drives the exit-code branches (`./subproc_helper_exit 0` for the success case, `./subproc_helper_exit 1` for the failure case). Replaces `echo`/`false`.
  - `tests/_helpers/subproc_helper_sleep.cpp` — `std::this_thread::sleep_for(std::chrono::milliseconds(std::atoi(argv[1])))`. Drives the timeout branch (`./subproc_helper_sleep 5000` with `timeoutMs = 100`). Replaces `sleep`.
  - `tests/_helpers/subproc_helper_flood.cpp` — writes N bytes of `'A'` to stdout, where N is `std::atoll(argv[1])`. Drives the byte-cap-truncation branch (`./subproc_helper_flood 1000000` with `byteCap = 1024`). Replaces `printf "$(seq ...)"`.
  - Helper exes built by `tests/CMakeLists.txt` and located via a configure-time `target_compile_definitions(SmatchetTests PRIVATE SMATCHET_TEST_HELPER_DIR="...")`. The test reads the dir + appends `.exe` on Windows; one branch, no shell.
  - Cancel/kill branch — covered by `SubprocessCapture::Cancel(cancel atomic)` raised mid-sleep against `subproc_helper_sleep`; assertion is `exitCode != 0 && durationMs < requested-sleep`.
- **Stub-runner end-to-end** (tiny C++ exe masquerading as `claude` — same cross-platform reason; no Bash dependency):
  - `tests/fixtures/stub-claude/stub_claude.cpp` — reads `--print` / `--output-format` / `--append-system-prompt-file` flags + positional prompt; emits canned stream-json events to stdout; writes `RUN_RESULT.json` to cwd; exits 0. Used by `ClaudeCodeLocalRunner.test.cpp` to exercise the full state machine without touching the real CLI. Built as a CMake exe target under `tests/fixtures/stub-claude/CMakeLists.txt`; runner finds it via the same `SMATCHET_TEST_HELPER_DIR` mechanism.
...
- **ImGui Test Engine (bucket E, wired)** per `agents/test-author.md` § Bucket E — `ninja-ui-test-msys2` preset, `SMATCHET_BUILD_UI_TESTS=ON`, registration via `IM_REGISTER_TEST` + `SmatchetRegisterAllUiTests`:
  - `tests/ui/agent_handoff_panel.test.cpp` — three test cases driven against a `HarnessRunState` fixture pre-seeded into the stub runner: (1) `AwaitingUser` row's `Submit clarification` button writes `USER_RESPONSE.json` + transitions to `Running`; (2) `Running` row's `Cancel` button transitions to `Cancelled` + raises the cancel atomic the stub watches; (3) `PrOpen` row's `Open PR` button records a `ShellExecuteW` call against the in-memory launch-recorder. Bash driver at `scripts/dev/test-ui-agent-handoff.sh` (auto-enrolled in `test-all.sh`). No manual residue.
...
Steps 4-7 are bucket-E covered (`scripts/dev/test-ui-agent-handoff.sh`) + CLI-smoke covered (clarification + iterate scripts above). Step 8 (merge → `Complete`) is a one-shot end-to-end probe — bucket-E covers the in-app state-change observer; the actual GitHub merge requires a live PR and is not a recurring gate.
...
| GitHub HTTP (PR-create + comment-poll + check-run-poll + workflow-rerun) | `GitHubClient` (`AddIssueCommentPlain` from `agentic-triage-flow` T2; this plan H7 adds `CreatePullRequest`, `FetchPrComments`, `FetchCheckRuns`, `FetchCheckRunAnnotations`, `FetchActionsJobLogs`, `RerunWorkflowRun` — the four check-run methods folded in from `docs/design/coderabbit-react-loop.md` so the surface arrives cohesive) |
...
3. **`claude` CLI exact flag set.** Verified against the local install: `claude --print --output-format stream-json --verbose --permission-mode bypassPermissions --append-system-prompt-file SEED.md "<positional prompt>"`. There is **no** `--prompt-file` flag — the user prompt is a positional argument (or piped via stdin). Phase 3 re-verifies against the SDK CLI of the day; if any flag has changed, the runner is the single place to fix.
