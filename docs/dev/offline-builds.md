...
> Slice 6 of [docs/plans/shipped/first-time-setup-hardening.md](../design/archive/first-time-setup-hardening.md). Niche audience — contributors behind restrictive firewalls, corporate proxies that block `https://github.com`, air-gapped CI runners, or first-time setups where re-downloading every dependency on every fresh clone is painful.
