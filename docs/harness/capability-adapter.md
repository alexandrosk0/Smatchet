...
|---|---|---|---|---|---|---|
| `semantic-code-search` | Sourcetrail `tools/sourcetrail/st_query.py` (call-graph/def) + `Grep` | `rg` fallback | (built-in search panel) | (not built-in — fall back to text-search) | `grep` (fallback) | `rg` over symbol set |
| `file-skeleton` | targeted `Read` / `st_query.py file <path>` | targeted file read | — | — | `read` (fallback) | `ctags -x <file>` |
