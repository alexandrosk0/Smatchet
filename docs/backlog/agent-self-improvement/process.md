...
- 2026-05-18 · orchestrator · [process] · P3 — Agent-docs improvements: 4 parked items deferred from PR #260
  Details: PR #260 ([`docs/design/agent-docs-improvements.md`](../../design/agent-docs-improvements.md)) shipped 5 P2/P3 actions (ship-loop default, post-ship protocol, security AI + handoff surface, worktree-spawn investigation, tooling.md triage). Four items from the original 8-item feedback were parked to avoid doc-restructure churn in the same change set. Listing them here as a single tracking entry so the evidence trail survives the plan's merge.
  Concrete next actions (separable, ship independently when adjacent work surfaces):
    1. **AGENTS.md three-way split** — AGENTS.md is 514 lines; the § Orchestrator delegation packet sub-section alone is ~120 lines. Proposed split: AGENTS.md (index + delegation routing) + `docs/agent-rules/PILLARS.md` + `docs/agent-rules/DELEGATION.md` + `docs/agent-rules/HANDOFF.md`. Cost ~1.5 h pure copy/paste with cross-link rewrites. Risk: harness adapter symlinks in `.claude/` need verification after the split. Trigger: next time AGENTS.md is edited.
    2. **CONTEXT.md glossary expansion (9 → ~32 entries)** — current glossary covers Agentic flow only. Missing: UX Pillars terminology (frame budget, Pillar 1-4), perf-monitor concepts (p99, sustained, steady-state, spike, `SMATCHET_UI_PERF_SCOPE`), plugin architecture (MCP server lifecycle, sandbox, sol2 bindings, gate macros), test taxonomy (Bucket A-E + fixture + scenario + golden image), harness concepts (coding harness runner, worktree, sentinel files, vexp pipeline, harness adapter). Cost ~1 h, definitions need writing, not copying.
    3. **Lock terminology glossary clarification** — `plan-lock` (187 hits) / `lock-claim` (69) / `lock-slug` (31) / `holds-lock` (4) are role-distinct, not synonyms. Add one paragraph to `docs/design/git-ref-plan-locks.md` naming the four roles. Bundle with #2 above. Cost ~10 min.
    4. **vexp section → `.claude/CLAUDE.md`** — AGENTS.md L482-514 vexp block is harness-specific; should live in `.claude/CLAUDE.md`. Already filed as external-blocker (vexp installer auto-regenerates the block on update). Re-evaluate when vexp upstream ships a per-project opt-out marker.
  Status: open
