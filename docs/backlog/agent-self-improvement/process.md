...
- 2026-05-20 · orchestrator · [process] · P2 — New-feature plans must include a `## Perf` section with before/after measurement + temp markers
  Details: PR #311 (markdown LD + plan-doc viewer + selectable rich preview, 5 slices) shipped without any perf consideration at plan time. User reported FPS drops on the AI chat panel post-merge. `perf-detective` then ran a reactive investigation; PR #313 followed with five perf optimisations (plan cache, font hoist, vector pre-alloc, off-screen culling, direct-AddText refactor) hitting a ~93 % reduction on the rich-render hot path. Total agent-time cost: ~3 hours of perf-detective + perf-instrument + perf-measure cycles + a full follow-up PR + reactive user feedback round-trips. If PR #311's plan had even a minimal `## Perf` section (baseline measurement against the closest existing scenario + a projected delta), the issue would have been caught at plan time and the optimisations would have shipped alongside the feature. Pillar 1's 6.94 ms budget is mentioned in AGENTS.md but enforcement is REACTIVE — `perf-detective` auto-fails commits that lift sustained mean above budget, but only when someone notices and asks. Plans default to "no perf consideration" because no rule requires one.
  Concrete next action: extend AGENTS.md § Project rules with a **"Plan § Perf required"** rule. Every `docs/design/<slug>.md` for a feature touching `Source_Core/` (or any path that compiles into `SmatchetStandalone`) MUST include a `## Perf` section containing: (1) **Baseline** — closest existing `perf-measure` scenario name + numbers from a pre-implementation snapshot, OR an explicit "no existing scenario covers this path; new scenario required" line; (2) **Projected impact** — qualitative ("expected to add ~0.X ms / frame in path Y") + the per-frame budget headroom remaining; (3) **Required markers** — list of `SMATCHET_UI_PERF_SCOPE("perf_temp:…")` to add during implementation, named per `agents/perf-instrument.md` conventions; (4) **Post-slice verification** — `perf-measure` delta vs baseline, asserted under the 6.94 ms steady-state + 16.67 ms p99 floors; (5) **Marker cleanup** — `perf-instrument` strip step at slice close. Exemptions: pure-docs slices (the existing pure-docs-skip envelope covers these), trivial-visual-only envelope (palette/locale-only changes don't shift perf), small-mechanical changes (renames, lint fixes). Stretch: update `agents/_shared/skills/grill-with-docs/SMATCHET-NOTES.md` to challenge plans missing the `## Perf` section so the grilling pass catches the gap before the plan is locked. ~30 min for the AGENTS.md rule edit + ~30 min for the grill-with-docs check + ~30 min for a doc template update (`docs/design/_template.md` if one exists, or create one). Payoff: every new-feature plan ships with the perf cost known and bounded — no more reactive PR #313-shaped follow-ups.
  Status: open
  Last-reviewed: 2026-05-20
