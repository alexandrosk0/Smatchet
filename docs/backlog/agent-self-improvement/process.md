...
- 2026-05-19 · orchestrator · [process] · P3 — `link_file()` short-circuits forever when destination exists; canonical edits don't propagate to `.claude/hooks/`
  Details: `scripts/setup-harness.sh` L77-79 returns 0 without action if `[[ -e "$link" ]]`. On Windows, `link_file()` falls back from `mklink` to `mklink //H` (hard link). When both fail (no admin / Developer Mode disabled), a plain copy may persist instead. Discovered during PR #266: `.claude/hooks/agent-token-log.py` (inode 101893941569706940) and `agents/_shared/token-tracking/agent-token-log.py` (inode 8162774325058581) had byte-identical content but DIFFERENT inodes — not a hard link. Editing the canonical didn't propagate to the hook copy. Manual `cp -f` mirror was the fix. Same gotcha applies to any `link_file` call in setup-harness.sh; the harness silently degrades to drift-prone state.
  Concrete next action: either (a) make `link_file()` detect drift — `cmp -s "$link" "$ROOT/$target" || (rm "$link" && re-link)`, OR (b) document the dual-edit discipline in `agents/_shared/token-tracking/README.md` + add an audit check to `scripts/dev/test-agent-contract.sh` that diffs the canonical against the hook copy and fails loudly on drift, OR (c) add `--force-relink` flag to `setup-harness.sh` for explicit refresh. Option (b) is the lowest-risk + catches the bug at PR time; (a) is the cleanest fix but touches setup-harness shape. ~30 min for (b).
  Status: open
  Last-reviewed: 2026-05-19

- 2026-05-19 · orchestrator · [process] · P3 — AGENTS.md § <section> cross-link rot when sections move
  Details: PR #273 (AGENTS.md § Delegation extraction) found ~74 external references of shape `AGENTS.md § <subsection>` across the repo (agents/*.md, docs/design/*, scripts, source headers, tests). Moving § Delegation to `docs/agent-rules/DELEGATION.md` would have left all 74 dangling — resolved via redirect-stub pattern (AGENTS.md keeps the anchor + names the moved subsections; new doc has canonical text). Pattern works but is convention-only; no automated check verifies that every `AGENTS.md § X` reference matches a real section name. A typo or section rename silently breaks the cross-link.
  Concrete next action: add a `scripts/dev/test-doc-anchors.sh` (bucket-A) that greps every `AGENTS.md § <name>` reference across the repo and verifies the named section exists either in AGENTS.md or in a doc that AGENTS.md links to (e.g. `docs/agent-rules/*.md`). Auto-enrol via the `test-*.sh` glob. Catches stale anchors before merge. ~1 h.
  Status: open
  Last-reviewed: 2026-05-19
