...
- 2026-05-28 · orchestrator · [process] · P1 — Implementer-side self-review didn't catch real shell-script bugs CR found in 4 of 9 slices this session
  Details: Slices 5, 6, 8, 10 shipped shell scripts (`p4-git-sync-check.sh`, `coverage.sh`, `git-janitor.sh`, `clear-session-context.sh`) that CR flagged with genuine non-stylistic bugs: (a) `python` dependency without preflight check in `git-janitor.sh` (script's own preflight only verified gh + cmake; python missing on python3-only hosts produced silent empty PR_STATE and incorrect "expected MERGED" refusal); (b) basename-based heuristic against `.git/worktrees/<basename>/` in `agents/git-janitor.md` Step 0 (gitdir id can diverge from working-dir basename — false-flags real worktrees as orphan); (c) unquoted `$git_not_p4` / `$p4_not_git` in `printf` (word-splitting / glob expansion on paths with spaces); (d) `curl -sSL` on Font Awesome TTF with mutable `6.x` branch and no checksum (supply-chain risk; 404 HTML page lands as the "font"); (e) `--threshold=VALUE` form bypassed validation while `--threshold VALUE` had it (asymmetric arg parsing). Net: ~6 separate CR re-review cycles consumed across the 9 slices, ~30 min each. All findings legitimate; none caught by my pre-push read-through. The pattern: I author shell scripts focused on the happy path, CR catches the edge cases the project's other shell scripts already handle.
  Concrete next action: ship `scripts/dev/lint-shell-self-review.sh` that runs before any `scripts/dev/*.sh` push and asserts: (1) every external command used is named in the script's preflight `command -v` checks; (2) `shellcheck` clean (catches unquoted expansion + many footguns); (3) `curl` invocations use `-f`; (4) network downloads have a sha256 verify line; (5) `--key=value` and `--key value` arg forms both validate. Wire into the pre-push lint hook. Plus a § Shell-script self-review checklist in `agents/_shared/skills/code-review/` (or similar) the implementer reviews before pushing. ~3 h: 2 h script + 30 min hook + 30 min doc. Wins on every shell-script PR going forward (Smatchet ships ~1-2 per week).
  Status: open
...
- 2026-05-28 · orchestrator · [process] · P2 — Pre-implementation triage caught 5 already-resolved backlog items; encode as a slice-start rule
  Details: `docs/design/tooling-process-backlog-sweep.md` budgeted slices for 5 items that turned out to be already-resolved on develop when the slice agent inspected the code: GH_API_DOWN exit-code mapping (`merge-watcher.py:328-335`), triage-budget per-HEAD reset (PR #418 merged 2026-05-23), STALE_WITH_FINDINGS / STALE_CLEAN / STALE_RESOLVED discrimination (7 occurrences in `merge-gates.sh`), UnicodeEncodeError `sys.stdout.reconfigure` (`merge-watcher.py:51`), 8-of-15 perf scenarios — cell-edit-burst + whisper-ai-assistant-autosend already had `rows[]` in `OnFinish`. Each item the plan called for ~30 min - 1 h of work; the pre-implementation triage (grep / code-read) took 5 min and saved ~3 h total. The plan-doc captured this triage explicitly in § Approach, which worked as a forcing function — without it, the implementer would have written redundant code.
  Concrete next action: add a § Pre-implementation triage rule to `docs/agent-rules/process-rules.md` § Plan-doc family: "For any plan item that says 'fix existing tooling' or 'extend X', the slice agent's first action is to grep / Read the code AND check git history (`git log --oneline -- <file>` + applied.md for prior fixes). If the fix has already shipped, archive the backlog entry and skip the slice — don't reimplement. Plan-doc § Approach should pre-flight this for items known to overlap recent work." ~10 min doc edit. Wins on every multi-slice plan touching ageing backlog.
  Status: open
  Last-reviewed: 2026-05-28
