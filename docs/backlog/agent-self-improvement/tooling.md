...
- 2026-05-28 · orchestrator · [tooling] · P1 — `smatchet-merge-watcher` autostart daemon exits `EXIT_127` every poll (can't launch `merge-gates.sh`); closed PRs never auto-unregister
  Details: Surfaced executing post-ship option 3 for PR #512 this session. `merge-watch status` showed 17 registered PRs (#372–#413) all stuck in state `EXIT_127` with one clustered `LAST_POLL` and no daemon currently running — the last autostart daemon run failed to launch its subprocess for every PR. `EXIT_127` = "command not found": `scripts/dev/merge-watcher.py` spawns `merge-gates.sh` via `BASH_BIN` (merge-watcher.py:296) and `gh` via `GH_BIN` (:206,:454), but the autostart wrapper (`scripts/dev/merge-watcher-install-autostart.ps1` Scheduled Task) runs without `bash`/`gh` on PATH, so every poll exits 127 and nothing ever merges. The same daemon started cleanly from an interactive shell (bash/gh/python all resolve), confirming env/PATH in the wrapper, not daemon logic (the 2026-05-21 GH_API_DOWN P1 is separately fixed: owner/repo + ORCH_USER now resolved at merge-watcher.py:160,203). Secondary: those 17 PRs are all closed/merged (none in `gh pr list --state open`) yet stay registered — the daemon doesn't unregister on a merge-gates "closed" result (exit 4), so the registry accretes dead entries that each burn a graphql poll per cycle. Net: post-ship-option-3 off-loop auto-merge is non-functional on the autostart path until the wrapper PATH is fixed.
  Concrete next action: (1) in `merge-watcher-install-autostart.ps1`, resolve absolute `bash.exe` (Git\usr\bin) + `gh.exe` and pass them as `BASH_BIN` / `GH_BIN` env to the daemon (already honored) or prepend to the task PATH; also export `PYTHONIOENCODING=utf-8` (the 2026-05-22 entry's still-open autostart note). (2) on merge-gates exit 4 (closed-externally), auto-unregister the PR so the registry self-cleans. (3) one-off: unregister the 17 stale closed PRs. Add a bats test that runs the daemon with an empty PATH + a fixture PR and asserts it emits a clear "bash not found" error instead of silently looping `EXIT_127`. ~2 h.
  Status: open
...
- 2026-05-20 · orchestrator · [tooling] · P2 — Bucket-E coverage for Preferences > Agentic tab (T7 residue)
  Promoted from parked: 2026-05-18 — bucket-E (ImGui Test Engine) is wired per `docs/design/archive/imgui-test-engine-bucket-e-execution.md`; gating premise removed.
