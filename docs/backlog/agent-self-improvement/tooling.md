...
- 2026-05-22 · orchestrator · [tooling] · P3 — retype change @2 revs to `text+w` so the dual-VCS ReadOnly contract holds across the whole baseline history
  Details: Phase 1 of `docs/design/git-to-perforce-migration.md` submitted change @2 (the 820-file baseline import) under bare `text` types in the typemap, which set Windows ReadOnly on every text rev. The typemap was subsequently corrected (`text+w` / `text+wx` for every text/script type) and the live working tree's ReadOnly bits were cleared via bulk `attrib -R /S /D`. Existing revs of change @2 still carry the old types; any `p4 sync` against those revs to a fresh client root would re-apply ReadOnly. Phases 2-7 shipped against the fixed typemap and have not yet hit this — but the next fresh-client sync (e.g. a second contributor onboarding) would surface the regression.
