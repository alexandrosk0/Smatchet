...
- 2026-05-28 · orchestrator · [tooling] · P2 — Backlog files conflict on every parallel-slice merge; entries are independent but the linear file structure forces manual resolution
  ...
  Concrete next action: two options. **(a) `.gitattributes` union merge driver** (cheap): add `docs/backlog/agent-self-improvement/applied.md merge=union` so git auto-concatenates both sides. Trade-off: doesn't preserve date order (the union may interleave); needs a post-merge `sort -r -k1` step or a periodic cleanup. **(b) One-entry-per-file under `applied.md.d/<date>-<slug>.md`** (correct): split the archive into per-entry files; concatenate into the rendered `applied.md` via a `make applied` script. Adds one extra file per archive but zero conflicts. ~30 min for (a), ~3 h for (b) + tooling. Recommend (a) ship now, (b) defer until conflict pain returns. Wins on every multi-slice plan (the tooling-process-backlog-sweep proves the pattern recurs).
...
- 2026-05-28 · orchestrator · [tooling] · P2 — No `vcvars64.bat` wrapper for bash sessions; C++ slices ship without local build verification
  ...
  Concrete next action: ship `scripts/dev/with-msvc-env.sh` that detects vcvars64.bat (standard VS install paths), sources its env into a sub-shell via `cmd.exe /c "vcvars64.bat && set" | grep "^[A-Z_]*=" | export-it`, then `exec "$@"` so `bash scripts/dev/with-msvc-env.sh cmake --build --preset ninja-iter-msvc --target SmatchetStandalone` just works. Document in BUILD.md. ~1 h. Wins on every bash-side C++ touch.
