...
- 2026-05-20 · orchestrator · [tooling] · P2 — Required CLI tools must be discoverable + verified at first-setup time
  Details: Two CLI tools required by the orchestrator's standard ship-loop bit this session and weren't documented anywhere agents would find before reaching for them. (1) **`jq`** — entirely missing from PATH. The merge-gates poller `scripts/dev/merge-gates.sh` calls `jq` directly on the GraphQL response; without it the script silently returned exit 0 with no measurements ("PR_\n" was the only output) and falsely indicated gate-pass on PR #311. Recovered by switching to `gh --jq` (built-in jq subset in `gh`) for the manual poll, but the canonical poller stays broken. (2) **`gh`** — installed but not on the bash-tool PATH; required `export PATH="$PATH:/c/Program Files/GitHub CLI"` prepended to every `gh` invocation. Cost: 4 separate prefix-the-PATH operations across the session + one false GATE_OK + ~5 min of debugging the poller's empty output. Both are listed nowhere in `docs/harness/SETUP.md` as required dependencies.
  Concrete next action: (1) add a **"Required CLI tools"** section to `docs/harness/SETUP.md` (or `AGENTS.md` § Harness adapter) listing the canonical tool set with install instructions per platform: `jq` (MSYS2 `pacman -S jq` or `mingw-w64-ucrt-x86_64-jq`), `gh` (GitHub CLI installer + PATH note), `cmake`, `ninja`, `gcc`/`g++` (MSYS2 UCRT64), `python` (for some scripts), `clang-format`/`clang-tidy`/`cppcheck` (lint hooks), `flock` (used by `lint-cpp-drain.sh`). (2) Add `scripts/dev/check-required-tools.sh` that runs `command -v <tool>` for each + prints a one-line PASS/FAIL summary. Wire it as the first step of `scripts/setup-harness.sh <name>` so the missing-tool warning fires at clone time. (3) Optionally have the lint-cpp-common.sh-style PATH-prepend pattern (`SMATCHET_TOOLCHAIN_BIN=/c/msys64/ucrt64/bin` prepended idempotently) extended to also probe `/c/Program Files/GitHub CLI` for `gh` so agents don't have to manually prefix. ~1 h for the doc + check script; ~30 min for the PATH-prepend helper. Payoff: zero "missing tool" surprises mid-session; agents observe required-tool availability up front rather than discovering gaps reactively at high-stakes moments (merge-gates poll, lint flush, etc.).
  Status: open
  Last-reviewed: 2026-05-20
