...
| **Norton Commander bucket-E live-apply test** — Was authored on PR #91 then removed during the teal-palette refinement because its assertions hard-coded the old blue palette and would break on any future palette tweak. Re-author against the shipped palette literals only when the palette stops moving. New file: `tests/ui/norton_commander_theme.test.cpp` (registers `Theme/NortonCommander_LiveApply` via `ImGuiTestEngine`, asserts `ImGui::GetStyle().Colors[WindowBg]` = teal, `Text` = yellow, `Header` = blue, `Border` = white). Also re-add `scripts/dev/test-norton-theme.sh` driver + the entry in `tests/ui/CMakeLists.txt` source list + the `extern "C"` line in `tests/ui/ui_tests_registry.cpp`. Doctest in `tests/Source_Core/SmatchetThemeSyntaxColors.test.cpp` already covers the C++ syntax palette side-channel — the bucket-E gap is only the live ImGui chrome path. | `tests/ui/`, `scripts/dev/test-norton-theme.sh` | Next PR after the Norton palette is visually validated by the user and locked. Estimated cost: 30 min (mostly carrying the deleted code back from git history `git show 5c299f7^..5c299f7 -- tests/ui scripts/dev/test-norton-theme.sh`). |
