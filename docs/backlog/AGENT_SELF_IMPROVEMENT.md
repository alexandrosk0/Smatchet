...
- 2026-05-16 · offline-sync · [infra] — Phase 3 `OfflineQueueServiceRuntime.test.cpp` + `TicketSyncService.test.cpp` deferred
  ...
  Status: open. Blocks: Phase 3 hostile-fixture cases ("queue replay with 401 stays in queue, no dead-letter on auth failure", "ApplyIssueFetchPack empty fetch in full-sync mode must reject not delete"). Mid-tx-throw inside `SaveTicket` partially covered by the LocalCacheManager tests already shipped — SQLite RAII transactions roll back cleanly on throw, but full hostile-fixture wiring (throw from inside a per-row insert) wants a SQLite-level fault-injection layer not in scope here.
...
- 2026-05-16 · offline-sync · [test] — Phase 3 `BackendAuditTrail` async-writer is a process-wide singleton; tests can only redirect the audit file via `ConfigManager::SetUserDataDirectory` BEFORE the first AppendEvent runs in the process
  ...
  Status: open. Workaround in place — not a Phase 3 blocker.
