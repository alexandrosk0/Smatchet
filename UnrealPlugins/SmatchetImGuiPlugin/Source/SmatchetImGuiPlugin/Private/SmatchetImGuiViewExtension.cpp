...
FSmatchetImGuiViewExtension::FSmatchetImGuiViewExtension(const FAutoRegister& AutoRegister,
                                                         SmatchetImGuiHostHandle InHost)
...
void FSmatchetImGuiViewExtension::BeginRenderViewFamily(FSceneViewFamily& InViewFamily) {
...
#else
        UE_LOG(LogSmatchetImGuiViewExtension, Warning,
               TEXT("Render integration active on unsupported/untested UE version path."));
#endif
    ...
    if (bUiVisible) {
        ...
        if (Now - LastUiStateLogSeconds > 2.0) {
            ...
            UE_LOG(LogSmatchetImGuiViewExtension, Log,
                   TEXT("Host state: UiVisible=%d Initialized=%d ImGuiDisplaySize=(%d,%d)"), bUiVisible ? 1 : 0,
                   bInitialized ? 1 : 0, Size.X, Size.Y);
