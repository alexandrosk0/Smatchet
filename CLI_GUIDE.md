...
| `view.toggle.<id>` | — | Toggle a side-bar / panel window. `<id>` is one of `views_dashboard`, `source_blame`, `log`, `backend_audit`, `performance`, `bulk_import`, `bulk_export`, `preferences`, `mcp_server`, `scripts`. Returns `{open: bool}`. |
