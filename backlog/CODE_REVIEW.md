...
62. ✅ **DONE (PR [#40](https://github.com/alexandrosk0/Smatchet/pull/40)).** Priority field hot-path: `SmatchetImageTextureCache::TryGetCached` (lock + map lookup + LRU touch, no I/O) added; `GetOrLoadFromFile` and `LoadOrFetchUrlImage` short-circuit on it before any disk read or `fs::exists` stat. Per-rawValue memo (`PriorityResolveMemo { CacheKey, Label }`, capped 256) added to `SmatchetFieldIconRender.cpp` so `TryDrawFieldValueIcon` / `TryGetInlineFieldIconTexture` skip `ParsePriorityJson` + slug normalisation + candidate-vector build + bundled-PNG `fs::exists` on every frame after first load. Memo stores the parsed label so the fast-path tooltip matches the slow path when `field == nullptr`. Closes the FPS regression on views that show the priority column.
63. ⏳ **OPEN — polish follow-ups deferred from PR [#40](https://github.com/alexandrosk0/Smatchet/pull/40) review.** P3 / nits, no current bugs:
    - The priority memo (`PriorityRawValueToCacheKey()` static `unordered_map`) is unprotected. Callers (`TryDrawFieldValueIcon`, `TryGetInlineFieldIconTexture`) are UI-thread-only today, but there is no comment or assertion guarding that invariant. Add a `// UI thread only` annotation, or wrap in a mutex if any future caller resolves priority icons off the UI thread.
    - `EvictCacheKey` does not invalidate the priority memo. Stale entries are self-healing (next frame's `TryGetCached` misses → slow path re-resolves and overwrites the memo), but a one-line comment on `PriorityResolveMemo` documenting that a future reader doesn't add a redundant invalidation hook.
    - `LoadPriorityIconWithFallbacks` reconstructs the `url:` / `file:` cache-key prefix from the resolved candidate string (`SmatchetFieldIconRender.cpp:316`), duplicating the prefix logic in `LoadTextureForResolvedPath`. Cleaner: have `LoadTextureForResolvedPath` return the cache key it used via an out-param.
    - `LoadOrFetchUrlImage` line 232 still constructs `std::string("url:") + url` inline after `memKey` was added at line 206. Reuse `memKey`.
    - Memo eviction policy is `m.clear()` on cap-hit. Priority cardinality (~10) means this never trips today; if other call sites later reuse the memo, swap for a small LRU.
