...
| N12 IsTrackerTransportErrorText | 🟡 IN PROGRESS (items 12/13a/13b shipped 2026-07-11) | Transport-ness travels structurally end-to-end: all four backends classify `TrackerError` kinds (item 12), the TrackerError-holding consumers classify by kind (13a: prefetch log routing, replay conflict re-fetch, `HandleFieldEditUpdateFailure`), and the string-seam consumers now carry an `ErrorTransient` flag from the mutation/create/catalog `TrackerError` (13b: field-edit offline-queue fallback, both create UIs, catalog chain through `IConnectivityDeps`, Linear probe). Remaining: slice 3 — classify Plane's resolve/exception streamed paths so the two seam fallbacks (`AppController.cpp` pack composer, `TicketSyncService.cpp` worker seam) can drop the sniff, then delete the heuristic (`docs/plans/active/retire-transport-error-text.md`). |
