...
| N12 IsTrackerTransportErrorText | 🟡 IN PROGRESS (slice 1 shipped 2026-07-10) | Transport-ness travels as `FetchErrorTransient` from the composition seams (slice 1), and the fetch path is now structured end-to-end (item 12): all four backends classify `TrackerError` kinds at their own error sites into the fetch summary / `FetchIssues` out-param, the seams prefer the kind over the text sniff, and Plane's replay re-fetch no longer wraps Unknown. Remaining: item 13 consumer swaps (mutation/replay/UI + catalog) + slice 3 deletes the heuristic (`docs/plans/active/retire-transport-error-text.md`). |
