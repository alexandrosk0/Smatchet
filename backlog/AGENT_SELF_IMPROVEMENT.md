...
- 2026-05-15 · git-janitor · [process] — PR-only-to-`develop` rule has no FF-clean docs-only escape valve
  ...
  Status: open · candidate for `git-janitor`. Apply once a second session hits the same friction, or sooner if the user signals the FF-clean push will keep recurring.
...
- 2026-05-15 · test-rig · [context] — `JiraClient.h` cascade blocks per-cpp testing of `TrackerFieldValueParser`
  ...
  Status: open · candidate for `tracker-backend`. ROI is high — value-parser bugs (timetracking, datetime, option-id) are the single largest source of cell-render regressions, and they're all pure-logic.
...
- 2026-05-15 · test-rig · [context] — `OfflineCreateQueue::kMaxReplayAttempts` lives behind `<SQLiteCpp/SQLiteCpp.h>`
  ...
  Status: open · candidate for `offline-sync`. Driver: any regression in the "after N attempts archive vs retry" decision lands as silent ticket loss; pure-function testability is worth the small refactor.
