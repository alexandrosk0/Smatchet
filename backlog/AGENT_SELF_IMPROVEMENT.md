...
- 2026-05-13 · test-author · [new-agent / tooling] — bucket E (ImGui Test Engine) not wired; bucket-E items currently flagged as manual residue with a deferred-automation note
  ...
  Status: open · plan at `docs/design/applied/imgui-test-engine-bucket-e.md` — scope-only; FetchContent + SmatchetUiTest target + tests/ui/ + ui_test.run CLI scoped in 5-commit migration order. Locked decisions table at top of plan.
