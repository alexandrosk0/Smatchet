...
- **`UpdateField` is set-replace.** `ITrackerIssueMutations::UpdateField(issueId, field, values, outError)` takes the **intended full set** for multi-value fields (labels/assignees/components), not a delta. Jira/Plane are natively set-replace; GitHub reconciles internally (pre-fetch + `LabelEditDiffPure` diff) so callers still see one set-replace virtual.
