...
- **No backend leak into shared interfaces.** Backend-specific code (`Jira*`, `Plane*`, `GitHub*`) must NOT leak into `ITrackerBackend` or its six role interfaces (`ITrackerIssueReader`, `ITrackerConnectivity`, `ITrackerFieldCatalog`, `ITrackerIssueMutations`, `ITrackerCollaboration`, `ITrackerActivity`). The shared interfaces are backend-agnostic; per-backend behaviour lives behind the concrete client (`JiraClient` / `PlaneClient` / `GitHubClient`) only. (The old monolithic `ITrackerClient` was split into these — never reintroduce that name.)
...
- **Parent references are `"KEY - Summary"` strings, split only via `ParentHierarchyPure`.** A ticket's `parent` field value is the parent's key, optionally followed by `" - "` + summary. Every consumer that needs the bare key (`IssueDraft`, `TrackerFieldPayloadPure::ExtractIssueKey`, the new-issue draft UI, the sync top-up, the grid hierarchy) calls `ParentHierarchyPure::ParentKeyFromFieldValue` / `ParentKeyOf` — never hand-roll a `find(" - ")`. Backends that carry parentage in a different shape (Jira `issuelinks` "is part of") normalise into `parent` at mapping time (`JiraIssueMappingPure`), so the hierarchy helpers stay backend-agnostic.
