...
void DrawAppearanceTypographySection(UiDrawSession& d) {
    ...
    if (!d.prefsFilter.ShowSetting("appearance.theme_font.font")) {
        return;
    }
...
void DrawAppearanceDisplaySection(UiDrawSession& d) {
    if (d.prefsFilter.ShowSetting("appearance.display.vsync")) {
    ...
    if (d.prefsFilter.ShowSetting("appearance.display.layout_reset_confirm")) {
...
void DrawAppearanceUiModeSection(UiDrawSession& d) {
    if (!d.prefsFilter.ShowSetting("appearance.layout_density.ui_mode")) {
        return;
    }
...
void DrawMobileNavPagesEditor(UiDrawSession& d, std::vector<std::string>& nav);
void DrawMobileHomePageCombo(UiDrawSession& d, std::vector<std::string>& nav);
...
void DrawAppearanceMobileSection(UiDrawSession& d) {
    const bool showDensity = d.prefsFilter.ShowSetting("appearance.layout_density.mobile_density");
    const bool showNavPages = d.prefsFilter.ShowSetting("appearance.layout_density.mobile_nav_pages");
    const bool showHomePage = d.prefsFilter.ShowSetting("appearance.layout_density.mobile_home_page");
    if (!showDensity && !showNavPages && !showHomePage) {
        return;
    }
    ImGui::Spacing();
    ...
    if (showDensity) {
        ...
        ImGui::SameLine();
        densityChanged |=
            ImGui::RadioButton("Comfortable", &density, static_cast<int>(MobileTouchDensity::Comfortable));
        if (densityChanged) {
    ...
    // the remaining hidden pages as unchecked rows to re-add. One descriptor covers
    // the whole editor — its per-row controls are not individually filterable.
    if (!showNavPages && !showHomePage) {
        return;
    }
    std::vector<std::string>& nav = d.cfg.MobileNavPages;
    if (showNavPages) {
        DrawMobileNavPagesEditor(d, nav);
    }
    if (!showHomePage) {
        return;
    }
    DrawMobileHomePageCombo(d, nav);
}

// Ordered visible-page rows (hide / reorder) followed by the hidden pages as
// unchecked rows to re-add. Split out of DrawAppearanceMobileSection so neither
// half crosses the function-length cap once the filter gates are in.
void DrawMobileNavPagesEditor(UiDrawSession& d, std::vector<std::string>& nav) {
    ImGui::Spacing();
...
// Home page combo, chosen from the visible pages.
void DrawMobileHomePageCombo(UiDrawSession& d, std::vector<std::string>& nav) {
    ImGui::Spacing();
    int homeIdx = 0;
...
void DrawTrackerNotificationsSectionBody(UiDrawSession& d) {
    if (d.prefsFilter.ShowSetting("tracker.notifications.enabled")) {
...
void DrawAppearancePreferencesTab(UiDrawSession& d) {
    SmatchetPreferencesUiDetail::PrefsSection(d, "appearance.theme_font", [&] { DrawAppearanceTypographySection(d); });
    ...
    SmatchetPreferencesUiDetail::PrefsSection(d, "appearance.display", [&] { DrawAppearanceDisplaySection(d); });
}
