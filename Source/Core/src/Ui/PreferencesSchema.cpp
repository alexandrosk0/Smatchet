...
const PrefsSettingDesc kSettings[] = {
    ...
    {"general.updates.check_now", "general.updates", "Check for Updates Now",
     "appearance update manual"},
    ...
    {"general.language_region.date_compact_threshold", "general.language_region",
     "Compact Relative Threshold (Days)", "appearance date format compact relative days", true},

    // -- general.storage (was: Local data tab) ------------------------------
    {"general.storage.mode", "general.storage", "Storage mode",
     "local data cache database offline sqlite"},
    ...
    {"appearance.theme_font.font", "appearance.theme_font", "Application Font",
     "appearance font size typeface"},
    ...
    // -- appearance.display -------------------------------------------------
    {"appearance.display.vsync", "appearance.display", "Enable vsync",
     "appearance vsync frame rate refresh"},
    {"appearance.display.layout_reset_confirm", "appearance.display",
     "Ask before layout-resetting changes", "appearance layout reset confirm skip"},
    ...
    // Jira:
    {"tracker.backend.jira_domain", "tracker.backend", "Domain",
     "tracker jira credentials atlassian url", true},
    {"tracker.backend.jira_email", "tracker.backend", "Email",
     "tracker jira credentials atlassian", true},
    {"tracker.backend.jira_api_token", "tracker.backend", "API Token",
     "tracker jira credentials token", true},
    {"tracker.backend.jira_inherit", "tracker.backend",
     "New issue: inherit fields from last row (Jira)", "tracker jira new issue inherit default", true},
    // Plane:
    {"tracker.backend.plane_url", "tracker.backend", "URL",
     "tracker plane credentials", true},
    {"tracker.backend.plane_workspace", "tracker.backend", "Workspace Slug",
     "tracker plane credentials workspace", true},
    {"tracker.backend.plane_api_key", "tracker.backend", "API Key",
     "tracker plane credentials key", true},
    {"tracker.backend.plane_inherit", "tracker.backend",
     "New issue: inherit fields from last row (Plane)", "tracker plane new issue inherit default", true},
    // GitHub Projects:
    {"tracker.backend.gh_base_url", "tracker.backend", "Base URL",
     "tracker github credentials enterprise", true},
    {"tracker.backend.gh_pat", "tracker.backend", "Personal Access Token",
     "tracker github credentials pat token", true},
    {"tracker.backend.gh_owner", "tracker.backend", "Owner",
     "tracker github credentials org", true},
    {"tracker.backend.gh_repo", "tracker.backend", "Repo",
     "tracker github credentials repository", true},
    {"tracker.backend.gh_project_number", "tracker.backend", "Project number",
     "tracker github credentials project", true},
    {"tracker.backend.gh_inherit", "tracker.backend",
     "New issue: inherit fields from last row (GitHub)", "tracker github new issue inherit default", true},
    // Linear:
    {"tracker.backend.linear_api_key", "tracker.backend", "API Key",
     "tracker linear credentials key", true},
    {"tracker.backend.linear_team_key", "tracker.backend", "Team Key",
     "tracker linear credentials", true},
    {"tracker.backend.linear_team", "tracker.backend", "Team",
     "tracker linear credentials", true},
    {"tracker.backend.linear_base_url", "tracker.backend", "Base URL",
     "tracker linear credentials", true},
    {"tracker.backend.linear_workspace_url", "tracker.backend", "Workspace URL",
     "tracker linear credentials workspace", true},
    {"tracker.backend.linear_inherit", "tracker.backend",
     "New issue: inherit fields from last row (Linear)", "tracker linear new issue inherit default", true},
    {"tracker.backend.test_connection", "tracker.backend", "Test connection",
     "tracker credentials verify check"},
    {"tracker.backend.open_views_dashboard", "tracker.backend", "Open Views Dashboard",
    ...
    {"tracker.notifications.enabled", "tracker.notifications",
     "Notify me when tracked issues change", "appearance ticket monitor notify watch"},
    ...
    {"connections.mcp.port", "connections.mcp", "MCP Port",
     "integrations mcp server port"},
    ...
    {"connections.perforce.p4_exe", "connections.perforce", "p4 executable",
     "annotate perforce p4 path"},
    {"connections.perforce.p4vc_exe", "connections.perforce", "p4vc executable",
     "annotate perforce p4vc path"},
    ...
    {"connections.perforce.path_remaps", "connections.perforce", "Path remaps",
     "annotate perforce path remap depot"},
    ...
    {"connections.activity.day_window", "connections.activity", "Activity day window",
     "user info activity feed days"},
    ...
    {"ai_voice.assistant.test_connection", "ai_voice.assistant", "Test connection",
     "assistant ai verify check"},
    // ConditionalDraw group: provider-specific rows draw only while their
    // provider is selected in "AI provider".
    {"ai_voice.assistant.openai_api_key", "ai_voice.assistant", "OpenAI API key",
     "assistant ai openai credentials key", true},
    {"ai_voice.assistant.model_openai", "ai_voice.assistant", "OpenAI model",
     "assistant ai openai gpt", true},
    ...
    {"ai_voice.assistant.base_url", "ai_voice.assistant", "Base URL",
     "assistant ai endpoint proxy gateway custom", true},
    {"ai_voice.assistant.allow_custom_openai", "ai_voice.assistant",
     "Allow custom endpoint (proxy / gateway)", "assistant ai openai endpoint proxy", true},
    ...
    {"ai_voice.assistant.model_anthropic", "ai_voice.assistant", "Anthropic model",
     "assistant ai anthropic claude", true},
    {"ai_voice.assistant.allow_custom_anthropic", "ai_voice.assistant",
     "Allow custom endpoint (proxy / gateway)", "assistant ai anthropic endpoint proxy", true},
    {"ai_voice.assistant.model_ollama", "ai_voice.assistant", "Ollama model",
     "assistant ai ollama local", true},
    ...
    {"ai_voice.assistant.model_deepseek", "ai_voice.assistant", "DeepSeek model",
     "assistant ai deepseek", true},
    // DeepSeek's endpoint row has its own buffer, distinct from the shared
    // OpenAI/Anthropic one above.
    {"ai_voice.assistant.deepseek_base_url", "ai_voice.assistant", "Base URL",
     "assistant ai deepseek endpoint", true},
    ...
    {"ai_voice.agent_context.project_path", "ai_voice.agent_context",
     "Project agents.md path (override)", "assistant ai agents.md harness context project"},
    {"ai_voice.agent_context.auto_discover", "ai_voice.agent_context",
     "Auto-discover project agents.md (walk up from cwd)",
     "assistant ai agents.md harness discover"},
#endif
    ...
    {"ai_voice.dictation.mode", "ai_voice.dictation", "Mode",
     "whisper voice dictation local cloud"},
    {"ai_voice.dictation.model", "ai_voice.dictation", "Speech model",
     "whisper voice dictation model"},
    ...
    {"ai_voice.dictation.language", "ai_voice.dictation", "Language:",
     "whisper voice dictation transcribe language"},
    {"ai_voice.dictation.trim", "ai_voice.dictation", "Trim leading/trailing silence",
     "whisper voice dictation audio silence"},
    {"ai_voice.dictation.max_clip", "ai_voice.dictation", "Max clip length:",
     "whisper voice dictation audio recording length"},
    {"ai_voice.dictation.auto_send", "ai_voice.dictation",
    ...
    {"ai_voice.diagnostics.test_e2e", "ai_voice.diagnostics",
    ...
    {"editing.grid.single_click", "editing.grid", "Single-click to edit grid cells",
     "grid rows editing click"},
    {"editing.grid.long_text_preview", "editing.grid",
     "Open long-text editor in preview mode", "grid editing markdown preview"},
    ...
    {"editing.time_estimates.duration_suggestions", "editing.time_estimates",
     "Duration Suggestions", "fields inputs time estimate duration suggestions"},

    // -- editing.work_log_templates (was: Fields Inputs tab) -----------------
    {"editing.work_log_templates.list", "editing.work_log_templates",
     "Work Log Description Templates", "fields inputs worklog work log template"},

    // -- editing.quick_comments (was: Fields Inputs tab) ---------------------
    {"editing.quick_comments.list", "editing.quick_comments",
     "Grid Right-Click Quick Comments", "fields inputs quick comment grid right-click"},

    // -- editing.annotate_comments (was: Fields Inputs tab) ------------------
    {"editing.annotate_comments.list", "editing.annotate_comments",
     "Annotate Quick Comments", "fields inputs annotate quick comment"},

#if defined(SMATCHET_EMBEDDED_IN_UNREAL)
    // -- editing.quick_create (was: Unreal tab) ------------------------------
    {"editing.quick_create.engine_version", "editing.quick_create", "Engine version",
     "unreal quick create context"},
    {"editing.quick_create.project", "editing.quick_create", "Project name and directory",
     "unreal quick create context"},
    {"editing.quick_create.platform", "editing.quick_create",
     "Platform, OS, and build configuration", "unreal quick create context"},
    {"editing.quick_create.level", "editing.quick_create", "Current level / map",
     "unreal quick create context"},
    {"editing.quick_create.pie_state", "editing.quick_create", "Play-in-editor (PIE) state",
     "unreal quick create context pie"},
    {"editing.quick_create.selected_actors", "editing.quick_create", "Selected actors",
     "unreal quick create context"},
    {"editing.quick_create.log_tail", "editing.quick_create", "Output Log tail",
     "unreal quick create context log"},
    // ConditionalDraw: drawn only while "Output Log tail" is enabled
    // (d.cfg.QuickCreateCtxLogTail).
    {"editing.quick_create.log_lines", "editing.quick_create", "Log lines",
     "unreal quick create context log lines", true},
#endif
    ...
    // -- annotate.analysis ---------------------------------------------------
    {"annotate.analysis.max_frames", "annotate.analysis", "Max frames",
     "annotate perforce p4 blame callstack"},
    {"annotate.analysis.ignore_keywords", "annotate.analysis",
     "Ignore keywords (comma or newline)", "annotate perforce filter"},
    // Kept under Annotate (not AI & Voice): this URL feeds the annotate
    // workflow's "open AI chat" hand-off, not the in-app assistant.
    {"annotate.analysis.ai_chat_url", "annotate.analysis", "AI chat URL (optional)",
     "annotate perforce ai chat link"},
    ...
    {"annotate.field_mapping.callstack_field", "annotate.field_mapping",
     "Callstack source field", "annotate tracker field callstack"},
    {"annotate.field_mapping.before_cl_field", "annotate.field_mapping",
     "Before-changelist field", "annotate tracker field changelist"},
    {"annotate.field_mapping.last_occurrences_field", "annotate.field_mapping",
     "Last-occurrences date field", "annotate tracker field date"},

    // -- annotate.colors -----------------------------------------------------
    {"annotate.colors.status_info", "annotate.colors", "Status info",
     "annotate color theme"},
    {"annotate.colors.status_error", "annotate.colors", "Status error",
     "annotate color theme"},
    {"annotate.colors.status_warning", "annotate.colors", "Status warning",
     "annotate color theme"},
    {"annotate.colors.find_highlight", "annotate.colors", "Find highlight",
     "annotate color theme search"},
    {"annotate.colors.text_disabled", "annotate.colors", "Text disabled",
     "annotate color theme"},
    {"annotate.colors.import_existing", "annotate.colors", "Import existing",
     "annotate color theme"},
