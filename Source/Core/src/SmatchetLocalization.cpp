...
const TranslationEntry kEntries[] = {
    ...
    {"views.jira_domain", "Domain", u8"Domaine"},
    {"views.jira_domain.help", "Which Jira site this view talks to. Extra sites are added in Preferences → Tracker.",
     u8"Site Jira auquel cette vue s'adresse. Les sites supplémentaires s'ajoutent dans Préférences → Suivi."},
    ...
    {"prefs.cat.appearance", "Appearance", u8"Apparence"},
    {"prefs.cat.annotate", "Annotate", u8"Annoter"},
    ...
    {"keybindings.editor.clearTooltip", "Remove every combo for this action",
     u8"Supprimer toutes les combinaisons de cette action"},
    {"keybindings.editor.addCombo", "+ Add", u8"+ Ajouter"},
    {"keybindings.editor.removeCombo", "Remove this combo", u8"Supprimer cette combinaison"},
    ...
    {"freshness.cached_offline", "Offline \xE2\x80\x94 showing saved data",
     u8"Hors ligne — données enregistrées affichées"},
    {"freshness.cached_stale", "Showing saved data \xE2\x80\x94 the last refresh failed",
     u8"Données enregistrées affichées — la dernière actualisation a échoué"},
    ...
    // "Quick Create Issue" doubles as the window title (WindowTitleFromSource keys
    // on the English text).
    {"quickcreate.window_title", "Quick Create Issue", u8"Création rapide de ticket"},
    {"toast.quick_create", "Quick create", u8"Création rapide"},
    {"quickcreate.created", "Created %s", u8"%s créé"},
    {"quickcreate.queued_offline", "Offline detected; queued for replay.",
     u8"Hors ligne détecté ; mis en file d'attente."},
    {"quickcreate.project", "Project", u8"Projet"},
    {"quickcreate.type", "Type", u8"Type"},
    {"quickcreate.type_none", "(no issue types cached)", u8"(aucun type de ticket en cache)"},
    {"quickcreate.summary", "Summary", u8"Résumé"},
    {"quickcreate.description", "Description", u8"Description"},
    {"quickcreate.context_generating", "(gathering context…)", u8"(collecte du contexte…)"},
    {"quickcreate.creating", "Creating…", u8"Création…"},
    {"quickcreate.refresh_context", "Refresh context", u8"Actualiser le contexte"},
    {"quickcreate.refresh_locked", "Description edited — refresh disabled.",
     u8"Description modifiée — actualisation désactivée."},
    {"quickcreate.full_flow_hint", "Fields beyond this popup are required — use \"+ New issue\" in the grid.",
     u8"Des champs hors de cette fenêtre sont requis — utilisez « + New issue » dans la grille."},
    {"quickcreate.prefs.tabTitle", "Unreal", u8"Unreal"},
    {"quickcreate.prefs.intro",
     "The quick-create issue popup (%s) prefills its description with context from the running Unreal Engine "
     "session. Choose which items are included:",
     u8"La fenêtre de création rapide (%s) préremplit sa description avec le contexte de la session Unreal Engine "
     u8"en cours. Choisissez les éléments inclus :"},
    {"quickcreate.prefs.unbound", "unbound", u8"non assigné"},
    {"quickcreate.prefs.engine_version", "Engine version", u8"Version du moteur"},
    {"quickcreate.prefs.project", "Project name and directory", u8"Nom et répertoire du projet"},
    {"quickcreate.prefs.platform", "Platform, OS, and build configuration",
     u8"Plateforme, OS et configuration de build"},
    {"quickcreate.prefs.level", "Current level / map", u8"Niveau / carte actuel"},
    {"quickcreate.prefs.pie_state", "Play-in-editor (PIE) state", u8"État Play-in-editor (PIE)"},
    {"quickcreate.prefs.selected_actors", "Selected actors", u8"Acteurs sélectionnés"},
    {"quickcreate.prefs.log_tail", "Output Log tail", u8"Fin de l'Output Log"},
    {"quickcreate.prefs.log_lines", "Log lines", u8"Lignes de log"},
    {"quickcreate.prefs.note", "Items missing from the engine snapshot are skipped even when enabled.",
     u8"Les éléments absents de l'instantané du moteur sont ignorés même s'ils sont activés."},
    ...
    {"grid.sort.hide_parents.tip", "Hide parent stories (show only leaf tasks/bugs)",
     u8"Masquer les stories parentes (afficher seulement les tâches/bugs feuilles)"},
    ...
    {"prefs.tracker.jira_extras.heading", "Additional Jira sites", u8"Sites Jira supplémentaires"},
    {"prefs.tracker.jira_extras.help", "Empty email or token inherit from the first Jira site.",
     u8"Un e-mail ou un jeton vide hérite du premier site Jira."},
    {"prefs.tracker.jira_extras.add", "Add Jira site", u8"Ajouter un site Jira"},
    {"prefs.tracker.jira_extras.remove", "Remove", u8"Supprimer"},
    {"prefs.tracker.jira_extras.email_hint", "optional — defaults to first site",
     u8"facultatif — hérite du premier site"},
    {"prefs.tracker.jira_extras.token_hint", "optional — defaults to first site",
     u8"facultatif — hérite du premier site"},
    ...
    {"panel.release_to_hide", "Release to hide the panel", u8"Relâchez pour masquer le panneau"},
    {"panel.drag_to_open", "Drag up to open the panel", u8"Faites glisser vers le haut pour ouvrir le panneau"},
