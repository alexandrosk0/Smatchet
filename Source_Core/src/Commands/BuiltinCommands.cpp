...
void RegisterBuiltinCommands(CommandRegistry& reg, AppController& app) {
...

    {
        ...
        c.Description = "Returns array of UI perf rows ({name, lastTotalMs, avgPerCallMs, maxMs, calls, lifetimeHits, emaAvgMs}).";
        reg.Register(std::move(c));
    }

    {
        Command c = MakeCommand(
            ...
            [](const nlohmann::json&, const CommandContext&) {
                ...
                for (const UiPerfRow& r : rows) totalCalls += r.calls;
            ...
            [&app](const nlohmann::json& args, const CommandContext&) {
                ...
                if (snapshot) {
                    for (const CachedTicket& t : *snapshot) {
                        ...
                        if (itSummary != fv.end()) one["summary"] = itSummary->second;
                        auto itStatus = fv.find("status");
                        if (itStatus != fv.end()) one["status"] = itStatus->second;
                        items.push_back(std::move(one));
                    }
                }
                return CommandResult::Success(PaginateJsonArray(items, limit, offset));
                ...
                if (snapshot && !q.empty()) {
                    for (const CachedTicket& t : *snapshot) {
                        ...
                        if (hit) {
                            ...
                            if (itS != t.fieldValues.end()) one["summary"] = itS->second;
                            items.push_back(std::move(one));
                        }
                    }
                }
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json& args, const CommandContext&) {
                ...
                return CommandResult::Failure(
                    ErrorCode::NotFound,
                    "Ticket '" + id + "' not found in active view.",
...

    {
        Command c = MakeCommand(
            ...
            [](const nlohmann::json& args, const CommandContext&) {
                ...
                else if (level == "debug") LOG_DEBUG("[cmd] %s", msg.c_str());
                else if (level == "warn")  LOG_WARN("[cmd] %s", msg.c_str());
                else if (level == "error") LOG_ERROR("[cmd] %s", msg.c_str());
                else                       LOG_INFO("[cmd] %s", msg.c_str());
                nlohmann::json out;
                out["written"] = true;
                return CommandResult::Success(std::move(out));
            });
        ...
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json&, const CommandContext& ctx) {
                if (ctx.DryRun) {
                    ...
                    return CommandResult::Success({{"wouldDo", "incremental sync from last-fetched cursor"}});
                }
                app.SyncWithBackend();
                nlohmann::json out;
                out["triggered"] = true;
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json&, const CommandContext&) {
                ...
                switch (s) {
                    case S::Unknown:                       stateStr = "unknown"; break;
                    case S::AuthenticatedReachable:        stateStr = "authenticated-reachable"; break;
                    case S::ReachableAuthOrConfigError:    stateStr = "reachable-auth-or-config-error"; break;
                    case S::TransportDown:                 stateStr = "transport-down"; break;
                    case S::ServiceUnavailable:            stateStr = "service-unavailable"; break;
                }
                nlohmann::json out;
                out["state"] = stateStr;
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json& args, const CommandContext&) {
                ...
                char timeSpent[64] = {};
                if (h > 0 && m > 0) std::snprintf(timeSpent, sizeof(timeSpent), "%dh %dm", h, m);
                else if (h > 0)     std::snprintf(timeSpent, sizeof(timeSpent), "%dh", h);
                else if (m > 0)     std::snprintf(timeSpent, sizeof(timeSpent), "%dm", m);
                else                std::snprintf(timeSpent, sizeof(timeSpent), "%ds", s);
                std::string err;
        ...
        {
            ParamSpec ps;
            ps.Name = "seconds"; ps.Type = ParamType::Int; ps.Required = true;
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json& args, const CommandContext& ctx) {
                ...
                if (!statusField) {
                    return CommandResult::Failure(ErrorCode::NotFound,
                        "Status field not found in catalog.", "Run fields.refresh_catalog first.");
                }
                std::string err;
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json&, const CommandContext&) {
                ...
                if (!ok) out["error"] = app.GetFieldCatalogError();
                return CommandResult::Success(std::move(out));
            });
        c.Idempotent = false;
        reg.Register(std::move(c));
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json& args, const CommandContext&) {
                ...
                if (!ok) {
                    return CommandResult::Failure(ErrorCode::BackendError,
                        "Watchers fetch failed: " + err);
                }
                nlohmann::json items = nlohmann::json::array();
                for (const TrackerUser& u : watchers) {
                    nlohmann::json one;
                    one["id"] = u.AccountId; one["displayName"] = u.DisplayName;
                    items.push_back(std::move(one));
                }
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json&, const CommandContext& ctx) {
                ...
                for (const auto& c2 : deadCreates) cIds.push_back(c2.DeadId);
                for (const auto& e  : deadEdits)   eIds.push_back(e.DeadId);
        ...
        Command c = MakeCommand(
            ...
            [](const nlohmann::json& args, const CommandContext&) {
                ...
                if (!key.empty()) {
                    ...
                    return CommandResult::Failure(ErrorCode::NotFound,
                        "Config key '" + key + "' not found or not exposed.");
                }
...

    {
        Command c = MakeCommand(
            ...
            [](const nlohmann::json& args, const CommandContext&) {
                ...
                for (const UiPerfRow& r : rows) {
                    doc.push_back({{"name", r.name}, {"lastTotalMs", r.lastTotalMs},
                                   {"avgPerCallMs", r.avgPerCallMs}, {"maxMs", r.maxMs},
                                   {"calls", r.calls}, {"emaAvgMs", r.emaAvgMs}});
                }
                // Write file.
                std::error_code ec;
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json& args, const CommandContext& ctx) {
                ...
                for (const auto& kv : fieldsMap.items()) {
                    ...
                    std::string val = kv.value().is_string()
        ...
        c.Params = {
            PString("id", "Ticket id.", true),
            {[]{ ParamSpec p; p.Name="fields"; p.Type=ParamType::Json; p.Required=true;
                 p.Description="JSON object: {fieldId: value, ...}"; return p; }()},
        };
        reg.Register(std::move(c));
    }

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json& args, const CommandContext& ctx) {
                ...
                if (ctx.DryRun) {
                    return CommandResult::Success({{"wouldDo", {
                        {"projectKey", projectKey}, {"issueType", issueTypeName},
                ...
                if (offline) {
                    ...
                    if (qid <= 0) {
                        return CommandResult::Failure(ErrorCode::HandlerError,
                            "Failed to queue offline create.");
                    }
                    return CommandResult::Success({
                        {"queued", true}, {"offlineId", static_cast<long long>(qid)}});
                }
        ...
        c.Params = {
            ...
            {[]{ ParamSpec p; p.Name="offline"; p.Type=ParamType::Bool;
                 p.Default=false; p.Description="Queue offline rather than creating live.";
                 return p; }()},
        };
        reg.Register(std::move(c));
    }
...

    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json&, const CommandContext&) {
                ...
                if (!pack.FetchError.empty()) out["error"] = pack.FetchError;
                if (!pack.Warning.empty())    out["warning"] = pack.Warning;
                return CommandResult::Success(std::move(out));
            });
        c.Idempotent = false;
        c.AsyncSafe = false;
...

    {
        ...
        c.DryRunSupported = true;
        c.Params = {
            {[]{ ParamSpec p; p.Name="on"; p.Type=ParamType::Bool; p.Required=true;
                 p.Description="true = enable read-only, false = disable."; return p; }()},
        };
        reg.Register(std::move(c));
    }
...

    {
        Command c = MakeCommand(
            ...
            [](const nlohmann::json& args, const CommandContext& ctx) {
                ...
                const bool newOpen = args.contains("open") ? args.value("open", false)
                                                           : !currentlyOpen;
                if (ctx.DryRun) {
        ...
        c.DryRunSupported = true;
        c.Params = {
            {[]{ ParamSpec p; p.Name="open"; p.Type=ParamType::Bool;
                 p.Description="true=show, false=hide, omit=toggle current state."; return p; }()},
        };
        reg.Register(std::move(c));
    }

    {
        Command c = MakeCommand(
            ...
            [](const nlohmann::json&, const CommandContext&) {
                ...
                nlohmann::json out;
                out["note"] = "Thread dump requires OS-specific APIs; counts are unavailable via this command.";
