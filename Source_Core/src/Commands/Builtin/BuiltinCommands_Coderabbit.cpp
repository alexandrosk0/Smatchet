...
void RegisterCoderabbitCommands(CommandRegistry& reg, AppController& app) {
    // ─── coderabbit-react.start ───────────────────────────────────────
    {
        Command c = MakeCommand(
            ...
            "an app restart.",
            [&app](const nlohmann::json&, const CommandContext&) { return FlipCoderabbitReactEnabled(app, true); });
        c.AsyncSafe = false;
    ...
    // ─── coderabbit-react.stop ────────────────────────────────────────
    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json&, const CommandContext&) { return FlipCoderabbitReactEnabled(app, false); });
        c.AsyncSafe = false;
        c.Idempotent = true;
        c.Destructive = true;
        reg.Register(std::move(c));
...
void RegisterCoderabbitCommands(CommandRegistry& reg, AppController& /*app*/) {
    ...
    for (size_t i = 0; i < sizeof(kNames) / sizeof(kNames[0]); ++i) {
        ...
        Command c = MakeCommand(name,
                                "CodeRabbit react loop disabled in this build (SMATCHET_WITH_AGENTIC=OFF).",
