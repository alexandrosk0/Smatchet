...
void RegisterCiReactCommands(CommandRegistry& reg, AppController& app) {
    // ─── ci-react.start ───────────────────────────────────────────────
    {
        Command c = MakeCommand(
            ...
            [&app](const nlohmann::json&, const CommandContext&) { return FlipCiReactEnabled(app, true); });
        c.AsyncSafe = false;
        c.Idempotent = true;
        c.Destructive = true;
    ...
    // ─── ci-react.stop ────────────────────────────────────────────────
    {
        Command c = MakeCommand("ci-react.stop",
                                ...
                                [&app](const nlohmann::json&, const CommandContext&) {
                                    return FlipCiReactEnabled(app, false);
                                });
        c.AsyncSafe = false;
        c.Idempotent = true;
        c.Destructive = true;
    ...
    // ─── ci-react.rerun ───────────────────────────────────────────────
    {
        ...
        ParamSpec runIdParam =
            PInt("workflow_id", "GitHub Actions workflow-run id to rerun (positive integer).", 0);
