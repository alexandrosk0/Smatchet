...
// CLI args land as JSON strings (Target_Standalone/CliCommandRunner.cpp § ParseArgs
...
class ThemeSwitchRoundtripScenario : public IScenario {
  public:
    ...
    nlohmann::json OnFinish(AppController& /*app*/) override {
        ...
        // Target_Standalone/main.cpp will write the PNG after the next
        // SwapBuffers.
        //
...
std::unique_ptr<smatchet::cmd::IScenario> MakeThemeSwitchRoundtripScenario() {
    return std::unique_ptr<smatchet::cmd::IScenario>(new smatchet::cmd::ThemeSwitchRoundtripScenario());
}
