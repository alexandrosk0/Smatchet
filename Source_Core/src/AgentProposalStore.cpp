...
bool IsLegalTransition(AgentProposalState from, AgentProposalState to) {
    ...
    case AgentProposalState::Failed:
        // (Retry carve-out) Operator may flip a Failed row back to Approved to
        // re-attempt the upstream apply. The OnProposalApproved callback fires
        // again on this transition, retrying the GitHubClient call.
        // (Dismiss carve-out) Operator may also send a Failed row to Rejected
        // to hide it from the panel permanently — the proposals UI filter is
        // Pending | Failed, so Rejected rows drop out of view. Audit trail
        // stays linear (Failed -> Rejected is a final terminal write).
        return to == AgentProposalState::Approved || to == AgentProposalState::Rejected;
...
bool RowToProposal(SQLite::Statement& q, AgentProposal& out, std::string& outError) {
    ...
    if (!ParseAgenticAction(actionStr, out.action)) {
        LOG_WARN("AgentProposalStore: unknown action literal in db (row id=%lld): %s", static_cast<long long>(out.id),
                 actionStr.c_str());
    ...
    // v3 columns — issue_title, issue_body. The SELECT lists used by Query +
    // Find both append these columns at indices 10 + 11; if a SELECT omits
    // them the getColumn will throw — caller must keep the SQL in sync with
    // this RowToProposal indexing contract.
    out.issueTitle = q.getColumn(10).getText();
    out.issueBody = q.getColumn(11).getText();
    // v4 column — issue_comments_json at index 12.
    out.issueCommentsJson = q.getColumn(12).getText();
    return true;
}
...
AgentProposalStore::AgentProposalStore(const std::string& dbPath)
    ...
    db_->exec("CREATE TABLE IF NOT EXISTS agent_proposals ("
              ...
              // v3 — upstream-issue context captured at triage time.
              "issue_title TEXT NOT NULL DEFAULT '', "
              "issue_body TEXT NOT NULL DEFAULT '', "
              // v4 — JSON-serialised comment thread (rendered in panel).
              "issue_comments_json TEXT NOT NULL DEFAULT '')");
    ...
    // v3 — belt-and-braces guard for fresh dbs upgraded mid-session. The
    // CREATE TABLE above already carries these for brand-new dbs; the migration
    // body in EnsureSchemaVersion handles v2-in-place upgrades. Idempotent.
    if (!SqliteTableHasColumn(*db_, "agent_proposals", "issue_title")) {
        db_->exec("ALTER TABLE agent_proposals ADD COLUMN issue_title TEXT NOT NULL DEFAULT ''");
    }
    if (!SqliteTableHasColumn(*db_, "agent_proposals", "issue_body")) {
        db_->exec("ALTER TABLE agent_proposals ADD COLUMN issue_body TEXT NOT NULL DEFAULT ''");
    }
    if (!SqliteTableHasColumn(*db_, "agent_proposals", "issue_comments_json")) {
        db_->exec("ALTER TABLE agent_proposals ADD COLUMN issue_comments_json TEXT NOT NULL DEFAULT ''");
    }
...
void AgentProposalStore::EnsureSchemaVersion() {
    LOG_TRACE("AgentProposalStore::EnsureSchemaVersion enter");
    ...
    LOG_DEBUG("AgentProposalStore::EnsureSchemaVersion read current=%d (build target=%d)", current,
    ...
    if (current == kCurrentSchemaVersion) {
        ...
        LOG_TRACE("AgentProposalStore::EnsureSchemaVersion exit reason=already_current (version=%d)", current);
        return;
    }
    ...
    if (current > kCurrentSchemaVersion) {
        ...
        LOG_ERROR("AgentProposalStore::EnsureSchemaVersion db schema_version=%d newer than build=%d — refusing",
    ...
    LOG_INFO("AgentProposalStore::EnsureSchemaVersion migrating from v%d to v%d", current, kCurrentSchemaVersion);
    ...
    while (current < kCurrentSchemaVersion) {
        if (current == 1) {
            ...
            // v2 -> v3: add agent_proposals.issue_title + issue_body so the
            // proposals panel can render upstream context inline.
            if (!SqliteTableHasColumn(*db_, "agent_proposals", "issue_title")) {
                db_->exec("ALTER TABLE agent_proposals ADD COLUMN issue_title TEXT NOT NULL DEFAULT ''");
            }
            if (!SqliteTableHasColumn(*db_, "agent_proposals", "issue_body")) {
            ...
            LOG_INFO("AgentProposalStore: migrated schema v2 -> v3 (issue_title + issue_body)");
        } else if (current == 3) {
            // v3 -> v4: add agent_proposals.issue_comments_json so the panel
            // can render the full comment thread alongside title + body.
            if (!SqliteTableHasColumn(*db_, "agent_proposals", "issue_comments_json")) {
                db_->exec("ALTER TABLE agent_proposals ADD COLUMN issue_comments_json TEXT NOT NULL DEFAULT ''");
            }
            LOG_INFO("AgentProposalStore: migrated schema v3 -> v4 (issue_comments_json)");
        }
        ++current;
    }
...
bool AgentProposalStore::GetSchemaVersion(int& outVersion, std::string& outError) const {
    LOG_TRACE("AgentProposalStore::GetSchemaVersion enter");
    outError.clear();
    outVersion = 0;
    try {
        SQLite::Statement q(*db_, "SELECT version FROM schema_version WHERE id = 1");
        if (!q.executeStep()) {
            ...
            LOG_WARN("AgentProposalStore::GetSchemaVersion exit reason=row_missing");
            return false;
        }
        outVersion = q.getColumn(0).getInt();
        LOG_DEBUG("AgentProposalStore::GetSchemaVersion -> %d", outVersion);
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::GetSchemaVersion: ") + ex.what();
        LOG_ERROR("AgentProposalStore::GetSchemaVersion exception: %s", ex.what());
        return false;
    }
}
...
bool AgentProposalStore::Insert(AgentProposal& proposal, std::string& outError) {
    LOG_TRACE("AgentProposalStore::Insert enter (issueKey=%s action=%s)", proposal.issueKey.c_str(),
              AgenticInferenceClientPure::ActionToString(proposal.action));
    outError.clear();
    try {
        ...
        const std::string actionStr = AgenticInferenceClientPure::ActionToString(proposal.action);

        // (Dedupe fix) Skip if an identical Pending row already exists for the
        // same (sourceTracker, issueKey, action, payloadDump). Two ticks of the
        // same triage scenario currently emit the same suggestion twice; the
        // user shouldn't have to approve/reject duplicates. Terminal-state
        // rows (Approved/Rejected/Applied/Failed) do NOT block re-insert — a
        // new pass legitimately produces a fresh Pending row for re-review.
        {
            SQLite::Statement dupQ(*db_, "SELECT id FROM agent_proposals "
                                         "WHERE source_tracker=? AND issue_key=? AND action=? AND payload_json=? "
                                         "AND state='Pending' LIMIT 1");
            dupQ.bind(1, proposal.sourceTracker);
            dupQ.bind(2, proposal.issueKey);
            dupQ.bind(3, actionStr);
            dupQ.bind(4, payloadDump);
            if (dupQ.executeStep()) {
                const std::int64_t dupId = dupQ.getColumn(0).getInt64();
                LOG_INFO("AgentProposalStore::Insert dedupe: skipping duplicate Pending row "
                         "(existing id=%lld issueKey=%s action=%s)",
                         static_cast<long long>(dupId), proposal.issueKey.c_str(), actionStr.c_str());
                proposal.id = dupId;
                return true;
            }
        }
        ...
        SQLite::Statement stmt(*db_, "INSERT INTO agent_proposals (source_tracker, issue_key, action, rationale, "
                                     ...
                                     "issue_title, issue_body, issue_comments_json) "
                                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        stmt.bind(1, proposal.sourceTracker);
        stmt.bind(2, proposal.issueKey);
        stmt.bind(3, actionStr);
        ...
        stmt.bind(10, proposal.issueTitle);
        stmt.bind(11, proposal.issueBody);
        stmt.bind(12, proposal.issueCommentsJson);
        LOG_DEBUG("SQL insert agent_proposals issueKey=%s action=%s payload_bytes=%zu title_bytes=%zu body_bytes=%zu "
                  "comments_bytes=%zu",
                  proposal.issueKey.c_str(), actionStr.c_str(), payloadDump.size(), proposal.issueTitle.size(),
                  proposal.issueBody.size(), proposal.issueCommentsJson.size());
        stmt.exec();
        proposal.id = db_->getLastInsertRowid();
        LOG_INFO("AgentProposalStore::Insert appended proposal id=%lld (issueKey=%s)",
                 static_cast<long long>(proposal.id), proposal.issueKey.c_str());
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::Insert: ") + ex.what();
        LOG_ERROR("AgentProposalStore::Insert exception (issueKey=%s): %s", proposal.issueKey.c_str(), ex.what());
        return false;
    }
}

bool AgentProposalStore::InsertMany(std::vector<AgentProposal>& proposals, std::string& outError) {
    LOG_TRACE("AgentProposalStore::InsertMany enter (count=%zu)", proposals.size());
    outError.clear();
    if (proposals.empty()) {
        LOG_TRACE("AgentProposalStore::InsertMany exit reason=empty_input");
        return true;
    }
    try {
        ...
        LOG_DEBUG("AgentProposalStore::InsertMany BEGIN transaction (rows=%zu)", proposals.size());

        const int64_t now = NowEpochSec();
        // (Dedupe fix) Pre-check identical Pending rows; skip duplicates per
        // the same contract as Insert. Done inside the transaction so the
        // SELECT sees rows committed by EARLIER iterations of the loop too
        // (LLM occasionally repeats a draft within the same batch).
        SQLite::Statement dupQ(*db_, "SELECT id FROM agent_proposals "
                                     "WHERE source_tracker=? AND issue_key=? AND action=? AND payload_json=? "
                                     "AND state='Pending' LIMIT 1");
        SQLite::Statement stmt(*db_, "INSERT INTO agent_proposals (source_tracker, issue_key, action, rationale, "
        ...
        for (auto& p : proposals) {
            ...
            const std::string actionStr = AgenticInferenceClientPure::ActionToString(p.action);

            dupQ.reset();
            dupQ.clearBindings();
            dupQ.bind(1, p.sourceTracker);
            dupQ.bind(2, p.issueKey);
            dupQ.bind(3, actionStr);
            dupQ.bind(4, payloadDump);
            if (dupQ.executeStep()) {
                const std::int64_t dupId = dupQ.getColumn(0).getInt64();
                LOG_INFO("AgentProposalStore::InsertMany dedupe: skipping duplicate Pending row "
                         "(existing id=%lld issueKey=%s action=%s)",
                         static_cast<long long>(dupId), p.issueKey.c_str(), actionStr.c_str());
                p.id = dupId;
                continue;
            }
            ...
            stmt.bind(10, p.issueTitle);
            stmt.bind(11, p.issueBody);
            stmt.bind(12, p.issueCommentsJson);
            LOG_DEBUG("SQL insert agent_proposals issueKey=%s action=%s payload_bytes=%zu title_bytes=%zu "
                      "body_bytes=%zu comments_bytes=%zu",
                      p.issueKey.c_str(), actionStr.c_str(), payloadDump.size(), p.issueTitle.size(),
                      p.issueBody.size(), p.issueCommentsJson.size());
            stmt.exec();
            p.id = db_->getLastInsertRowid();
            LOG_INFO("AgentProposalStore::InsertMany appended proposal id=%lld (issueKey=%s action=%s)",
                     static_cast<long long>(p.id), p.issueKey.c_str(), actionStr.c_str());
        }

        transaction.commit();
        LOG_DEBUG("AgentProposalStore::InsertMany COMMIT transaction (rows=%zu)", proposals.size());
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::InsertMany: ") + ex.what();
        LOG_ERROR("AgentProposalStore::InsertMany exception — rolling back (rows=%zu): %s", proposals.size(),
                  ex.what());
...
bool AgentProposalStore::Transition(int64_t id, AgentProposalState newState, const std::string& applyError,
                                    std::string& outError) {
    LOG_TRACE("AgentProposalStore::Transition enter (id=%lld newState=%s applyError_bytes=%zu)",
              static_cast<long long>(id), AgentProposalStateToString(newState), applyError.size());
    outError.clear();
    ...
    AgenticInferenceClientPure::ProposedAction approvedAction = AgenticInferenceClientPure::ProposedAction::Unknown;
    ...
    try {
        ...
        AgentProposalState current;
        {
            ...
            if (!q.executeStep()) {
                ...
                LOG_WARN("AgentProposalStore::Transition exit reason=row_not_found (id=%lld)",
            ...
            if (!ParseAgentProposalState(stateStr, current)) {
                outError = std::string("unknown state literal in db: ") + stateStr;
                LOG_WARN("AgentProposalStore::Transition exit reason=unknown_state_literal (id=%lld got=%s)",
                         static_cast<long long>(id), stateStr.c_str());
                return false;
            }
            LOG_DEBUG("AgentProposalStore::Transition read current state=%s for id=%lld", stateStr.c_str(),
                      static_cast<long long>(id));
        ...
        if (!IsLegalTransition(current, newState)) {
            ...
            LOG_WARN("AgentProposalStore::Transition exit reason=illegal_transition (id=%lld %s -> %s)",
                     static_cast<long long>(id), AgentProposalStateToString(current),
                     AgentProposalStateToString(newState));
            return false;
        }
        const int64_t now = NowEpochSec();
        const std::string err = (newState == AgentProposalState::Failed) ? applyError : std::string();
        LOG_DEBUG("SQL update agent_proposals id=%lld state=%s->%s apply_error_bytes=%zu", static_cast<long long>(id),
                  AgentProposalStateToString(current), AgentProposalStateToString(newState), err.size());
        ...
        if (changed == 0) {
            ...
            LOG_WARN("AgentProposalStore::Transition exit reason=row_vanished (id=%lld)", static_cast<long long>(id));
            return false;
        }
        transaction.commit();
        LOG_INFO("AgentProposalStore::Transition id=%lld %s -> %s committed", static_cast<long long>(id),
                 AgentProposalStateToString(current), AgentProposalStateToString(newState));
        ...
        if (current == AgentProposalState::Pending && newState == AgentProposalState::Approved &&
            approvedAction != AgenticInferenceClientPure::ProposedAction::Unknown) {
            LOG_DEBUG("AgentProposalStore::Transition: decision: schedule OnProposalApproved fire (id=%lld action=%s)",
                      static_cast<long long>(id), AgenticInferenceClientPure::ActionToString(approvedAction));
            fireApprovedCallback = true;
        }
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::Transition: ") + ex.what();
        LOG_ERROR("AgentProposalStore::Transition exception (id=%lld): %s", static_cast<long long>(id), ex.what());
        return false;
    }
    ...
    if (fireApprovedCallback && onApproved_) {
        LOG_INFO("apply OnProposalApproved id=%lld action=%s", static_cast<long long>(id),
                 AgenticInferenceClientPure::ActionToString(approvedAction));
        try {
...
void AgentProposalStore::SetOnProposalApproved(OnApprovedFn cb) {
    LOG_TRACE("AgentProposalStore::SetOnProposalApproved enter (cb_set=%d)", cb ? 1 : 0);
    onApproved_ = std::move(cb);
}

bool AgentProposalStore::Query(const Filter& f, std::vector<AgentProposal>& out, std::string& outError) const {
    LOG_TRACE("AgentProposalStore::Query enter (states=%zu sourceTracker=%s issueKey=%s limit=%d)", f.states.size(),
              f.sourceTracker.c_str(), f.issueKey.c_str(), f.limit);
    out.clear();
    outError.clear();
    try {
        ...
        if (states.size() > kMaxFilterStates) {
            LOG_WARN("AgentProposalStore::Query: Filter::states capped from %zu to %zu", states.size(),
                     static_cast<std::size_t>(kMaxFilterStates));
            states.resize(kMaxFilterStates);
        }

        std::string sql = "SELECT id, source_tracker, issue_key, action, rationale, payload_json, "
                          "state, created_at_sec, last_updated_at_sec, apply_error, issue_title, issue_body, "
                          "issue_comments_json FROM agent_proposals";
        ...
        LOG_TRACE("AgentProposalStore::Query executing SQL: %s", sql.c_str());
        while (q.executeStep()) {
            AgentProposal p;
            std::string rowErr;
            if (!RowToProposal(q, p, rowErr)) {
                ...
                LOG_WARN("AgentProposalStore::Query exit reason=row_to_proposal_failed: %s", rowErr.c_str());
                return false;
            }
            out.push_back(std::move(p));
        }
        LOG_TRACE("AgentProposalStore::Query returned %zu rows", out.size());
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::Query: ") + ex.what();
        LOG_ERROR("AgentProposalStore::Query exception: %s", ex.what());
        return false;
    }
}

bool AgentProposalStore::UpdatePayload(int64_t id, const nlohmann::json& newPayload, std::string& outError) {
    LOG_TRACE("AgentProposalStore::UpdatePayload enter (id=%lld)", static_cast<long long>(id));
    outError.clear();
    try {
        // Refuse non-Pending rows — once Approved the payload is what the
        // apply path saw, and rewriting it would break the audit chain.
        AgentProposalState current;
        {
            ...
            if (!q.executeStep()) {
                outError = "row not found";
                LOG_WARN("AgentProposalStore::UpdatePayload exit reason=row_not_found (id=%lld)",
                         static_cast<long long>(id));
                return false;
            }
            const std::string stateStr = q.getColumn(0).getText();
            if (!ParseAgentProposalState(stateStr, current)) {
                outError = std::string("unknown state literal: ") + stateStr;
                LOG_WARN("AgentProposalStore::UpdatePayload exit reason=unknown_state_literal id=%lld state=%s",
                         static_cast<long long>(id), stateStr.c_str());
                return false;
            }
        }
        if (current != AgentProposalState::Pending) {
            outError = std::string("payload edit refused: row is ") + AgentProposalStateToString(current) +
                       std::string(" (only Pending rows are editable)");
            LOG_WARN("AgentProposalStore::UpdatePayload exit reason=non_pending id=%lld state=%s",
                     static_cast<long long>(id), AgentProposalStateToString(current));
            return false;
        }
        const std::string payloadDump = newPayload.is_null() ? std::string("{}") : newPayload.dump();
        const int64_t now = NowEpochSec();
        SQLite::Statement up(*db_, "UPDATE agent_proposals SET payload_json=?, last_updated_at_sec=? "
                                   "WHERE id=?");
        up.bind(1, payloadDump);
        up.bind(2, static_cast<long long>(now));
        up.bind(3, static_cast<long long>(id));
        LOG_DEBUG("SQL update agent_proposals id=%lld payload_bytes=%zu (edit-before-send)", static_cast<long long>(id),
                  payloadDump.size());
        up.exec();
        LOG_INFO("AgentProposalStore::UpdatePayload id=%lld committed (%zu bytes)", static_cast<long long>(id),
                 payloadDump.size());
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::UpdatePayload: ") + ex.what();
        LOG_ERROR("AgentProposalStore::UpdatePayload exception (id=%lld): %s", static_cast<long long>(id), ex.what());
        return false;
    }
}

bool AgentProposalStore::Find(int64_t id, AgentProposal& out, std::string& outError) const {
    LOG_TRACE("AgentProposalStore::Find enter (id=%lld)", static_cast<long long>(id));
    outError.clear();
    try {
        ...
        if (!q.executeStep()) {
            outError = "row not found";
            LOG_WARN("AgentProposalStore::Find exit reason=row_not_found (id=%lld)", static_cast<long long>(id));
            return false;
        }
        std::string rowErr;
        if (!RowToProposal(q, out, rowErr)) {
            outError = "AgentProposalStore::Find: " + rowErr;
            LOG_WARN("AgentProposalStore::Find exit reason=row_to_proposal_failed (id=%lld): %s",
                     static_cast<long long>(id), rowErr.c_str());
            return false;
        }
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::Find: ") + ex.what();
        LOG_ERROR("AgentProposalStore::Find exception (id=%lld): %s", static_cast<long long>(id), ex.what());
        return false;
    }
}
...
bool AgentProposalStore::GetPollCursor(const std::string& sourceTracker, const std::string& repoKey,
    ...
    LOG_TRACE("AgentProposalStore::GetPollCursor enter (sourceTracker=%s repoKey=%s)", sourceTracker.c_str(),
              repoKey.c_str());
    outError.clear();
    ...
    try {
        ...
        if (q.executeStep()) {
            ...
            LOG_DEBUG("AgentProposalStore::GetPollCursor hit (sourceTracker=%s repoKey=%s lastSeen=%lld)",
                      sourceTracker.c_str(), repoKey.c_str(), static_cast<long long>(outLastSeenUpdatedAt));
        } else {
            LOG_DEBUG("AgentProposalStore::GetPollCursor miss (sourceTracker=%s repoKey=%s) — first-call semantics",
                      sourceTracker.c_str(), repoKey.c_str());
        }
        ...
        LOG_ERROR("AgentProposalStore::GetPollCursor exception: %s", ex.what());
        return false;
    }
}

bool AgentProposalStore::SetPrWatch(const PrWatchRow& row, std::string& outError) {
    LOG_TRACE("AgentProposalStore::SetPrWatch enter (proposalId=%lld prUrl=%s iter=%d)",
              static_cast<long long>(row.proposalId), row.prUrl.c_str(), row.iterationCount);
    outError.clear();
    try {
        ...
        LOG_DEBUG("SQL upsert agent_pr_watch proposalId=%lld prUrl=%s lastSeenCommentId=%s iter=%d",
                  static_cast<long long>(row.proposalId), row.prUrl.c_str(), row.lastSeenCommentIdStr.c_str(),
                  row.iterationCount);
        stmt.exec();
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::SetPrWatch: ") + ex.what();
        LOG_ERROR("AgentProposalStore::SetPrWatch exception (proposalId=%lld): %s",
                  static_cast<long long>(row.proposalId), ex.what());
        return false;
    }
}

bool AgentProposalStore::GetPrWatch(std::int64_t proposalId, PrWatchRow& outRow, std::string& outError) const {
    LOG_TRACE("AgentProposalStore::GetPrWatch enter (proposalId=%lld)", static_cast<long long>(proposalId));
    outError.clear();
    outRow = PrWatchRow();
    try {
        ...
        if (q.executeStep()) {
            ...
            LOG_DEBUG("AgentProposalStore::GetPrWatch hit (proposalId=%lld prUrl=%s iter=%d)",
                      static_cast<long long>(proposalId), outRow.prUrl.c_str(), outRow.iterationCount);
        } else {
            LOG_DEBUG("AgentProposalStore::GetPrWatch miss (proposalId=%lld) — first-poll semantics",
        ...
        LOG_ERROR("AgentProposalStore::GetPrWatch exception (proposalId=%lld): %s", static_cast<long long>(proposalId),
                  ex.what());
        return false;
    }
}
...
bool AgentProposalStore::DeletePrWatch(std::int64_t proposalId, std::string& outError) {
    LOG_TRACE("AgentProposalStore::DeletePrWatch enter (proposalId=%lld)", static_cast<long long>(proposalId));
    outError.clear();
    try {
        SQLite::Statement stmt(*db_, "DELETE FROM agent_pr_watch WHERE proposal_id=?");
        stmt.bind(1, static_cast<long long>(proposalId));
        LOG_DEBUG("SQL delete agent_pr_watch proposalId=%lld", static_cast<long long>(proposalId));
        stmt.exec();
        ...
        LOG_ERROR("AgentProposalStore::DeletePrWatch exception (proposalId=%lld): %s",
...
bool AgentProposalStore::SetOpenPrWatch(const OpenPrWatchRow& row, std::string& outError) {
    LOG_TRACE("AgentProposalStore::SetOpenPrWatch enter (prUrl=%s prNumber=%d headSha=%s)", row.prUrl.c_str(),
              row.prNumber, row.headSha.c_str());
    outError.clear();
    try {
        ...
        LOG_DEBUG("SQL upsert agent_open_pr_watch prUrl=%s prNumber=%d headRefName=%s headSha=%s", row.prUrl.c_str(),
                  row.prNumber, row.headRefName.c_str(), row.headSha.c_str());
        stmt.exec();
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::SetOpenPrWatch: ") + ex.what();
        LOG_ERROR("AgentProposalStore::SetOpenPrWatch exception (prUrl=%s): %s", row.prUrl.c_str(), ex.what());
        return false;
    }
}

bool AgentProposalStore::GetOpenPrWatch(const std::string& prUrl, OpenPrWatchRow& outRow, std::string& outError) const {
    LOG_TRACE("AgentProposalStore::GetOpenPrWatch enter (prUrl=%s)", prUrl.c_str());
    outError.clear();
    outRow = OpenPrWatchRow();
    try {
        SQLite::Statement q(*db_, "SELECT pr_url, pr_number, head_ref_name, head_sha, last_seen_comment_id_str, "
                                  "       last_seen_check_run_id, iteration_count, last_polled_at_sec "
        ...
        if (q.executeStep()) {
            ...
            LOG_DEBUG("AgentProposalStore::GetOpenPrWatch hit (prUrl=%s prNumber=%d iter=%d)", outRow.prUrl.c_str(),
                      outRow.prNumber, outRow.iterationCount);
        } else {
            LOG_DEBUG("AgentProposalStore::GetOpenPrWatch miss (prUrl=%s) — first-poll semantics", prUrl.c_str());
        }
        // Row absent -> defaulted outRow + ok=true (first-poll semantics).
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::GetOpenPrWatch: ") + ex.what();
        LOG_ERROR("AgentProposalStore::GetOpenPrWatch exception (prUrl=%s): %s", prUrl.c_str(), ex.what());
        return false;
    }
}

bool AgentProposalStore::ListOpenPrWatch(std::vector<OpenPrWatchRow>& outRows, std::string& outError) const {
    LOG_TRACE("AgentProposalStore::ListOpenPrWatch enter");
    outError.clear();
    outRows.clear();
    try {
        SQLite::Statement q(*db_, "SELECT pr_url, pr_number, head_ref_name, head_sha, last_seen_comment_id_str, "
        ...
        LOG_DEBUG("AgentProposalStore::ListOpenPrWatch returned %zu rows", outRows.size());
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::ListOpenPrWatch: ") + ex.what();
        LOG_ERROR("AgentProposalStore::ListOpenPrWatch exception: %s", ex.what());
        outRows.clear();
        return false;
    }
}
...
bool AgentProposalStore::DeleteOpenPrWatch(const std::string& prUrl, std::string& outError) {
    LOG_TRACE("AgentProposalStore::DeleteOpenPrWatch enter (prUrl=%s)", prUrl.c_str());
    outError.clear();
    try {
        SQLite::Statement stmt(*db_, "DELETE FROM agent_open_pr_watch WHERE pr_url=?");
        stmt.bind(1, prUrl);
        LOG_DEBUG("SQL delete agent_open_pr_watch prUrl=%s", prUrl.c_str());
        stmt.exec();
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::DeleteOpenPrWatch: ") + ex.what();
        LOG_ERROR("AgentProposalStore::DeleteOpenPrWatch exception (prUrl=%s): %s", prUrl.c_str(), ex.what());
        return false;
    }
}
...
bool AgentProposalStore::SetOpenPrCommentCursor(const std::string& prUrl, const std::string& lastSeenCommentIdStr,
                                                std::string& outError) {
    LOG_TRACE("AgentProposalStore::SetOpenPrCommentCursor enter (prUrl=%s lastSeenCommentIdStr=%s)", prUrl.c_str(),
              lastSeenCommentIdStr.c_str());
    outError.clear();
    try {
        ...
        stmt.bind(3, prUrl);
        LOG_DEBUG("SQL update agent_open_pr_watch.last_seen_comment_id_str prUrl=%s val=%s", prUrl.c_str(),
                  lastSeenCommentIdStr.c_str());
        stmt.exec();
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::SetOpenPrCommentCursor: ") + ex.what();
        LOG_ERROR("AgentProposalStore::SetOpenPrCommentCursor exception (prUrl=%s): %s", prUrl.c_str(), ex.what());
        return false;
    }
}
...
bool AgentProposalStore::SetOpenPrCheckRunCursor(const std::string& prUrl, std::int64_t lastSeenCheckRunId,
                                                 std::string& outError) {
    LOG_TRACE("AgentProposalStore::SetOpenPrCheckRunCursor enter (prUrl=%s lastSeenCheckRunId=%lld)", prUrl.c_str(),
              static_cast<long long>(lastSeenCheckRunId));
    outError.clear();
    try {
        // Idempotent — UPDATE on a missing row returns 0 rows changed and is
        ...
        stmt.bind(2, static_cast<long long>(NowEpochSec()));
        stmt.bind(3, prUrl);
        LOG_DEBUG("SQL update agent_open_pr_watch.last_seen_check_run_id prUrl=%s val=%lld", prUrl.c_str(),
                  static_cast<long long>(lastSeenCheckRunId));
        stmt.exec();
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::SetOpenPrCheckRunCursor: ") + ex.what();
        LOG_ERROR("AgentProposalStore::SetOpenPrCheckRunCursor exception (prUrl=%s): %s", prUrl.c_str(), ex.what());
        return false;
    }
}
...
bool AgentProposalStore::IncrementOpenPrIterationCount(const std::string& prUrl, int& outNewCount,
                                                       std::string& outError) {
    LOG_TRACE("AgentProposalStore::IncrementOpenPrIterationCount enter (prUrl=%s)", prUrl.c_str());
    outError.clear();
    outNewCount = 0;
    try {
        ...
        int current = -1;
        {
            ...
            if (!q.executeStep()) {
                outError = "row not found";
                LOG_WARN("AgentProposalStore::IncrementOpenPrIterationCount exit reason=row_not_found (prUrl=%s)",
                         prUrl.c_str());
                return false;
            }
            current = q.getColumn(0).getInt();
        ...
        const int next = current + 1;
        {
            ...
            u.bind(3, prUrl);
            LOG_DEBUG("SQL update agent_open_pr_watch.iteration_count prUrl=%s %d -> %d", prUrl.c_str(), current, next);
            const int changed = u.exec();
            if (changed == 0) {
                outError = "row vanished between SELECT and UPDATE";
                LOG_WARN("AgentProposalStore::IncrementOpenPrIterationCount exit reason=row_vanished (prUrl=%s)",
                         prUrl.c_str());
                return false;
            }
        }
        transaction.commit();
        outNewCount = next;
        LOG_INFO("AgentProposalStore::IncrementOpenPrIterationCount prUrl=%s iter=%d -> %d", prUrl.c_str(), current,
                 next);
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::IncrementOpenPrIterationCount: ") + ex.what();
        LOG_ERROR("AgentProposalStore::IncrementOpenPrIterationCount exception (prUrl=%s): %s", prUrl.c_str(),
                  ex.what());
        return false;
    }
}
...
bool AgentProposalStore::SetHandoffStatus(std::int64_t proposalId, const std::string& status, std::string& outError) {
    LOG_TRACE("AgentProposalStore::SetHandoffStatus enter (proposalId=%lld status=%s)",
              static_cast<long long>(proposalId), status.c_str());
    outError.clear();
    try {
        SQLite::Statement stmt(*db_, "UPDATE agent_proposals SET handoff_status=? WHERE id=?");
        stmt.bind(1, status);
        stmt.bind(2, static_cast<long long>(proposalId));
        LOG_DEBUG("SQL update agent_proposals.handoff_status proposalId=%lld status=%s",
                  static_cast<long long>(proposalId), status.c_str());
        const int changed = stmt.exec();
        if (changed == 0) {
            outError = "row not found";
            LOG_WARN("AgentProposalStore::SetHandoffStatus exit reason=row_not_found (proposalId=%lld)",
                     static_cast<long long>(proposalId));
            return false;
        }
        LOG_INFO("AgentProposalStore::SetHandoffStatus proposalId=%lld status=%s", static_cast<long long>(proposalId),
                 status.c_str());
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::SetHandoffStatus: ") + ex.what();
        LOG_ERROR("AgentProposalStore::SetHandoffStatus exception (proposalId=%lld): %s",
                  static_cast<long long>(proposalId), ex.what());
        return false;
    }
}
...
bool AgentProposalStore::GetHandoffStatus(std::int64_t proposalId, std::string& outStatus,
    ...
    LOG_TRACE("AgentProposalStore::GetHandoffStatus enter (proposalId=%lld)", static_cast<long long>(proposalId));
    outError.clear();
    outStatus.clear();
    try {
        ...
        if (!q.executeStep()) {
            outError = "row not found";
            LOG_WARN("AgentProposalStore::GetHandoffStatus exit reason=row_not_found (proposalId=%lld)",
                     static_cast<long long>(proposalId));
            return false;
        }
        outStatus = q.getColumn(0).getText();
        LOG_DEBUG("AgentProposalStore::GetHandoffStatus proposalId=%lld -> status=%s",
                  static_cast<long long>(proposalId), outStatus.c_str());
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::GetHandoffStatus: ") + ex.what();
        LOG_ERROR("AgentProposalStore::GetHandoffStatus exception (proposalId=%lld): %s",
                  static_cast<long long>(proposalId), ex.what());
        return false;
    }
}
...
bool AgentProposalStore::SetPollCursor(const std::string& sourceTracker, const std::string& repoKey,
    ...
    LOG_TRACE("AgentProposalStore::SetPollCursor enter (sourceTracker=%s repoKey=%s lastSeenUpdatedAt=%lld)",
              sourceTracker.c_str(), repoKey.c_str(), static_cast<long long>(lastSeenUpdatedAt));
    outError.clear();
    try {
        ...
        LOG_DEBUG("SQL upsert agent_poll_cursor sourceTracker=%s repoKey=%s lastSeenUpdatedAt=%lld",
                  sourceTracker.c_str(), repoKey.c_str(), static_cast<long long>(lastSeenUpdatedAt));
        stmt.exec();
        return true;
    } catch (const std::exception& ex) {
        outError = std::string("AgentProposalStore::SetPollCursor: ") + ex.what();
        LOG_ERROR("AgentProposalStore::SetPollCursor exception (sourceTracker=%s repoKey=%s): %s",
                  sourceTracker.c_str(), repoKey.c_str(), ex.what());
        return false;
    }
}
