...
static void JqlAcpTryFlushPendingToViewJql(UiDrawSession& d) {
    ...
    s.replace(static_cast<size_t>(d.jqlAcpReplaceStart), static_cast<size_t>(d.jqlAcpReplaceEnd - d.jqlAcpReplaceStart),
              ins);
    CopyStringToBuffer(d.viewJqlBuf, s);
...
static void ApplyViewsActiveJqlFromBuffers(AppController& app, UiDrawSession& d, Views& viewState,
...
void SmatchetUI::drawViewsDashboardWindow(AppController& app, UiDrawSession& d) {
    ...
    auto loadBuffersFromView = [&](const ViewDefinition& view) {
        ...
        const std::string selectedFieldsCsv = JoinCsvLocal(view.Fields);
    ...
    if (activeView) {
        ...
        ImVec2 jqlFieldRectSize{};
        {
            ...
            if (ImGui::Button("Apply JQL")) {
                ApplyViewsActiveJqlFromBuffers(app, d, ViewState, *activeView);
            }
        ...
        const auto syncSelectedFieldsBuffer = [&]() {
            ...
            const std::string csv = JoinCsvLocal(updated);
            ...
            if (ImGui::BeginPopupContextItem()) {
                if (key != "id" && ImGui::MenuItem("Remove column from view")) {
                    ...
                    if (key.rfind("field:", 0) == 0) {
                        ...
                        const std::string csv = JoinCsvLocal(fields);
                        CopyStringToBuffer(d.selectedFieldsBuf, csv);
...
// Returns the plain-text string that a grid cell should expose when copying the
// rectangular selection to the clipboard. Mirrors what the user sees rendered
// for read-only cells; date/datetime fields are formatted the same compact way.
