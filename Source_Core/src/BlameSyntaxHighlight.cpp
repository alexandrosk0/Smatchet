...
bool IsKeyword(const std::string& w) {
    static const char* kws[] = {
        ...
        "xor", "xor_eq", "override", "final", "import", "module", "uint8_t", "uint16_t", "uint32_t",
        "uint64_t", "int8_t", "int16_t", "int32_t", "int64_t", "size_t", "ssize_t", "nullptr_t"};
...
void BlameDrawColoredCppLine(const char* utf8Line, const BlameUiThemeColors& theme) {
    ...
    while (i < n) {
        ...
        if (c >= '0' && c <= '9') {
            size_t j = i + 1;
            while (j < n &&
                   ((utf8Line[j] >= '0' && utf8Line[j] <= '9') || utf8Line[j] == '.' || utf8Line[j] == 'x' ||
