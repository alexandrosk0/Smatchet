...
Result RunOnUiThread(AppController& app, Fn fn) {
...
CommandResult RunOnUiThreadAsCommandResult(AppController& app, Fn fn) {
    try {
        return RunOnUiThread<CommandResult>(app, std::move(fn));
    } catch (const std::exception& e) {
        return CommandResult::Failure(ErrorCode::HandlerError,
            std::string("UI-thread handler threw: ") + e.what());
    } catch (...) {
        return CommandResult::Failure(ErrorCode::HandlerError,
            "UI-thread handler threw (unknown exception).");
    }
}
