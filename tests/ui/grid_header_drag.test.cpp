...
bool JiraFixtureConfigured() {
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4996) // getenv: portable; _dupenv_s is MSVC-only
#endif
...
// Left/right edges of the horizontally scrolling part of the grid (right of the frozen "id").
void ScrollStrip(const ImGuiTable& table, float& minX, float& maxX) {
    minX = table.InnerClipRect.Min.x;
    maxX = table.InnerClipRect.Max.x;
    for (int order = 0; order < table.FreezeColumnsRequest; ++order) {
...
bool DragHeaderIntoEdgeAndHold(ImGuiTestContext* ctx, const std::string& key, bool toRight, int wantOrder) {
    ...
    ScrollStrip(*table, stripMinX, stripMaxX);
...
void RegisterGridHeaderDragAcrossScroll(ImGuiTestEngine* engine) {
    ...
    t->TestFunc = [](ImGuiTestContext* ctx) {
        ...
        g_ui.cfg.WhisperSetupCompleted = true; // keep the first-run banner off the header row
        app->SyncWithBackend();
        ...
        const int lastOrder = static_cast<int>(original.size()) - 1;
        ...
        // Make the grid overflow horizontally: widen the first movable column by dragging its
        // right border to the right edge, pushing every later column off screen.
        {
            ImGuiTable* table = LiveTicketGrid();
            IM_CHECK_NO_RET(table != nullptr);
            if (table == nullptr) {
                return;
            }
            float stripMinX = 0.0f;
            ...
            ScrollStrip(*table, stripMinX, stripMaxX);
            const float y = HeaderRowY(*table);
            ctx->MouseMoveToPos(ImVec2(table->Columns[IndexOfKey(dragged)].MaxX - 1.0f, y));
            ctx->MouseDown(ImGuiMouseButton_Left);
            ctx->MouseMoveToPos(ImVec2(stripMaxX - 8.0f, y));
            ctx->MouseUp(ImGuiMouseButton_Left);
        }
        ...
        if (!DragHeaderIntoEdgeAndHold(ctx, dragged, /*toRight=*/true, lastOrder)) {
            return;
        }
