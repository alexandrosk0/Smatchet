...
- The bucket-C/E lane is bootable and gating merges again, re-arming the scenario TUs + 5.3K LOC of existing harness and closing the Pillar-4 visual-validation gap that previously fell to humans. (Historically this section's highest-leverage action was *restoring* that lane — now done.) The residual UI-coverage work is continued `_detail` extraction to pull draw-layer logic into the ctest-instrumented denominator. *2026-07-14:* `SmatchetAutocompleteUi_detail` grew the JQL value-quoting helpers (`ValueNeedsQuotesForId` / `QuotedJqlStyle` / `InsertTokenForUserAccountId`) and the `\x7F` caret-anchor `StripCaretAnchorSentinel`, lifted out of the `SmatchetAutocompleteUi.cpp` anon namespace into the already-tested sibling (TU count unchanged — the shell stays a UI TU — but the string logic is now bucket-A tested).
