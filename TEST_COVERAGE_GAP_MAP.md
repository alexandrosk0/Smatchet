...
Additionally, the bucket-C/E scenario lanes (the intended coverage story for the UI draw layer and the 30 `Commands/Scenarios/*` TUs) were **dropped from the blocking check set on 2026-06-15** because the Mesa-GL CI runner couldn't boot the exe (`AGENTS.md` § Merge gates). *Update 2026-07-05:* the lanes boot and pass again on PR CI, and the merge-gate poller has since flipped to **block-on-any-red** (`agents/scripts/core/merge-gates.sh` — `MERGE_GATES_BLOCK_ALLOWLIST_RE="."` retires the curated allow-list; only a check whose name contains "advisory" is exempt). The bucket lanes therefore block merges again — Tier 5's restoration step is DONE; what remains for the UI layer is continued `_detail` extraction plus automating the 22 pending V-series manual smokes onto the now-gating lanes.
...
> **Campaign status (2026-07-05, PR #1618):** DONE at the guard-contract level — the `command-contract-sweep` scenario dispatches every registered command through the real Dispatch guard path (missing-required-arg / validation-error / confirm-required / unknown-command) mutation-free, with an auto-enrolled driver (`scripts/dev/test-command-contract.sh`). Happy-path per-command invocation against the fixture backend remains a flagged follow-up. Plan doc: `docs/plans/coverage-gap-tier3-command-contract.md`.
