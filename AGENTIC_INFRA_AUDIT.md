...
| C6 | P2 | MCP live-HTTP `Authorize` path (DNS-rebind gate, SSE cap) tested only via pure helpers, never over a real socket | backlog entry → **test shipped 2026-07-13** (`tests/ui/mcp_live_http_auth.test.cpp`; awaits a Windows/CI bucket-E run to compile+execute) |
