...
**Opportunities, not defects (unchanged):** B6/B7 (MCP protocol primitives, registry polish), C3 (declared environment capability tiers), Proposals **P2/P3** (MCP `resources`/`prompts` primitives), **P5/P6** (capability tiers + Linux doctest lane), **P7** (counts-by-reference lint), **P8** (postmortem→eval flywheel), **P9** (clangd-index nav). These are design-dependent enhancements, not audit defects — opt-in, not carried as open work.
...
|---|---|---|---|
| A1 | P2 | `AGENTS.md` is 159 lines against its own ≤150 contract budget | backlog entry → remediated 2026-07-08 (trimmed to 149) → **regressed to 151 + gate-blindness root-caused & fixed 2026-07-13** |
...
> **Correction (2026-07-13): A1's remediation was illusory, and the reason was a gate bug.** The 2026-07-08 pass trimmed `AGENTS.md` to 149 and the applied-entry claimed "cap binding again" — but it never was. `AGENTS.md` documents the deviation escape-hatch in backtick prose, and `agent_size_audit.py`'s `_suppressed()` matched that prose as a real deviation marker, so the gate treated the contract file as *permanently self-exempt*. The file drifted straight back over 150 (#1753 +1, #1764 +1 = 151) with the delta gate exiting 0 on both crossings and no fire. A textbook instance of this report's own thesis: the gate anchoring the enforcement contract-card was structurally blind to the contract file. Fixed here — the matcher now requires a real `<!-- SMATCHET_DEVIATION(...) -->` HTML comment (backtick code spans stripped first), `AGENTS.md` is trimmed to 149, and a selftest + bats case lock the prose-must-not-suppress semantics. Tracked as `tooling/2026-07-13-agent-size-gate-prose-self-suppression.md`.
...
| B5 | P2 | Lua `ai.*` called from the background automation worker races `luaContext_` | GitHub Issue candidate → **Issue #1678 filed 2026-07-08, closed-completed 2026-07-09** (fix shape (b): `ai.*` bound only on the UI-thread Lua state; worker/MCP scripts see `nil`) |
...
| C6 | P2 | MCP live-HTTP `Authorize` path (DNS-rebind gate, SSE cap) tested only via pure helpers, never over a real socket | backlog entry → **RESOLVED 2026-07-13** (`tests/ui/mcp_live_http_auth.test.cpp`; CI-verified on the `Bucket-E UI tests` Mesa-GL lane, PR #1812) |
...
**C6 — the best security engineering in the repo is tested one layer below where it binds.** `IsMcpHostOriginAllowed`, constant-time compare, and the SSE cap have solid pure-helper doctests, but no test drives `Authorize` over a real `httplib` socket with a hostile `Host:`/`Origin:` header or races the SSE cap. The repo already owns the exact fixture shape (`JiraCatalogHttpFixture.h` — in-process httplib loopback server). Filed as `test/2026-07-06-mcp-live-http-auth-direct-test.md`. **Resolved 2026-07-13**: bucket-E TU `tests/ui/mcp_live_http_auth.test.cpp` (`McpLiveHttp/Authorize_RealSocket`) starts a second `McpPlugin` on its own port and asserts over a real `httplib::Client` — 200 with a valid token, 401 without/with a wrong token, 403 on a DNS-rebind `Host:` even with a valid token (the Host gate precedes the token check), 403 on a cross-origin `Origin:`, and 503 once the SSE cap is saturated. **CI-verified** on the `Bucket-E UI tests (Mesa headless GL)` lane (PR #1812). The verification loop itself became a live demonstration of finding **C3**: the Linux-container authoring session could not build the bucket-E rig, so CI caught two MSVC `/W4 /WX` warnings it was blind to (`C2446` on `res != nullptr`, `C4456` on the `IM_CHECK`-macro `res` shadow) — each a fix + CI round-trip. Note the regular `Windows + MSVC` lane does **not** compile `tests/ui/` (opt-in `SMATCHET_BUILD_UI_TESTS`); only the bucket-E lanes exercise this test.
